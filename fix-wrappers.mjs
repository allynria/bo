...
if (!inFile || !outFile) { console.error("Usage: node fix-wrappers.mjs <in.js> <out.js>"); process.exit(2); }
...
function wrapWithIIFE(stmtNode, catchBodyRaw) {
  ...
  const cAst = parser.parse(`try{}catch(err){${catchBodyRaw}}`, { sourceType: "module", errorRecovery: true });
  let catchClause = null;
  traverse(cAst, { CatchClause(p){ catchClause = p.node; p.stop(); } });
  const iife = t.callExpression(
    t.arrowFunctionExpression(
      [], t.blockStatement([ t.tryStatement(tryBlock, catchClause || t.catchClause(t.identifier("err"), t.blockStatement([]))) ]),
      true
    ), []
  );
