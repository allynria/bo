...
export async function runDisagreementCore(
  ctx,
  { convId, userText, beliefLines, contradictionLines }
) {
  ...
  if (contradictionLines?.length)
    reasons.push({ code: 'contradiction', msg: 'conflicts with recent events' });
  ...
  try {
    reasons.push(...rulesCritique({ userText, beliefs: beliefsList, recentFacts }).reasons);
  } catch {}
  ...
  try {
    ...
    spine = { mood: 'neutral', trust: 0.5, suspicion: 0.1 };
  }
  ...
  if (hasBlock && enforce === 'hard') {
    const style =
      spine.mood === 'hostile' || spine.mood === 'annoyed'
        ? 'firm'
        : spine.mood === 'guarded'
          ? 'guarded'
          : 'soft';
    lines.push(
      buildRefusalBooster({
        ...
        style,
        mood: spine.mood,
        reason: appendWhy ? reasons.map((r) => r.msg).join('; ') : undefined,
      })
    );
    action = 'refuse';
  } else if (hasBlock) {
    ...
    lines.push(
      `(Continuity check: ${reasons.map((r) => r.msg).join('; ')}. Keep the world and character consistent.)`
    );
    action = 'constrain';
  }
  ...
  if (hasBlock) {
    try {
      for (const r of reasons)
        ctx?.metrics?.inc?.('disagreement_core_trigger_total', {
          count: 1,
          source: r.code || 'other',
        });
    } catch {}
    try {
      ctx?.io?.events?.emit?.('disagree.core', {
        action,
        reasons,
        mood: spine.mood,
        trust: spine.trust,
        suspicion: spine.suspicion,
      });
    } catch {}
  }
