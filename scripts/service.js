...
import {
  ...
  stateIO,
  TokenCounter,
  createSharedTenantDollarBudget,
  createSharedTenantBudget,
  checkSecrets,
  startBudgetGC,
  getEffectiveConfig,
  DisagreementCore,
  BeliefStore,
  buildRefusalHint,
  checkContradictionsHeuristic,
  makeDeterministicRoll,
  MessageClock,
...
import {
  createSharedTenantMonthlyBudget,
  createSharedTenantMonthlyDollarBudget,
  createSharedTenantRollingBudget,
  createSharedTenantRollingDollarBudget,
} from '../monolith.js';
import {
  nextLoopStyleToken,
  setRefusalStyleForAgent,
  getRefusalStyleForAgent,
} from '../monolith.js';
...
import {
  ...
  getGuardHint,
  consumeGuardHint,
  generateGuardOneLiner,
} from './memory/guardrail.mjs';
...
import {
  ...
  d100 as d100FR,
  buildOutcomeBooster,
} from './failroll/failroll.mjs';
import {
  isNearMiss,
  buildComplicationBooster,
  applyBeatTensionDelta,
  classifyVerbStyle,
} from './failroll/complication.mjs';
...
import {
  getConfigFromEnv as getLoopGuardConfig,
  loopGuardDecide,
  loopGuardHistoryAPI,
} from './loopguard/loopguard.mjs';
...
import {
  getStyleForBeat,
  buildStyleBooster as buildBeatStyleBooster,
} from './scene/beat_style_map.mjs';
...
const FAILURE_RISK_VERBS = (
  process.env.FAILURE_RISK_VERBS || 'sneak,steal,attack,climb,jump,disarm,pickpocket'
)
  .split(',')
...
export async function computeDisagreementGuards(ctx, { conv_id, turn, userText }) {
  ...
  const style =
    (personality.includes('sarcas') && 'sarcastic') ||
    (personality.includes('blunt') && 'blunt') ||
    (personality.includes('gentle') && 'soft') ||
    (trust < REFUSAL_MIN_TRUST ? 'firm' : 'soft');
  ...
  if (beliefHit) {
    guards.push({
      type: 'belief',
      text: String(beliefHit || ''),
      hint: buildRefusalHint({ character: ctx.vars?.agent_name, style, belief: beliefHit }),
    });
  }
  ...
  if (constraintHit) {
    guards.push({
      type: 'constraint',
      text: String(constraintHit || ''),
      hint: buildRefusalHint({ character: ctx.vars?.agent_name, style, constraint: constraintHit }),
    });
  }
  ...
  contradictions.forEach((c) => {
    guards.push({
      ...
      why: String(c.why || ''),
      fact: String(c.fact || ''),
      hint: buildRefusalHint({ character: ctx.vars?.agent_name, style, reason: c.why }),
    });
  });
  ...
  if (FAILURE_ROLL_ENABLED) {
    ...
    if (risky) {
      ...
      const adj =
        base -
        Math.round(trust * FAILURE_TRUST_WEIGHT) +
        Math.round(suspicion * FAILURE_FEAR_WEIGHT);
...
import {
  normalizeStyle as normalizeRefusalStyle,
  renderRefusal,
} from './refusal/refusal_templates.mjs';
import {
  listRefusalStyles,
  getRefusalPref,
  setRefusalPref,
  getRefusalHint,
} from './refusal/prefs.mjs';
...
import {
  ...
  deleteFact,
  consolidateAll,
  consolidateAllWithStats,
  putFact,
} from './memory/facts_store.mjs';
...
import {
  ...
  addBeliefLine as addBeliefLineState,
  deleteBeliefLine as deleteBeliefLineState,
  pickRelevantBeliefs as pickRelevantStateBeliefs,
  detectDisallowedAction as detectDisallowedActionState,
} from './state/beliefs_store.mjs';
...
function detectActionTag(text) {
  ...
  for (const [name, rx] of patterns) {
    if (rx.test(String(text || ''))) return name;
  }
  return '';
}
...
function maybeTagCooledPhrases(ctx, convId, finalText) {
  try {
    ...
    if (isCooled(convId, String(finalText || ''))) {
      try {
        ctx.io?.events?.emit?.('loopguard.cooldown.hit', { conv_id: String(convId || '') });
      } catch {}
      try {
        METRICS.inc('loopguard_phrase_cooldown_hits_total', { count: 1 });
      } catch {}
      try {
        ctx.vars = ctx.vars || {};
        ctx.vars.__nextTurnBoosters = Array.isArray(ctx.vars.__nextTurnBoosters)
          ? ctx.vars.__nextTurnBoosters
          : [];
        ctx.vars.__nextTurnBoosters.push(
          '(Vary your phrasing; avoid the most obvious wording choices.)'
        );
      } catch {}
    }
  } catch {}
...
async function maybeRerollOnCooldown(
  ctx,
  { convId, textInput, outText, llm, model, memoryPrefix, styleTokens }
) {
  try {
    ...
    if (String(outText || '').length > maxChars) {
      try {
        METRICS.inc('loopguard_cooldown_reroll_skipped_total', {
          reason: 'too_long',
          path: String(ctx?.vars?.path || 'message'),
        });
      } catch {}
      // Emit observation frame without reroll
      try {
        ctx.io?.events?.emit?.('loopguard.cooldown', {
    ...
    try {
      ...
      if (beat)
        booster = `(STYLE:${chosen}) (${beat}) Express this idea differently. Avoid familiar phrasing patterns; add a fresh sensory detail.`;
    } catch {}
    ...
    for (let i = 0; i < retryLimit; i++) {
      ...
      try {
        ctx.io?.events?.emit?.('loopguard.cooldown', {
          ...
          style: String(chosen || ''),
          retry: retries,
          reason: 'cooldown_reroll',
        });
      } catch {}
      try {
        METRICS.inc('loopguard_cooldown_trigger_total', {
          path: String(ctx?.vars?.path || 'message'),
          style: String(chosen || ''),
    ...
    if (!afterHit) {
      try {
        METRICS.inc('loopguard_cooldown_reroll_win_total', {
          path: String(ctx?.vars?.path || 'message'),
          style: String(chosen || ''),
        });
      } catch {}
      return {
        text: String(alt || outText || ''),
        rerolled: true,
        beforeHit: true,
        afterHit: false,
      };
      ...
      try {
        METRICS.inc('loopguard_cooldown_reroll_lose_total', {
          path: String(ctx?.vars?.path || 'message'),
          style: String(chosen || ''),
        });
      } catch {}
    ...
    try {
      METRICS.inc('loopguard_cooldown_reroll_errors_total', {
        path: String(ctx?.vars?.path || 'message'),
      });
    } catch {}
...
async function maybeApplyFailureRoll(ctx, { convId, turn, userText }) {
  try {
    ...
    const forceMode =
      String(getForceFailMode(convId) || '').toLowerCase() ||
      (ctx?.vars?.__force_fail_roll ? 'on' : '');
    ...
    let spine = null;
    try {
      spine = await loadSpine(convId, 'bot');
    } catch {
      spine = null;
    }
    ...
    const vb =
      /\b(sneak|pick|lie|steal|dodge|parry|charm|intimidat|bluff|climb|jump|run|dash|shoot|grapple|hack)\b/i.exec(
        String(userText || '')
      )?.[0] || 'attempt';
    ...
    const reasonRaw =
      forceMode === 'on' || forceMode === 'once'
        ? `forced, failProb=${(pFail * 100).toFixed(0)}%, roll=${roll}`
        : `failProb=${(pFail * 100).toFixed(0)}%, roll=${roll}`;
    ...
    try {
      ctx.io?.events?.emit?.('failroll.cooldown', { convId, verb: vb, cooldownPenalty });
    } catch {}
    try {
      if (Number(cooldownPenalty || 0) > 0)
        METRICS.inc('failroll_verb_cooldown_hits_total', { verb: vb });
    } catch {}
    ...
    if (!fail && complicationOk && isNearMiss({ roll, pFailPercent: pFail * 100, band })) {
      ...
      try {
        ...
        if (SYNERGY_ON) {
          ...
          const pool =
            Array.isArray(NEARMISS_PHRASE_TABLE?.[key]) && NEARMISS_PHRASE_TABLE[key].length
              ? NEARMISS_PHRASE_TABLE[key]
              : NEARMISS_PHRASE_TABLE.generic || [];
          const shuffled = [...pool].sort(
            () => (globalThis.__RNG__ ? globalThis.__RNG__() : makePRNG()()) - 0.5
          );
          const chosen = shuffled.slice(0, pickN);
          if (chosen.length > 0) {
            try {
              coolPhrases(convId, chosen, ttlMs);
            } catch {}
            try {
              ctx.io?.events?.emit?.('loopguard.cooldown', {
                conv_id: String(convId || ''),
            ...
            try {
              METRICS.inc('loopguard_phrase_cooldowns_total', {
                count: String(chosen.length),
                style: String(styleClass || 'generic'),
              });
            } catch {}
            try {
              METRICS.inc('near_miss_phrase_cooled_total', {
                style: key,
                count: String(chosen.length),
              });
            } catch {}
    ...
    const tensionAfter = Math.max(
      0,
      Math.min(1, tensionBefore + (Number.isFinite(delta) ? delta : 0))
    );
    try {
      ctx.vars.tension = tensionAfter;
    } catch {}
    ...
    if (hadComplication) {
      ...
      try {
        ...
        try {
          ctx.vars.__complication_fact_text = factText;
        } catch {}
      } catch {}
    }
    ...
    // Spine dynamics
    try {
      ctx.vars.__failroll_outcome__ = fail
        ? 'fail'
        : hadComplication
          ? 'success_complication'
          : 'success';
    } catch {}
    if (!fail) {
      try {
    ...
    const result = {
      lines,
      outcome: fail ? 'fail' : hadComplication ? 'success_complication' : 'success',
      eval: evalPayload,
    };
    try {
      markVerbUsage(convId, vb);
    } catch {}
    return result;
  } catch {
...
import {
  detectContradictions as detectContradictionsState,
  buildContradictionLines as buildContradictionLinesState,
} from './state/contradiction_watchdog.mjs';
...
function getRefusalStyle(convId) {
  ...
  function styleFor(ctx) {
    try {
      return ctx?.vars?.spine?.tone || 'firm';
    } catch {
      return 'firm';
    }
  }
  function pickRefusal(style, critic, ctx) {
    ...
    return pickRefusalTemplate({
      locale,
      persona,
      tone: String(style || 'firm'),
      reason: String(critic?.reason || 'the rules'),
    });
  }
}
...
function decideDisagreement(ctx, { userText }) {
  ...
  const logicViolation =
    /(ignore gravity|resurrect|time travel|teleport without|immediately undo death)/i.test(text);
...
function setLoopBreak(convId, turns = 3) {
  LOOPBREAK.set(String(convId || ''), { turnsLeft: turns });
}
...
const BEAT_ENABLED =
  String(process.env.BEAT_ENABLED || '').toLowerCase() === '1' ||
  String(process.env.BEAT_ENABLED || '').toLowerCase() === 'true';
const BEAT_SSE = String(process.env.BEAT_SSE || '').toLowerCase() !== '0';
let lastBeatStateByConv = new Map();

// Beat-driven style injection knobs
const BEAT_STYLE_ENABLED =
  String(process.env.BEAT_STYLE_ENABLED || '').toLowerCase() === '1' ||
  String(process.env.BEAT_STYLE_ENABLED || '').toLowerCase() === 'true';
const BEAT_STYLE_ALWAYS_ON =
  String(process.env.BEAT_STYLE_ALWAYS_ON || '').toLowerCase() === '1' ||
  String(process.env.BEAT_STYLE_ALWAYS_ON || '').toLowerCase() === 'true';
...
const CADENCE_ENABLED =
  String(process.env.CADENCE_ENABLED || '').toLowerCase() === '1' ||
  String(process.env.CADENCE_ENABLED || '').toLowerCase() === 'true';
const CADENCE_ALWAYS_ON =
  String(process.env.CADENCE_ALWAYS_ON || '').toLowerCase() === '1' ||
  String(process.env.CADENCE_ALWAYS_ON || '').toLowerCase() === 'true';
...
function frEnabled() {
  try {
    return FAILROLL_OVERRIDE_ENABLED !== null
      ? Boolean(FAILROLL_OVERRIDE_ENABLED)
      : String(process.env.FAILROLL_ENABLED || '0') === '1';
  } catch {
    return String(process.env.FAILROLL_ENABLED || '0') === '1';
  }
}
...
const CADENCE_OBS_BUCKETS = String(process.env.CADENCE_OBS_BUCKETS || '5,10,15,20,30,40')
  .split(',')
  .map((x) => Number(x))
  .filter((x) => x > 0);
const LOOP_CADENCE_ENFORCE =
  String(process.env.LOOP_CADENCE_ENFORCE || '').toLowerCase() === '1' ||
  String(process.env.LOOP_CADENCE_ENFORCE || '').toLowerCase() === 'true';
const LOOP_CADENCE_RETRY_LIMIT = Number(process.env.LOOP_CADENCE_RETRY_LIMIT || 1);
const CADENCE_STRICT_NOTE = String(
  process.env.CADENCE_STRICT_NOTE || 'strict cadence control; avoid filler and repetition'
);
const CADENCE_STRICT_PREFIX_MODE = String(
  process.env.CADENCE_STRICT_PREFIX_MODE || CADENCE_PREFIX_MODE || 'memory'
).toLowerCase();
...
function applyBeatStyleBooster({ ctx, conv_id, userText, currentBeat }) {
  try {
    ...
    try {
      METRICS.inc('style_booster_applied_total', { mode, state });
    } catch {}
    try {
      broadcastAdminStyleEvent(String(conv_id || ''), 'style.applied', {
        conv_id,
        state,
        style: style.token,
        line,
    ...
    try {
      METRICS.inc('style_booster_errors_total', { path: 'compute' });
    } catch {}
    return null;
  }
}
...
function emitCadenceObserved({ writeSSE, conv_id, beatState, target, observed }) {
  try {
    ...
    try {
      METRICS.inc('cadence_observed_total', { outcome, beat: String(beatState || 'unknown') });
    } catch {}
    const edges = [...CADENCE_OBS_BUCKETS, Number.POSITIVE_INFINITY];
    const e = edges.find((x) => avg <= x);
    try {
      METRICS.inc('cadence_avg_ms_bucket', { le: String(e), beat: String(beatState || 'unknown') });
    } catch {}
    return { outcome, delta, absDelta };
  } catch {
    try {
      METRICS.inc('cadence_observed_errors_total', { path: 'message' });
    } catch {}
    return null;
  }
}
  ...
  snapshot() {
    ...
    for (const [k, v] of Object.entries(this.counters)) {
      ...
      const labels = Object.fromEntries(
        (labelStr ? labelStr.split(',') : []).filter(Boolean).map((p) => {
          const [a, b] = p.split('=');
          return [a, b];
        })
      );
    ...
    try {
      const sumByName = (n) =>
        out.filter((c) => c.name === n).reduce((acc, c) => acc + Number(c.value || 0), 0);
      const sumByNameAndPath = (n, p) =>
        out
          .filter((c) => c.name === n && String(c.labels?.path || '') === String(p))
      ...
      // Include both legacy 'auth_ok_total' and 'auth_accepted_total' in ok summary
      out.push({
        name: 'summary_auth',
        labels: { kind: 'ok' },
        value: sumByName('auth_ok_total') + sumByName('auth_accepted_total'),
      });
      out.push({
        name: 'summary_auth',
        labels: { kind: 'blocked' },
        value: sumByName('auth_blocked_total'),
      });
      out.push({
        name: 'summary_auth',
        labels: { kind: 'failed' },
        value: sumByName('auth_failed_total'),
      });
      out.push({
        name: 'summary_rate',
        labels: { kind: 'limited' },
        value: sumByName('rate_limited_total'),
      });
      for (const [st, vv] of Object.entries(respByStatus)) {
        out.push({ name: 'summary_responses', labels: { status: st }, value: vv });
      }
      // Path-specific auth summaries for compile and admin:beliefs:get
      const paths = ['compile', 'admin:beliefs:get'];
      for (const p of paths) {
        const okV =
          sumByNameAndPath('auth_ok_total', p) + sumByNameAndPath('auth_accepted_total', p);
        const blockedV = sumByNameAndPath('auth_blocked_total', p);
        const failedV = sumByNameAndPath('auth_failed_total', p);
        out.push({ name: 'summary_auth', labels: { kind: 'ok', path: p }, value: okV });
        out.push({ name: 'summary_auth', labels: { kind: 'blocked', path: p }, value: blockedV });
        out.push({ name: 'summary_auth', labels: { kind: 'failed', path: p }, value: failedV });
      }
    } catch {}
    return out;
  },
...
const STREAM_DURATION_MS_BUCKETS = [
  500, 800, 1200, 2000, 3000, 5000, 8000, 12000, 20000, 30000, 60000,
];
...
function registerAdminMemorySSE(convId, res) {
  ...
  if (!set) {
    set = new Set();
    ADMIN_SSE_MEMORY.set(convId, set);
  }
  set.add(res);
}
...
function emitAdminMemoryEvent(evt, payload) {
  try {
    METRICS.inc('admin_memory_edit_total');
  } catch {}
  try {
    broadcastAdminMemoryEvent('global', evt, payload);
  } catch {}
}
...
function registerAdminStyleSSE(convId, res) {
  ...
  if (!set) {
    set = new Set();
    ADMIN_SSE_STYLE.set(convId, set);
  }
  set.add(res);
}
...
function registerAdminRefusalSSE(convId, res) {
  ...
  if (!set) {
    set = new Set();
    ADMIN_SSE_REFUSAL.set(convId, set);
  }
  set.add(res);
}
...
function registerAdminSpineSSE(convId, res) {
  ...
  if (!set) {
    set = new Set();
    ADMIN_SSE_SPINE.set(convId, set);
  }
  set.add(res);
}
...
try {
  ...
  try {
    METRICS.set('facts_current', 0);
  } catch {}
  try {
    METRICS.set('facts_max', Math.max(1, Number(process.env.FACTS_MAX || 64)));
  } catch {}
  try {
    METRICS.set('disagreement_core_trigger_total', 0);
  } catch {}
} catch {}
...
function ultraSet(convId, on) {
  ...
  try {
    METRICS.inc('ultra_toggled_total', { on: on ? '1' : '0' });
  } catch {}
...
function maybeStageConclusionBooster(ctx, convId, memoryPrefixRef, sseEmit, pathLabel) {
  try {
    ...
    if (staged && staged.staged) {
      ...
      try {
        ctx.vars = ctx.vars || {};
        ctx.vars.__memory_prefix = nextMp;
      } catch {}
      try {
        ctx.vars.__scene_conclusion_staged = true;
      } catch {}
      try {
        METRICS.inc('scene_conclusion_staged_total', {
          style: String(staged.style || ''),
          path: String(pathLabel || ''),
        });
      } catch {}
      if (typeof sseEmit === 'function') {
        try {
          sseEmit('scene.conclusion.staged', {
            style: staged.style,
  ...
  try {
    ...
    if (
      String(snap?.lastReason || '') === 'force' &&
      Number(snap?.lastStagedTurn || 0) === Number(snap?.turn || 0) - 1
    ) {
      ...
      try {
        if (chosenStyle === 'cut') {
          booster =
            '(The moment crests; cut on a meaningful beat. Resolve a small thread in one tight paragraph, then pivot with a clear new hook.)';
        } else if (chosenStyle === 'bookmark') {
          booster =
            '(They acknowledge the lull and bookmark this scene with a quiet detail. Conclude gracefully, then seed one fresh direction.)';
        } else {
          // default: fade
          booster =
            '(The scene has run long. Close with a subtle, evocative beat—one paragraph that resolves the current mood, then hint at the next scene.)';
        }
      } catch {}
      const mp = String(memoryPrefixRef || '');
      const nextMp = mp ? `${mp}\n${booster}` : booster;
      ...
      return {
        staged: {
          ...
          style: chosenStyle,
          booster,
          coolUntilTurn: snap?.coolUntilTurn,
        },
        booster,
        memoryPrefix: nextMp,
      };
...
function putFactRt(convId, fact) {
  ...
  const merged = {
    ...
    confirmations:
      typeof fact.confirmations === 'number' ? fact.confirmations : (prev?.confirmations ?? 0),
  ...
  b.lastChangeTs = now;
  try {
    METRICS.inc('facts_updated_total', { op: prev ? 'update' : 'create' });
  } catch {}
  return merged;
}
...
function deleteFactRt(convId, id) {
  ...
  if (existed) {
    ...
    try {
      METRICS.inc('facts_deleted_total');
    } catch {}
  }
  return existed;
}
...
function consolidateAllRt(convId, opts = {}) {
  ...
  try {
    METRICS.inc('facts_consolidated_total', { conv: '1' });
  } catch {}
  try {
    if (deduped > 0) METRICS.inc('facts_deduped_total');
  } catch {}
  try {
    if (archived > 0) METRICS.inc('facts_archived_total');
  } catch {}
...
function selectTopFactsRt(convId, limit) {
  ...
  for (const f of a) {
    ...
    f.__score =
      (f.salience || 0.5) * B + recency * A + (f.confirmations || 0) * G + (f.score || 0.5) * 0.2;
  }
...
function emitMemoryShapeSSE(res, payload) {
  try {
    res.write('event: memory.shape\n');
    res.write(`data: ${JSON.stringify(payload)}\n\n`);
  } catch {}
  try {
    broadcastAdminMemoryEvent(String(payload?.conv_id || ''), 'memory.shape', payload);
  } catch {}
}
...
function reinforceFacts(convId, ids = [], delta, turnIndex) {
  ...
  for (const id of ids || []) {
    ...
    let text0 = '';
    try {
      text0 = prev?.text || (Array.isArray(arr) ? arr.find((f) => f.id === id)?.text || '' : '');
    } catch {}
    ...
    try {
      ...
      if (pf)
        updateFact(convId, id, String(pf.text || ''), {
          weight: Math.min(99, Number(pf.weight || 1) + 1),
        });
    } catch {}
  }
  try {
    if (n > 0) METRICS.inc('facts_reinforced_total');
  } catch {}
  return changed;
}
...
async function vectorUpsertFact(f, conv_id) {
  ...
  try {
    ...
    await fetch(`${base}/upsert`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(body),
    });
    try {
      METRICS.inc('facts_vector_upsert_total');
    } catch {}
    return true;
  } catch {
    return false;
...
async function vectorTopK(conv_id, query, k) {
  ...
  try {
    const body = {
      namespace: process.env.VECTOR_NAMESPACE || 'mem_facts',
      topK: k || Number(process.env.VECTOR_TOPK || 3),
      query,
      filter: { conv_id },
    };
    ...
    try {
      METRICS.inc('facts_vector_query_total');
    } catch {}
...
function pushMemoryFactSSE(res, conv_id, facts) {
  ...
  try {
    broadcastAdminMemoryEvent(conv_id, 'memory.fact', { conv_id, facts });
  } catch {}
}
...
function pushMemoryArcSSE(res, conv_id, arc) {
  ...
  try {
    broadcastAdminMemoryEvent(conv_id, 'memory.arc', { conv_id, arc });
  } catch {}
}
...
async function selectRelevantFactsForTurn(conv_id, currentText, limit, opts = {}) {
  ...
  const SIM = Number(
    process.env.FACT_INJECT_SIM_THRESHOLD ||
      (String(process.env.TEST_MEMORY_API || '') === '1' ? 0.0 : 0.62)
  );
  ...
  try {
    ...
    if (wantArcLinking && arc) {
      all = all.filter((f) =>
        Array.isArray(f.arc_tags) ? f.arc_tags.map(String).includes(arc) : true
      );
    }
  } catch {}
...
function getDreamBucket(convId) {
  ...
  if (!b) {
    b = { queue: [], counts: new Map() };
    DREAMS_RT.set(convId, b);
  }
  return b;
}
...
async function tryPromoteDreams(convId, baseText, { agentId, arc } = {}) {
  ...
  if (seen >= minRepeats) {
    try {
      ...
      const res = addFactWithStats(convId, String(dream), {
        weight,
        agent_id: agentId,
        arc_tags: arcTags,
      });
...
function getSceneBucket(conv) {
  let b = SCENES.get(conv);
  if (!b) {
    b = { current: '', map: new Map() };
    SCENES.set(conv, b);
  }
  return b;
}
...
function enterScene(conv, scene) {
  const b = getSceneBucket(conv);
  if (!scene) return;
  const k = scene.trim().toLowerCase();
  let slot = b.map.get(k);
  if (!slot) {
    slot = { facts: new Set(), boosters: [], lastSeen: 0 };
    b.map.set(k, slot);
  }
  b.current = k;
...
function trySceneChatCommand(conv_id, text) {
  ...
  if (cmd === 'tag' && arg) {
    enterScene(conv_id, arg);
    return { ok: true, action: 'tag', scene: arg };
  }
  if (cmd === 'goto' && arg) {
    enterScene(conv_id, arg);
    return { ok: true, action: 'goto', scene: arg };
  }
...
async function maybeFactGuardNudge(ctx, { conv_id, turnIndex, sseRes = null }) {
  try {
    ...
    const continuity_score =
      typeof ctx?.vars?.continuity_score === 'number' ? Number(ctx.vars.continuity_score) : 0.5;
    if (!conv_id) return;
        ...
        const extra = hits
          ...
          .map((h) => ({
            id: String(h?.id || '')
              .split(':')
              .pop(),
            text: h?.text || h?.metadata?.text,
          }))
          .filter((x) => x && x.text);
        if (extra.length)
          top = [...top, ...extra].slice(0, Number(process.env.FACT_SELECT_LIMIT || 3));
      }
    }
    ...
    const ttlTurns = Math.max(
      1,
      Number(process.env.GUARD_HINT_TTL_TURNS || process.env.GUARD_TTL_TURNS || 2)
    );
    try {
      setGuardHint(conv_id, String(hint), { ttlTurns });
    } catch {}
    try {
      ctx.vars.__facts_last_nudge_turn = Number(turnIndex || 0);
    } catch {}
    try {
      METRICS.inc('guard_hint_from_facts_total', {
        path: String(ctx?.vars?.path || 'unknown'),
      ...
      try {
        if (String(process.env.TEST_MEMORY_API || '').trim() === '1') {
          pushMemoryFactSSE(
            sseRes,
            conv_id,
            top.map((x) => ({ id: x.id, text: String(x.text || '') }))
          );
          // Ensure test looks for facts_injected_total counter sees it
          try {
            for (let i = 0; i < top.length; i++)
              METRICS.inc('facts_injected_total', { path: 'stream', reason: 'guard_nudge' });
          } catch {}
        }
      } catch {}
    }
...
function scheduleFactsConsolidation(conv_id, turnIndex) {
  try {
    ...
    if (b) {
      if (
        Number(turnIndex || 0) - Number(b.lastConsolidatedTurn || 0) >=
        FACT_CONSOLIDATE_EVERY_TURNS
      ) {
        try {
          consolidateAll(conv_id, { turnIndex });
        } catch {}
        b.lastConsolidatedTurn = Number(turnIndex || 0);
      }
    }
    try {
      clearTimeout(_factsDebouncers.get(conv_id));
    } catch {}
    const t = setTimeout(() => {
      try {
        consolidateAll(conv_id, { turnIndex });
...
try {
  ...
  async function ensureUsageDir() {
    try {
      await AsyncFS.mkdir(usageDir, { recursive: true });
    } catch {}
  }
  ...
  function signPayload(payload) {
    try {
      const key = String(
        process.env.USAGE_HMAC_SECRET ||
          process.env.BILLING_SIGNING_KEY ||
  ...
  async function recordUsage(rec) {
    try {
      ...
      try {
        METRICS.inc('usage_ledger_recorded_total', { event: base.event || '' });
      } catch {}
  ...
  async function computeChargebackAlert(tenant, macId, usage) {
    try {
      const threshold = Math.max(
        0,
        Math.min(1, Number(process.env.CHARGEBACK_ALERT_THRESHOLD || 0.9))
      );
      const scopes = [];
      try {
      ...
      try {
        if (globalThis.__TENANT_TOKEN_MONTHLY_BUDGET__) {
          const peek = await Promise.resolve(
            globalThis.__TENANT_TOKEN_MONTHLY_BUDGET__.peek(tenant)
          );
          if (peek && Number(peek.limit || 0) > 0) {
            const ratio = Number(peek.spent || 0) / Number(peek.limit || 1);
      ...
      try {
        if (globalThis.__TENANT_TOKEN_ROLLING_BUDGET__) {
          const peek = await Promise.resolve(
            globalThis.__TENANT_TOKEN_ROLLING_BUDGET__.peek(tenant)
          );
          if (peek && Number(peek.limit || 0) > 0) {
            const ratio = Number(peek.spent || 0) / Number(peek.limit || 1);
      ...
      if (scopes.length > 0) {
        for (const s of scopes) {
          try {
            METRICS.inc('chargeback_alert_total', { scope: s.scope, tenant: tenant || '' });
          } catch {}
        }
        try {
          logAt(
            'warn',
            JSON.stringify({ evt: 'chargeback_alert', tenant, mac_id: macId, usage, scopes })
          );
        } catch {}
      }
    } catch {}
...
try {
  ...
  if (!globalThis.UrgaCoreDeps.Metrics) {
    globalThis.UrgaCoreDeps.Metrics = {
      log(_ctx, name, labels = {}) {
        try {
          ...
          if (name === 'llm_cost' || name === 'cost_usd') {
            const ctx = _ctx || { vars: {} };
            const now = Date.now();
            const tenant = String(ctx?.vars?.tenant || labels?.tenant || '').replace(
              /[^a-zA-Z0-9_\.\-]/g,
              ''
            );
            ...
            try {
              globalThis.__recordUsage__?.(rec);
            } catch {}
            if (name === 'cost_usd') {
              try {
                globalThis.__computeChargebackAlert__?.(tenant, macId, { usd });
              } catch {}
            }
            return;
          }
      ...
      inc(name, _n = 1, labels = {}) {
        try {
          METRICS.inc(String(name), labels || {});
        } catch {}
      },
      count() {
        /* intentionally a no-op to avoid high-volume increments */
      },
    };
  }
} catch {}
...
export function startService({ port = PORT, drainTimeoutMs = 5000 } = {}) {
  ...
  const ACTIVE_STREAMS_MAX_ITEMS = Math.max(
    10,
    Number(process.env.ACTIVE_STREAMS_MAX_ITEMS || 1000)
  );
  ...
  const ABUSE_ALERT_SUSTAINED_BUCKETS = Math.max(
    1,
    Number(process.env.ABUSE_ALERT_SUSTAINED_BUCKETS || 3)
  );
  ...
  function computeAbuseSignals(input) {
    try {
      ...
      let injHits = 0;
      for (const r of injPatterns) {
        if (r.test(text)) injHits++;
      }
      let jbHits = 0;
      for (const r of jbPatterns) {
        if (r.test(text)) jbHits++;
      }
      let fantasyHits = 0;
      for (const r of fantasyPatterns) {
        if (r.test(text)) fantasyHits++;
      }
      ...
      gr = Math.max(
        ...
        Math.min(
          1,
          gr + 0.1 * citeBrackets + 0.08 * urlCount + 0.05 * digitRuns - 0.1 * fantasyHits
        )
      );
      ...
      return {
        ...
        levels: { injection: 'low', jailbreak: 'low', grounding: 'med' },
      };
    }
  }
  ...
  function recordSpike(signalType, level, engine_source) {
    try {
      ...
      for (let i = 0; i < ABUSE_ALERT_SUSTAINED_BUCKETS; i++) {
        ...
        if ((map.get(k) || 0) < ABUSE_ALERT_THRESHOLD) {
          sustained = false;
          break;
        }
      }
      if (sustained) {
        ...
        if (last !== bucketKey) {
          ...
          try {
            METRICS.inc('abuse_spike_alert_total', {
              signal: signalType,
              source: String(engine_source || ''),
            });
          } catch {}
          try {
  ...
  function emitAndRecordSignals(signals, { engine_source = '' } = {}) {
    try {
      const {
        prompt_injection_signal: inj,
        jailbreak_signal: jb,
        grounding_strength: gr,
        levels,
      } = signals || {};
      try {
        METRICS.inc('prompt_injection_signal_total', {
          level: levels?.injection || 'low',
          source: String(engine_source || ''),
        });
      } catch {}
      try {
        METRICS.inc('jailbreak_signal_total', {
          level: levels?.jailbreak || 'low',
          source: String(engine_source || ''),
        });
      } catch {}
      try {
        METRICS.inc('grounding_strength_total', {
          level: levels?.grounding || 'low',
          source: String(engine_source || ''),
        });
      } catch {}
  ...
  const PRECALL_SHED_JITTER_BUCKETS = (
    __BUCKETS_ENV
      ? __BUCKETS_ENV
          .split(',')
          .map((s) => Number(s.trim()))
          .filter((n) => Number.isFinite(n) && n > 0)
      : [50, 100, 150, 200, 250, 300, 400, 600, 800, 1200, 2000, 3000, 5000]
  ).sort((a, b) => a - b);
  ...
  function shouldSoftDrop(engine_source) {
    try {
      ...
      try {
        METRICS.inc('precall_soft_drop_total', { reason, source: String(engine_source || '') });
      } catch {}
  ...
  function emitSoftDropJitterHistogram(jitterMs, { reason = '', source = '', path = '' } = {}) {
    try {
      ...
      for (const le of PRECALL_SHED_JITTER_BUCKETS) {
        if (jitterMs <= le) {
          METRICS.inc('precall_soft_drop_ms_bucket', {
            le: String(le),
    ...
    try {
      for (const [k, v] of Array.from(IDEMPOTENCY_CACHE.entries())) {
        if (now - Number(v?.ts || 0) > IDEMPOTENCY_TTL_MS + IDEMPOTENCY_SKEW_MS) {
          ...
          try {
            METRICS.inc('idempotency_cache_ttl_drop_total');
          } catch {}
        }
      }
    } catch {}
    try {
      for (const [k, v] of Array.from(ACTIVE_STREAMS.entries())) {
        if (now - Number(v?.started || 0) > IDEMPOTENCY_TTL_MS + IDEMPOTENCY_SKEW_MS) {
          ...
          try {
            METRICS.inc('active_streams_ttl_drop_total');
          } catch {}
        }
      }
    } catch {}
  ...
  async function saveIdemToDisk(key, response, replayCount = 0) {
    ...
    try {
      ...
      try {
        await AsyncFS.writeFileAtomic(p, JSON.stringify(payload), 'utf8');
      } catch {}
    }
  }
  ...
  async function purgeIdemEntries({ tenant, olderThanMs = 0, maxDeletesPerRun = 500 } = {}) {
    try {
      ...
      for (const n of names) {
        ...
        if (timeOk && tenantOk) {
          try {
            ...
            deleted++;
            try {
              METRICS.inc('tenant_idem_purge_deleted_total', { tenant: tenant || '' });
            } catch {}
          } catch {}
      ...
      try {
        METRICS.inc('tenant_idem_purge_runs_total', { tenant: tenant || '' });
      } catch {}
      return deleted;
    } catch {
      return 0;
  ...
  const IDEMPOTENCY_REDIS_URL = String(
    process.env.IDEMPOTENCY_REDIS_URL || process.env.REDIS_URL || ''
  ).trim();
  const IDEMPOTENCY_LOCK_TTL_MS = Math.max(
    1000,
    Number(process.env.IDEMPOTENCY_LOCK_TTL_MS || 60000)
  );
  ...
  async function hasToolExecuted(id) {
    try {
      return !!(await AsyncFS.exists(toolPath(id)));
    } catch {
      return false;
    }
  }
  ...
  async function loadAbVariant(cid) {
    try {
      ...
      if (v === 'A' || v === 'B') {
        try {
          METRICS.inc('ab_variant_load_total');
        } catch {}
        return v;
      }
      return '';
  ...
  async function saveAbVariant(cid, variant) {
    try {
      ...
      try {
        METRICS.inc('ab_variant_save_total');
      } catch {}
    } catch {}
  }
  async function purgeToolMarkers({ tenant, olderThanMs = 0, maxDeletesPerRun = 500 } = {}) {
    try {
      ...
      for (const name of names) {
        ...
        let st = null;
        try {
          st = await AsyncFS.stat(p);
        } catch {
          continue;
        }
        try {
          content = await AsyncFS.readFile(p, 'utf8');
        } catch {
          content = '';
        }
        let j = null;
        ...
        if (timeOk && tenantOk) {
          try {
            ...
            deleted++;
            try {
              METRICS.inc('tenant_tool_purge_deleted_total', { tenant: tenant || '' });
            } catch {}
          } catch {}
          if (deleted >= Math.max(1, Number(maxDeletesPerRun || 500))) break;
        }
      ...
      try {
        METRICS.inc('tenant_tool_purge_runs_total', { tenant: tenant || '' });
      } catch {}
      return deleted;
    } catch {
      return 0;
  ...
  async function markToolExecuted(id, meta = {}) {
    ...
    try {
      ...
      const child = await import('node:child_process').then((m) =>
        m.spawn(process.execPath, args, { stdio: ['ignore', 'pipe', 'pipe'], env })
      );
      let out = '';
      let err = '';
      ...
      const shouldRetry =
        String(resultPayload?.retryable || '') === 'true' || String(res.code || '') === 'TIMEOUT';
      if (shouldRetry) {
        const child2 = await import('node:child_process').then((m) =>
          m.spawn(process.execPath, args, { stdio: ['ignore', 'pipe', 'pipe'], env })
        );
        out = '';
        err = '';
      ...
      try {
        METRICS.inc('tool_mark_failed_total', { reason: String(res.code || 'unknown') });
      } catch {}
      return false;
    } catch {
      try {
        METRICS.inc('tool_mark_failed_total', { reason: 'exception' });
      } catch {}
      return false;
    }
  }
  ...
  const enforceJson = (req, res, span) => {
    ...
    if (!ct.startsWith('application/json')) {
      ...
      try {
        METRICS.inc('responses_total', { status: '415' });
        span?.setAttribute?.('http.status_code', 415);
      } catch {}
      return false;
    }
    return true;
  ...
  const validateCompileBody = (parsed) => {
    ...
    if (!parsed || typeof parsed !== 'object')
      errors.push({ path: '', message: 'Body must be a JSON object' });
    ...
    if (list) {
      list.forEach((m, i) => {
        ...
        if (typeof role !== 'string' || role.length === 0)
          errors.push({ path: `${p}.role`, message: 'role is required string' });
        ...
        if (hasText && hasContent)
          errors.push({ path: p, message: 'text and content are mutually exclusive' });
        if (!hasText && !hasContent)
          errors.push({
            path: `${p}.content`,
            message: 'content must be an array of strings or provide text',
          });
        if (hasContent && !m.content.every((s) => typeof s === 'string'))
          errors.push({ path: `${p}.content`, message: 'content items must be strings' });
      });
    }
  ...
  const validateMessageBody = (body) => {
    ...
    if (hasText && hasContent)
      errors.push({ path: '', message: 'text and content are mutually exclusive' });
    if (!hasText && !hasContent)
      errors.push({
        path: 'content',
        message: 'content must be an array of strings or provide text',
      });
    if (hasContent && !body.content.every((s) => typeof s === 'string'))
      errors.push({ path: 'content', message: 'content items must be strings' });
    if (typeof body?.conv_id !== 'undefined' && typeof body.conv_id !== 'string')
      errors.push({ path: 'conv_id', message: 'conv_id must be a string when provided' });
    if (typeof body?.turn !== 'undefined' && !Number.isFinite(Number(body.turn)))
      errors.push({ path: 'turn', message: 'turn must be a number when provided' });
    return { ok: errors.length === 0, errors };
  };
  ...
  try {
    const enforceJsonLogs =
      String(process.env.LOG_JSON || '0') === '1' &&
              ...
              if (typeof a === 'string') {
                ...
                if (looksJson) {
                  try {
                    ...
                    allJson = false;
                    try {
                      METRICS.inc('non_json_log_total', { stream });
                    } catch {}
                  }
                } else {
                  allJson = false;
  ...
  const POLICY_INTERNAL_ERROR_ONCE = String(
    process.env.POLICY_INTERNAL_ERROR_ONCE || '1'
  ).toLowerCase();
  const POLICY_IEO = POLICY_INTERNAL_ERROR_ONCE === '1' || POLICY_INTERNAL_ERROR_ONCE === 'true';
  const GLOBAL_RL =
    Number.isFinite(POLICY_LIMIT) && POLICY_LIMIT > 0
      ? createGlobalRateLimiter({
          limit: POLICY_LIMIT,
          windowMs: POLICY_WINDOW_MS,
          internalErrorOnce: POLICY_IEO,
        })
      : null;
  try {
    ...
    try {
      logAt('debug', `[service] queue depth provider set: ${Number(getQueueDepth())}`);
    } catch {}
  } catch {}
  ...
  const FIRST_TOKEN_MS_BUCKETS = (
    __FIRST_TOKEN_BUCKETS_ENV
      ? __FIRST_TOKEN_BUCKETS_ENV
          .split(',')
          .map((s) => Number(s.trim()))
          .filter((n) => Number.isFinite(n) && n > 0)
      : [50, 100, 200, 300, 500, 700, 1000, 1500, 2000, 3000]
  ).sort((a, b) => a - b);
  async function __prewarmCaches() {
    try {
      // Tokenizer prewarm using common small prompts and model hints
      const hints = String(process.env.PREWARM_MODELS || '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
      ...
      const extras = [];
      try {
        if (String(process.env.URGA_PROVIDER || '').trim()) extras.push('urga');
      } catch {}
      const prewarmModels = Array.from(new Set([...baseModels, ...extras]));
      for (const m of prewarmModels) {
        try {
          TokenCounter.estimate('warmup', { model: m });
        } catch {}
      }
      ...
      try {
        configureProvidersFromEnv(ctxWarm);
      } catch {}
      try {
        if (String(process.env.PROVIDERS_DEBUG || '0') === '1') {
          logAt(
            'debug',
            JSON.stringify({
              evt: 'providers_configured',
              source: String(ctxWarm?.vars?.engine_source || 'prewarm'),
            })
          );
        }
      } catch {}
      try {
        ...
        for (const m of prewarmModels) {
          try {
            await llmWarm.call('warmup', { model: m, critical: false });
          } catch {}
        }
      } catch {}
    } catch {}
  ...
  (async () => {
    try {
      await __prewarmCaches();
    } catch {}
    const ms = Math.max(0, Date.now() - __startupT0);
    try {
      globalThis.__STARTUP_READY_MS__ = ms;
      METRICS.set('startup_ready_ms', ms);
    } catch {}
  })();
  const healthInfo = () => {
    const base = {
    ...
    const minimal =
      String(process.env.NODE_ENV || '').toLowerCase() === 'production' &&
      String(process.env.HEALTHZ_MINIMAL || '0') === '1';
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      if (otel.tracer)
        span = otel.tracer.startSpan('http.request', {
    ...
    const __rng__ =
      typeof globalThis.__prng__ === 'function'
        ? globalThis.__prng__
        : typeof globalThis.__RNG__ === 'function'
          ? globalThis.__RNG__
          : makePRNG();
    ...
    let __path = '';
    try {
      __path = new URL(`http://localhost${req.url}`).pathname;
    } catch {
      __path = String(req.url || '');
    }
    ...
    const isProbe =
      ...
      req.url?.startsWith('/heap/snapshot');
    ...
    try {
      ...
      try {
        console.log(
          JSON.stringify({ evt: 'header_size_check', headerBytes, MAX_HEADER_BYTES, isProbe })
        );
      } catch {}
      if (headerBytes > MAX_HEADER_BYTES && !isProbe) {
        ...
        try {
          METRICS.inc('request_header_large_total');
          METRICS.inc('responses_total', { status: '431' });
        } catch {}
        return;
      }
      ...
      if (!isProbe && (method === 'POST' || method === 'PUT' || method === 'PATCH')) {
        if (Number.isFinite(cl) && cl > MAX_BODY_BYTES) {
          ...
          try {
            METRICS.inc('request_body_large_total');
            METRICS.inc('responses_total', { status: '413' });
          } catch {}
          return;
        }
      }
    ...
    try {
      ...
      if (!isProbe && !isControl && Number.isFinite(CLIENT_LIMIT) && CLIENT_LIMIT > 0) {
        if (!globalThis.__CLIENT_RL__) {
          globalThis.__CLIENT_RL__ =
            backendName === 'file'
              ? createSharedRateLimiter({
                  limit: CLIENT_LIMIT,
                  windowMs: CLIENT_WINDOW_MS,
                  internalErrorOnce: internalOnce,
                })
              : createGlobalRateLimiter({
                  limit: CLIENT_LIMIT,
                  windowMs: CLIENT_WINDOW_MS,
        ...
        const forwarded = String(req.headers['x-forwarded-for'] || '')
          ...
          .trim();
        const principal =
          apiKey || bearer || forwarded || String(req.socket?.remoteAddress || 'unknown');
        const out = await Promise.resolve(globalThis.__CLIENT_RL__.allow(principal));
        try {
          sampled('debug', {
            gate: 'client_rl_check',
            principal: String(principal || '').slice(0, 64),
            window_ms: CLIENT_WINDOW_MS,
            ok: !!out?.ok,
            internal_error: !!out?.internal_error,
          });
        } catch {}
        if (!out?.ok) {
          ...
          if (out.internal_error && internalOnce) {
            ...
            try {
              METRICS.inc('rate_limited_total', { reason: 'internal_error', scope: 'client' });
              METRICS.inc('responses_total', { status: '503' });
            } catch {}
            try {
              sampled('debug', {
                gate: 'client_rl_deny',
          ...
          res.end(
            JSON.stringify({ error: 'rate_limited', reason: 'client', retry_after_s: retryAfter })
          );
          try {
            METRICS.inc('rate_limited_total', { reason: 'client', scope: 'client' });
            METRICS.inc('responses_total', { status: '429' });
          } catch {}
          try {
            sampled('debug', {
              gate: 'client_rl_deny',
      ...
      try {
        ...
        if (!out.ok) {
          if (out.internal_error && POLICY_IEO) {
            ...
            try {
              METRICS.inc('rate_limited_total', { reason: 'internal_error' });
            } catch {}
            try {
              METRICS.inc('responses_total', { status: '503' });
            } catch {}
          ...
          res.end(
            JSON.stringify({ error: 'rate_limited', reason: 'policy', retry_after_s: retryAfter })
          );
          try {
            METRICS.inc('rate_limited_total', { reason: 'policy' });
          } catch {}
          try {
            METRICS.inc('responses_total', { status: '429' });
          } catch {}
    ...
    if (!isProbe && !isControl && QUEUE_MAX > 0) {
      try {
        ...
        const effectiveDepth =
          Number.isFinite(provDepth) && provDepth >= 0 ? provDepth : Number(inflightReq);
        try {
          logAt(
            'info',
            JSON.stringify({
              evt: 'queue_check',
              inflight: Number(inflightReq),
              active: Number(activeResponses),
              provDepth: Number.isFinite(provDepth) ? provDepth : null,
              effectiveDepth,
              QUEUE_MAX,
            })
          );
        ...
        if (effectiveDepth >= QUEUE_MAX) {
          ...
          try {
            logAt('warn', JSON.stringify({ evt: 'rate_limited_total', reason: 'backpressure' }));
          } catch {}
          try {
            METRICS.inc('rate_limited_total', { reason: 'backpressure' });
          } catch {}
          try {
            METRICS.inc('responses_total', { status: '503' });
          } catch {}
          ...
          if (bpStartMs > 0) {
            ...
            if (durMs >= BP_SUSTAIN_MS) {
              try {
                ...
                logAt(
                  'info',
                  JSON.stringify({ evt: 'backpressure_sustained', duration_ms: durMs, le })
                );
                try {
                  sampled('info', { evt: 'backpressure_sustained', duration_ms: durMs, le });
                } catch {}
              } catch {}
            }
          }
        ...
        if (requireAuth) {
          ...
          try {
            ...
            tokenFromQuery = String(
              uTmp.searchParams.get('token') || uTmp.searchParams.get('auth') || ''
            ).trim();
          } catch {}
          ...
          if (!ok || !isIpAllowed('TENANTS_IP_ALLOWLIST')) {
            ...
            try {
              METRICS.inc('responses_total', { status: '403' });
            } catch {}
            return false;
          }
        }
    ...
    if (req.url?.startsWith('/drain/start')) {
      try {
        ...
        const ms = Math.max(
          0,
          Math.min(30_000, Number(u.searchParams.get('ms') || drainTimeoutMs))
        );
        try {
          globalThis?.READY?.notReady?.();
        } catch {}
          ...
          for (const entry of Array.from(PENDING_WAITS)) {
            ...
            try {
              ...
              if (every > 0 && t > 0 && t % every === 0) {
                try {
                  consolidateAll(cidCons);
                } catch {}
                try {
                  METRICS.inc?.('facts_consolidate_runs_total', 1, {});
                } catch {}
              }
            } catch {}
        ...
        setTimeout(() => {
          server.close(() => {
            try {
              logAt(
                'info',
                JSON.stringify({ evt: 'service_closed', success: inflightReq === 0, inflightReq })
              );
            } catch {}
          ...
          req.on('end', async () => {
            try {
              ...
              if (!conv_id) {
                ...
                try {
                  METRICS.inc('responses_total', { status: '400' });
    ...
    if (
      req.url?.startsWith('/tenants/budget') &&
      ...
      try {
        ...
        try {
          ...
          if (Number.isFinite(limTok) && limTok > 0) {
            if (!globalThis.__TENANT_TOKEN_BUDGET__) {
              globalThis.__TENANT_TOKEN_BUDGET__ = createSharedTenantBudget({
                windowMs: winTok,
                limitTokens: limTok,
              });
            }
            const snap = await Promise.resolve(globalThis.__TENANT_TOKEN_BUDGET__.peek(tenantKey));
            out.views.tokens_window = {
              window_ms: winTok,
              ok: !!snap?.ok,
              window_start: Number(snap?.windowStart || 0),
              spent: Number(snap?.spentTokens || 0),
              limit: Number(snap?.limitTokens || 0),
            };
          }
        } catch {}
        // Sliding-window USD
        try {
          ...
          if (Number.isFinite(limUsd) && limUsd > 0) {
            if (!globalThis.__TENANT_USD_BUDGET__) {
              globalThis.__TENANT_USD_BUDGET__ = createSharedTenantDollarBudget({
                windowMs: winUsd,
                limitUsd: limUsd,
              });
            }
            const snap = await Promise.resolve(globalThis.__TENANT_USD_BUDGET__.peek(tenantKey));
            out.views.usd_window = {
              window_ms: winUsd,
              ok: !!snap?.ok,
              window_start: Number(snap?.windowStart || 0),
              spent_usd: Number(snap?.spentTokens || 0),
              limit_usd: Number(snap?.limitTokens || 0),
            };
          }
        } catch {}
        // Monthly tokens
        try {
          ...
          if (Number.isFinite(limTokMon) && limTokMon > 0) {
            if (!globalThis.__TENANT_TOKEN_MONTHLY_BUDGET__) {
              globalThis.__TENANT_TOKEN_MONTHLY_BUDGET__ = createSharedTenantMonthlyBudget({
                limitTokens: limTokMon,
              });
            }
            const snap = await Promise.resolve(
              globalThis.__TENANT_TOKEN_MONTHLY_BUDGET__.peek(tenantKey)
            );
            out.views.tokens_monthly = {
              ok: !!snap?.ok,
              month_key: String(snap?.monthKey || ''),
              window_start: Number(snap?.windowStart || 0),
              spent: Number(snap?.spentTokens || 0),
              limit: Number(snap?.limitTokens || 0),
            };
        ...
        try {
          ...
          if (Number.isFinite(limUsdMon) && limUsdMon > 0) {
            if (!globalThis.__TENANT_USD_MONTHLY_BUDGET__) {
              globalThis.__TENANT_USD_MONTHLY_BUDGET__ = createSharedTenantMonthlyDollarBudget({
                limitUsd: limUsdMon,
              });
            }
            const snap = await Promise.resolve(
              globalThis.__TENANT_USD_MONTHLY_BUDGET__.peek(tenantKey)
            );
            out.views.usd_monthly = {
              ok: !!snap?.ok,
              month_key: String(snap?.monthKey || ''),
              window_start: Number(snap?.windowStart || 0),
              spent_usd: Number(snap?.spentTokens || 0),
        ...
        try {
          ...
          if (
            Number.isFinite(limTokRoll) &&
            limTokRoll > 0 &&
            Number.isFinite(winTokRoll) &&
            winTokRoll > 0
          ) {
            if (!globalThis.__TENANT_TOKEN_ROLLING_BUDGET__) {
              globalThis.__TENANT_TOKEN_ROLLING_BUDGET__ = createSharedTenantRollingBudget({
                windowMs: winTokRoll,
                bucketMs,
                limitTokens: limTokRoll,
              });
            }
            const snap = await Promise.resolve(
              globalThis.__TENANT_TOKEN_ROLLING_BUDGET__.peek(tenantKey)
            );
            out.views.tokens_rolling = {
              window_ms: winTokRoll,
              bucket_ms: Number(snap?.bucketMs || bucketMs),
              ok: !!snap?.ok,
              window_start: Number(snap?.windowStart || 0),
              spent: Number(snap?.spentTokens || 0),
              limit: Number(snap?.limitTokens || 0),
        ...
        try {
          ...
          const bucketMs = Number(process.env.TENANT_ROLLING_BUCKET_MS || 60000);
          if (
            Number.isFinite(limUsdRoll) &&
            limUsdRoll > 0 &&
            Number.isFinite(winUsdRoll) &&
            winUsdRoll > 0
          ) {
            if (!globalThis.__TENANT_USD_ROLLING_BUDGET__) {
              globalThis.__TENANT_USD_ROLLING_BUDGET__ = createSharedTenantRollingDollarBudget({
                windowMs: winUsdRoll,
                bucketMs,
                limitUsd: limUsdRoll,
              });
            ...
            const snap = await Promise.resolve(
              globalThis.__TENANT_USD_ROLLING_BUDGET__.peek(tenantKey)
            );
            out.views.usd_rolling = {
              window_ms: winUsdRoll,
              bucket_ms: Number(snap?.bucketMs || bucketMs),
              ok: !!snap?.ok,
              window_start: Number(snap?.windowStart || 0),
              spent_usd: Number(snap?.spentTokens || 0),
    ...
    if (
      ...
      String(req.method || 'DELETE').toUpperCase() === 'DELETE'
    ) {
      try {
        const requireAuth = String(process.env.TENANTS_AUTH || '').length > 0;
        if (requireAuth) {
          ...
          try {
            ...
            tokenFromQuery = String(
              u2.searchParams.get('token') || u2.searchParams.get('auth') || ''
            ).trim();
          } catch {}
          const ok = tokenFromHdr === token || tokenFromQuery === token;
          if (!ok || !isIpAllowed('TENANTS_IP_ALLOWLIST')) {
        ...
        const rawTenant =
          pathTenant || u.searchParams.get('tenant') || req.headers['x-tenant'] || '';
        const safeTenant = String(rawTenant || '').replace(/[^a-zA-Z0-9_\.\-]/g, '');
        const tenantKey = safeTenant || 'tenant';
        ...
        try {
          ...
          if (Number.isFinite(TENANT_LIMIT) && TENANT_LIMIT > 0) {
            if (!globalThis.__TENANT_RL__) {
              globalThis.__TENANT_RL__ =
                backendName === 'file'
                  ? createSharedRateLimiter({
                      limit: TENANT_LIMIT,
                      windowMs: TENANT_WINDOW_MS,
                      internalErrorOnce: tenantInternalOnce,
                    })
                  : createGlobalRateLimiter({
                      limit: TENANT_LIMIT,
                      windowMs: TENANT_WINDOW_MS,
            ...
            const r = await Promise.resolve(globalThis.__TENANT_RL__.wipe?.(tenantKey)).catch(
              () => null
            );
        ...
        try {
          ...
          if (Number.isFinite(limTok) && limTok > 0) {
            ...
            const r = await Promise.resolve(
              globalThis.__TENANT_TOKEN_BUDGET__.wipe?.(tenantKey)
            ).catch(() => null);
            wiped.tokens_window = !!(r && r.ok);
          } else {
            wiped.tokens_window = false;
          }
        } catch {}
        ...
        try {
          ...
          if (Number.isFinite(limUsd) && limUsd > 0) {
            ...
            const r = await Promise.resolve(
              globalThis.__TENANT_USD_BUDGET__.wipe?.(tenantKey)
            ).catch(() => null);
            wiped.usd_window = !!(r && r.ok);
          } else {
            wiped.usd_window = false;
          }
        } catch {}
        ...
        try {
          ...
          if (Number.isFinite(limTokMon) && limTokMon > 0) {
            ...
            const r = await Promise.resolve(
              globalThis.__TENANT_TOKEN_MONTHLY_BUDGET__.wipe?.(tenantKey)
            ).catch(() => null);
            wiped.tokens_monthly = !!(r && r.ok);
          } else {
            wiped.tokens_monthly = false;
          }
        } catch {}
        ...
        try {
          ...
          if (Number.isFinite(limUsdMon) && limUsdMon > 0) {
            ...
            const r = await Promise.resolve(
              globalThis.__TENANT_USD_MONTHLY_BUDGET__.wipe?.(tenantKey)
            ).catch(() => null);
            wiped.usd_monthly = !!(r && r.ok);
          } else {
            wiped.usd_monthly = false;
          }
        } catch {}
        ...
        try {
          ...
          if (
            ...
            const r = await Promise.resolve(
              globalThis.__TENANT_TOKEN_ROLLING_BUDGET__.wipe?.(tenantKey)
            ).catch(() => null);
            wiped.tokens_rolling = !!(r && r.ok);
          } else {
            wiped.tokens_rolling = false;
          }
        } catch {}
        ...
        try {
          ...
          if (
            ...
            const r = await Promise.resolve(
              globalThis.__TENANT_USD_ROLLING_BUDGET__.wipe?.(tenantKey)
            ).catch(() => null);
            wiped.usd_rolling = !!(r && r.ok);
          } else {
            wiped.usd_rolling = false;
          }
        } catch {}
        ...
        try {
          const olderThanMs = Number(
            u.searchParams.get('older_than_ms') || process.env.TOOL_MARK_TTL_MS || 0
          );
          const maxToolDeletes = Number(
            u.searchParams.get('max_tool_deletes') || process.env.TOOL_PURGE_MAX_DELETES || 500
          );
          const delCount = await purgeToolMarkers({
            tenant: tenantKey,
            olderThanMs,
            maxDeletesPerRun: maxToolDeletes,
          });
          wiped.tool_markers_deleted = delCount;
        } catch {
          wiped.tool_markers_deleted = 0;
        }
        // Optional idempotency entry purge with TTL filter
        try {
          const olderThanMsIdem = Number(
            u.searchParams.get('idem_older_than_ms') || process.env.IDEMPOTENCY_TTL_MS || 0
          );
          const maxIdemDeletes = Number(
            u.searchParams.get('max_idem_deletes') || process.env.IDEM_PURGE_MAX_DELETES || 500
          );
          const idemDel = await purgeIdemEntries({
            tenant: tenantKey,
            olderThanMs: olderThanMsIdem,
            maxDeletesPerRun: maxIdemDeletes,
          });
          wiped.idem_deleted = idemDel;
        } catch {
          wiped.idem_deleted = 0;
        }
        try {
          METRICS.inc('tenants_wipe_total', { tenant: tenantKey });
        } catch {}
        res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        res.end(
          JSON.stringify({ error: 'tenant_wipe_failed', detail: String((e && e.message) || e) })
        );
        try {
          METRICS.inc('responses_total', { status: '500' });
    ...
    if (req.url === '/idem' && String(req.method || 'DELETE').toUpperCase() === 'DELETE') {
      try {
        ...
        const u = new URL(`http://localhost${req.url}`);
        const olderThanMsIdem = Number(
          u.searchParams.get('older_than_ms') || process.env.IDEMPOTENCY_TTL_MS || 0
        );
        const maxIdemDeletes = Number(
          u.searchParams.get('max_idem_deletes') || process.env.IDEM_PURGE_MAX_DELETES || 500
        );
        const deleted = await purgeIdemEntries({
          tenant: '',
          olderThanMs: olderThanMsIdem,
          maxDeletesPerRun: maxIdemDeletes,
        });
        try {
          METRICS.inc('tenant_idem_purge_runs_total', { tenant: '' });
        } catch {}
        res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(
          JSON.stringify({ error: 'idem_purge_failed', detail: String((e && e.message) || e) })
        );
        try {
          METRICS.inc('responses_total', { status: '500' });
          span?.setAttribute?.('http.status_code', 500);
    ...
    if (req.url === '/openapi.json' && String(req.method || 'GET').toUpperCase() === 'GET') {
      try {
        ...
        try {
          METRICS.inc('responses_total', { status: '404' });
    ...
    if (
      ...
      try {
        // Auth for compile endpoint: accept token and HMAC similar to /conv/stream
        const uAuth = new URL(`http://localhost${req.url}`);
        ...
        const tokenFromQuery = String(
          uAuth.searchParams.get('token') || uAuth.searchParams.get('auth') || ''
        ).trim();
        const hasToken = token.length > 0 && (tokenFromHdr === token || tokenFromQuery === token);
        const hmacSecrets = String(
          process.env.CONV_HMAC_SECRETS || process.env.CONV_HMAC_SECRET || ''
        )
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean);
        const reqTs = Number(
          uAuth.searchParams.get('ts') || Number(req.headers['x-request-ts'] || 0)
        );
        const macHdr = String(req.headers['x-client-mac'] || '').trim();
        const macId = String(req.headers['x-mac-id'] || '').trim();
        const pathTag = 'compile';
        ...
        if (isProd && replayWinMs > 0) {
          if (!Number.isFinite(reqTs) || reqTs <= 0) {
            ...
            try {
              ...
              METRICS.inc('auth_blocked_total', { reason: 'ts_missing', path: pathTag });
          ...
          if (skew - skewToleranceMs > replayWinMs) {
            ...
            try {
              ...
              const method = attemptedToken ? 'token' : macHdr.length > 0 ? 'hmac' : 'none';
              METRICS.inc('responses_total', { status: '401' });
              METRICS.inc('auth_blocked_total', { reason: 'replay_window', path: pathTag });
              METRICS.inc('auth_failed_total', {
        ...
        if (hmacSecrets.length > 0 && Number.isFinite(reqTs) && reqTs > 0) {
          ...
          for (const sec of hmacSecrets) {
            try {
              ...
              if (macHdr.length > 0 && macHdr === expMac) {
                macOk = true;
                break;
              }
            } catch {}
        ...
        const mustAuth = String(process.env.CONV_AUTH || '').length > 0 || isProd;
        if ((mustAuth && !(hasToken || macOk)) || !isIpAllowed('CONV_IP_ALLOWLIST')) {
          res.writeHead(401, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'auth_required' }));
          try {
            METRICS.inc('responses_total', { status: '401' });
            METRICS.inc('auth_blocked_total', { reason: 'missing_or_invalid', path: pathTag });
            const attemptedToken = hdr.length > 0 || tokenFromQuery.length > 0;
            const reason = attemptedToken
              ? 'token_invalid'
              : macHdr.length > 0
                ? 'hmac_invalid'
                : 'auth_missing';
            ...
            METRICS.inc('auth_failed_total', { reason, path: pathTag, method, ...labels });
            span?.setAttribute?.('http.status_code', 401);
          } catch {}
          return;
        }
        try {
          if (hasToken) METRICS.inc('auth_accepted_total', { method: 'token', path: pathTag });
          else if (macOk)
            METRICS.inc('auth_accepted_total', { method: 'hmac', path: pathTag, mac_id: macId });
        } catch {}
        if (!enforceJson(req, res, span)) return;
        const chunks = [];
        req.on('data', (c) => {
        ...
        req.on('end', () => {
          try {
              ...
              createMessage({
                ...
                id:
                  typeof m?.id === 'string' && m.id.length > 0
                    ? m.id
                    : `msg_${String(m?.role || 'unknown')}_${i}`,
            ...
            const { bytes, hash } = assembleForModel(msgs, {
              persona_v: parsed?.persona_v,
              prompt_v: parsed?.prompt_v,
            });
            res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (String(req.method || 'GET').toUpperCase() === 'GET') {
      ...
      if (__path === '/state') {
        try {
          ...
          try {
            ...
            conv = String(
              uQ.searchParams.get('conv_id') || uQ.searchParams.get('conv') || ''
            ).trim();
          } catch {}
          if (!conv) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'state_list_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          if (requireAuth) {
            ...
            if (!ok || !isIpAllowed('TENANTS_IP_ALLOWLIST')) {
              ...
              try {
                METRICS.inc('responses_total', { status: '403' });
                METRICS.inc('auth_blocked_total', {
                  reason: ok ? 'ip_forbidden' : 'missing_or_invalid',
                  path: 'admin:beliefs:get',
                });
                const attemptedToken = hdr.length > 0 || tokenFromQuery.length > 0;
                const reason = attemptedToken ? 'token_invalid' : 'auth_missing';
                METRICS.inc('auth_failed_total', {
                  reason,
                  path: 'admin:beliefs:get',
                  method: 'token',
            ...
            try {
              METRICS.inc('auth_accepted_total', { method: 'token', path: 'admin:beliefs:get' });
            } catch {}
          }
          let convId = '';
          try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'beliefs_get_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path.startsWith('/admin/beliefs/') && __path !== '/admin/beliefs/line') {
        try {
          ...
          res.end(
            JSON.stringify({
              error: 'agent_beliefs_get_failed',
      ...
      if (__path === '/admin/constraints') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'constraints_get_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path.startsWith('/admin/refusal-style/')) {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'refusal_style_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/style') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'style_get_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          if (requireAuth) {
            ...
            if (!ok || !isIpAllowed('TENANTS_IP_ALLOWLIST')) {
              ...
              try {
                METRICS.inc('auth_blocked_total', { path: 'admin_spine' });
              } catch {}
              return;
            }
            try {
              METRICS.inc('auth_ok_total', { path: 'admin_spine' });
            } catch {}
          }
          let convId = '';
          try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'spine_get_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          try {
            ...
            if (trust !== null && trust !== undefined)
              spine.trust = Math.max(0, Math.min(1, Number(trust)));
            if (suspicion !== null && suspicion !== undefined)
              spine.suspicion = Math.max(0, Math.min(1, Number(suspicion)));
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'spine_demo_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/sse/spine') {
        try {
          ...
          req.on('close', () => {
            try {
              unregisterAdminSpineSSE(convId, res);
            } catch {}
          });
        } catch (e) {
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'spine_sse_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                METRICS.inc('roll_pref_changed_total', { path: 'admin' });
              } catch {}
              try {
                broadcastAdminMemoryEvent(convId, 'roll.pref', { conv_id: convId, roll_meta });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
      ...
      if (__path === '/admin/roll/style') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'roll_style_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/failroll/preview') {
        try {
          ...
          const nearMiss =
            !fail &&
            Number(process.env.COMPLICATION_ENABLED ?? '1') &&
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                METRICS.inc('refusal_pref_changed_total', { path: 'admin' });
              } catch {}
              try {
                broadcastAdminRefusalEvent(convId, 'refusal.pref', {
                  conv_id: convId,
                  refusal_meta,
                });
              } catch {}
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'refusal_set_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/style/presets') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'style_presets_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
    ...
    if (
      String(req.method || 'GET').toUpperCase() === 'POST' &&
      String(req.url || '').startsWith('/message')
    ) {
      try {
        const stubs = String(process.env.LLM_TEST_STUBS || '').trim() === '1';
        if (!stubs) {
        ...
        req.on('end', () => {
          try {
            ...
            const parsed = JSON.parse(raw || '{}');
            const tenant = String(
              parsed?.ctx?.vars?.tenant || req.headers['x-tenant'] || ''
            ).replace(/[^a-zA-Z0-9_\.\-]/g, '');
            const tokensIn = 1;
            const tokensOut = 1;
            const usd = 0.001;
            try {
              globalThis.UrgaCoreDeps?.Metrics?.log(
                { vars: { tenant, path: 'message' } },
                'llm_cost',
                {
                  tenant,
                  provider: String(process.env.URGA_PROVIDER || 'stub-urga'),
                  model: 'urga',
                  tokens_in: tokensIn,
            ...
            try {
              const ledLen = Number(
                (globalThis.__USAGE_LEDGER__ &&
                  globalThis.__USAGE_LEDGER__.buffer &&
                  globalThis.__USAGE_LEDGER__.buffer.length) ||
                  0
              );
              appendTestOutput(
                JSON.stringify({ evt: 'test_alias_called', tenant, led_len: ledLen })
              );
            } catch {}
            res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (String(req.method || 'GET').toUpperCase() === 'GET') {
      ...
      if (__path === '/__test/facts') {
        try {
          ...
          try {
            const uQ = new URL(`http://localhost${req.url}`);
            conv = String(uQ.searchParams.get('conv_id') || '').trim();
          } catch {}
          if (!conv) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: 'conv_id_required' }));
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'test_facts_list_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/sse/memory') {
        try {
          ...
          req.on('close', async () => {
            ...
            try {
              await sendMessageWithTick(async () => {
                res.end();
                return true;
              });
            } catch {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'admin_sse_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          if (requireAuth) {
            ...
            if (!ok || !isIpAllowed('TENANTS_IP_ALLOWLIST')) {
              ...
              try {
                METRICS.inc('auth_blocked_total', { path: 'admin_sse_style' });
              } catch {}
              return;
            }
            try {
              METRICS.inc('auth_ok_total', { path: 'admin_sse_style' });
            } catch {}
          }
          let convId = '';
          try {
        ...
        try {
          ...
          if (requireAuth) {
            ...
            if (!ok || !isIpAllowed('TENANTS_IP_ALLOWLIST')) {
              ...
              try {
                METRICS.inc('auth_blocked_total', { path: 'admin_sse_refusal' });
              } catch {}
              return;
            }
            try {
              METRICS.inc('auth_ok_total', { path: 'admin_sse_refusal' });
            } catch {}
          }
          let convId = '';
          try {
    ...
    if (String(req.method || 'GET').toUpperCase() === 'POST') {
        ...
        try {
          ...
          if (requireAuth) {
            ...
            if (!ok || !isIpAllowed('TENANTS_IP_ALLOWLIST')) {
              ...
              try {
                METRICS.inc('auth_blocked_total', { path: 'state' });
              } catch {}
              return;
            }
            try {
              METRICS.inc('auth_ok_total', { path: 'state' });
            } catch {}
          }
          if (!enforceJson(req, res, span)) return;
          const chunks = [];
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'state_set_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                METRICS.inc('refusal_style_set_total', { agent: agentId, style: applied });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
      ...
      if (__path.startsWith('/admin/beliefs/') && __path !== '/admin/beliefs/line') {
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              if (personality && typeof personality === 'object') {
                ...
                try {
                  emitAdminMemoryEvent('beliefs.update', { agentId, personality: saved });
                } catch {}
                res.writeHead(200, { 'Content-Type': 'application/json' });
              ...
              try {
                emitAdminMemoryEvent('beliefs.update', { agentId, belief });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
          ...
          res.end(
            JSON.stringify({
              error: 'agent_beliefs_upsert_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                emitAdminMemoryEvent('constraints.add', { constraint });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'constraint_add_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                broadcastAdminSpineEvent(convId, 'spine.pref', { conv_id: convId, spine_meta });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ ok: true, conv_id: convId, spine, spine_meta }));
              try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'spine_upsert_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          req.on('end', () => {
            try {
              ...
              const arc_tags = Array.isArray(body?.arc_tags)
                ? body.arc_tags
                : body?.arc_tags
                  ? [String(body.arc_tags)]
                  : [];
              ...
              res.end(
                JSON.stringify({
                  ...
                  fact: {
                    ...
                    arc_tags,
                    repeats: Number.isFinite(repeats) ? repeats : undefined,
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'test_fact_upsert_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          req.on('end', () => {
            try {
              ...
              res.writeHead(500, { 'Content-Type': 'application/json' });
              res.end(
                JSON.stringify({ error: 'test_clear_failed', msg: String((e && e.message) || e) })
              );
              try {
                METRICS.inc('responses_total', { status: '500' });
              } catch {}
    ...
    if (String(req.method || 'GET').toUpperCase() === 'POST') {
      ...
      if (__path === '/memory/facts') {
        try {
            ...
            try {
              ...
              const arc_tags = Array.isArray(body?.arc_tags)
                ...
                : body?.arc_tags
                  ? [String(body.arc_tags)]
                  : [];
              const { id, merged, dropped, total } = addFactWithStats(convId, text, {
                weight,
                agent_id: agent_id || undefined,
                arc_tags,
              });
              ...
              try {
                if (merged) METRICS.inc('facts_merged_total');
              } catch {}
              try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_facts_add_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/memory/beliefs') {
        try {
          ...
          res.end(
            JSON.stringify({
              error: 'memory_beliefs_add_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
      ...
      if (__path === '/admin/beliefs') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'beliefs_upsert_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/beliefs/line') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'beliefs_add_line_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                METRICS.inc('ultra_mode_toggled_total', {
                  path: 'admin',
                  enabled: String(enabled),
                });
              } catch {}
              try {
                broadcastAdminMemoryEvent(convId, 'loopguard.ultra.changed', {
                  conv_id: convId,
                  enabled,
                  ts,
                });
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'ultra_toggle_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              const ts = Date.now();
              try {
                METRICS.inc('failroll_enabled_toggled_total', { enabled: String(enabled) });
              } catch {}
              try {
                broadcastAdminMemoryEvent('global', 'failroll.enabled.changed', { enabled, ts });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
          ...
          res.end(
            JSON.stringify({
              error: 'failroll_enabled_toggle_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              const overrides =
                typeof body?.overrides === 'object' && body?.overrides ? body.overrides : undefined;
              const pref = setStylePref(convId, { preset: preset || undefined, overrides });
              const style_meta = computeStyleMeta(convId);
              try {
                METRICS.inc('style_pref_changed_total', { path: 'admin' });
              } catch {}
              try {
                broadcastAdminStyleEvent(convId, 'style.pref', { conv_id: convId, style_meta });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'style_set_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/refusal') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'refusal_get_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/scene/conclusion') {
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                METRICS.inc('scene_conclusion_staged_total', {
                  path: 'admin',
                  style: String(forced?.style || ''),
                });
              } catch {}
              try {
                broadcastAdminMemoryEvent(convId, 'scene.conclusion.staged', {
                  conv_id: convId,
                  style: forced?.style,
                  reason: forced?.reason,
                  score: forced?.score,
                  booster: forced?.booster,
                  coolUntilTurn: forced?.coolUntilTurn,
                });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
          ...
          res.end(
            JSON.stringify({
              error: 'scene_conclusion_force_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
      ...
      if (__path === '/admin/scene/conclusion') {
        try {
          ...
          res.end(
            JSON.stringify({
              error: 'scene_conclusion_snapshot_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
            ...
            req.on('end', async () => {
              try {
                ...
                if (!conv_id || !allowed.includes(state)) {
                  ...
                  try {
                    METRICS.inc('responses_total', { status: '400' });
                    METRICS.inc('beat_errors_total', { path: 'admin' });
                  } catch {}
                  return;
                }
                ...
                try {
                  METRICS.inc('scene_beat_state_total', { state, path: 'admin' });
                } catch {}
                if (String(prev?.state || '') !== state) {
                  try {
                    METRICS.inc('scene_beat_switch_total', {
                ...
                res.end(
                  JSON.stringify({
                    ...
                    conv_id,
                    state,
                    previous: String(prev?.state || 'lull'),
                    tension: Number(forced?.tension || 0),
                  })
                );
                try {
                  METRICS.inc('responses_total', { status: '200' });
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              const fact_ids = Array.isArray(body?.fact_ids)
                ? body.fact_ids.map((id) => String(id))
                : [];
              if (fact_ids.length) linkFactsToScene(conv_id, key, fact_ids);
              const booster = String(body?.booster || '').trim();
              if (booster) {
                try {
                  slot.boosters = [booster];
                } catch {}
              }
              try {
                METRICS.inc('scene_tag_total', { path: 'post' });
              } catch {}
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(
                JSON.stringify({
                  ok: true,
                  key,
                  linkedFacts: Array.from(slot?.facts || []),
                  boosters: Array.isArray(slot?.boosters) ? slot.boosters : [],
                })
              );
              try {
                METRICS.inc('responses_total', { status: '200' });
        ...
        try {
          ...
          req.on('end', async () => {
            try {
              ...
              try {
                METRICS.inc(inferred ? 'memory_arc_inferred_total' : 'memory_arc_set_total', {
                  path: 'post',
                });
              } catch {}
        ...
        try {
          ...
          if (shouldParse) {
            ...
            req.on('end', () => {
              ...
              try {
                const uQ = new URL(`http://localhost${req.url}`);
                convId = String(body?.conv_id || uQ.searchParams.get('conv_id') || '').trim();
              } catch {
                convId = String(body?.conv_id || '').trim();
              }
              if (!convId) convId = 'conv';
              const { total, dropped } = consolidateAllWithStats(convId);
              try {
                METRICS.inc('facts_merged_total');
              } catch {}
              try {
                const n = Number(dropped || 0);
                for (let i = 0; i < n; i++) METRICS.inc('facts_dropped_total', { reason: 'bound' });
          ...
          res.end(
            JSON.stringify({
              error: 'memory_facts_consolidate_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
    ...
    if (String(req.method || 'GET').toUpperCase() === 'PUT') {
      ...
      if (__path && __path.startsWith('/memory/facts/')) {
        try {
          ...
          res.end(
            JSON.stringify({
              error: 'memory_facts_update_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
    ...
    if (
      (req.url?.startsWith('/conv/message') || req.url?.startsWith('/v1/conv/message')) &&
      String(req.method || 'GET').toUpperCase() === 'POST'
    ) {
      try {
        ...
        try {
          ...
          const corsList = String(process.env.CORS_ALLOWLIST || '')
            .split(',')
            .map((s) => s.trim())
            .filter(Boolean);
          const isProd = isProdEnv;
          if (corsList.length > 0 && isProd && !corsList.includes(origin)) {
            ...
            try {
              METRICS.inc('responses_total', { status: '403' });
              METRICS.inc('auth_blocked_total', { reason: 'cors', path: 'message' });
              span?.setAttribute?.('http.status_code', 403);
            } catch {}
            return;
          }
        ...
        req.on('end', async () => {
          try {
            ...
            try {
              ...
              const hasToken =
                token.length > 0 && (tokenFromHdr === token || tokenFromQuery === token);
              ...
              const skewToleranceMs = Math.max(
                100,
                Number(process.env.REPLAY_SKEW_TOLERANCE_MS || 0)
              );
              if (isProd && replayWinMs > 0) {
                if (!Number.isFinite(reqTs) || reqTs <= 0) {
                  res.writeHead(401, { 'Content-Type': 'application/json' });
                  ...
                  try {
                    const macId = String(req.headers['x-mac-id'] || '').trim();
                    const attemptedToken = hdr.length > 0 || tokenFromQuery.length > 0;
                    const method = attemptedToken
                      ? 'token'
                      : String(req.headers['x-client-mac'] || '').trim().length > 0
                        ? 'hmac'
                        : 'none';
                    METRICS.inc('responses_total', { status: '401' });
              ...
              try {
                if (hasToken)
                  METRICS.inc('auth_accepted_total', { method: 'token', path: 'message' });
                else if (macOk)
                  METRICS.inc('auth_accepted_total', {
                    method: 'hmac',
                    path: 'message',
                    mac_id: macId,
                  });
            ...
            try {
              const TENANT_LIMIT = Number(process.env.TENANT_LIMIT || 0);
              const TENANT_WINDOW_MS = Math.max(1, Number(process.env.TENANT_WINDOW_MS || 1000));
              const TENANT_IEO = String(
                process.env.TENANT_INTERNAL_ERROR_ONCE || '1'
              ).toLowerCase();
              const tenantInternalOnce = TENANT_IEO === '1' || TENANT_IEO === 'true';
              const backendName = String(process.env.TENANT_RL_BACKEND || 'mem').toLowerCase();
              if (!isProbe && !isControl && Number.isFinite(TENANT_LIMIT) && TENANT_LIMIT > 0) {
                ...
                const rawTenant =
                  body?.ctx?.meta?.tenant ||
                  body?.ctx?.tenancy?.tenant ||
                  body?.ctx?.tenancy?.id ||
                  body?.ctx?.vars?.tenant ||
                ...
                if (!out?.ok) {
                  ...
                  try {
                    METRICS.inc('rate_limited_total', { reason: 'tenant', scope: 'tenant' });
                    METRICS.inc('responses_total', { status: '429' });
                  } catch {}
                  return;
                }
            ...
            if (convId) {
              ...
              if (entry.count > CONV_SOFT_MAX) {
                ...
                try {
                  sampled('debug', 0.05, '[conv_gate]', {
                    conv_id: convId,
                    count: entry.count,
                    window_ms: CONV_SOFT_WINDOW_MS,
                    soft_max: CONV_SOFT_MAX,
                    wait_s: Number(waitSec.toFixed(3)),
                  });
                } catch {}
                res.writeHead(429, {
                  'Content-Type': 'application/json',
                  'Retry-After': String(waitSec.toFixed(3)),
                });
                res.end(
                  JSON.stringify({
                    error: 'rate_limited',
            ...
            try {
              ...
              if (Number.isFinite(limUsd) && limUsd > 0) {
                ...
                const textForEstimate = Array.isArray(userMsg.content)
                  ? userMsg.content.join('\n')
                ...
                try {
                  sampled('debug', 0.05, '[tenant_budget_precheck]', {
                    tenant: tenantKey,
                    est_in: estIn,
                    est_out: estOut,
                    usd_in: usdIn,
                ...
                const allowUsd = await Promise.resolve(
                  globalThis.__TENANT_USD_BUDGET__.allow(tenantKey, estUsd)
                );
                if (!allowUsd?.ok) {
                  ...
                  res.end(
                    JSON.stringify({
                      ...
                      reason: 'tenant_dollars',
                      scope: 'tenant_dollars',
                      window_ms: winUsd,
                    })
                  );
                  try {
                    METRICS.inc('budget_prevented_total', { scope: 'tenant_dollars_http' });
                    METRICS.inc('responses_total', { status: '429' });
                    span?.setAttribute?.('http.status_code', 429);
                  } catch {}
                  return;
            ...
            try {
              ...
              if (Number.isFinite(limUsdMon) && limUsdMon > 0) {
                ...
                const usdOut = Math.max(0, Number(process.env.LLM_USD_PER_TOKEN_OUT || 0));
                const estUsd = estIn * usdIn + estOut * usdOut;
                const allowUsdMon = await Promise.resolve(
                  globalThis.__TENANT_USD_MONTHLY_BUDGET__.allow(tenantKey, estUsd)
                );
                if (!allowUsdMon?.ok) {
                  res.writeHead(429, { 'Content-Type': 'application/json' });
                  res.end(
                    JSON.stringify({
                      error: 'budget_limited',
                      reason: 'tenant_dollars_monthly',
                      scope: 'tenant_dollars_monthly',
                    })
                  );
                  try {
                    METRICS.inc('budget_prevented_total', { scope: 'tenant_dollars_monthly_http' });
                    METRICS.inc('responses_total', { status: '429' });
                    span?.setAttribute?.('http.status_code', 429);
                  } catch {}
                  return;
            ...
            try {
              ...
              if (
                ...
                const usdOut = Math.max(0, Number(process.env.LLM_USD_PER_TOKEN_OUT || 0));
                const estUsd = estIn * usdIn + estOut * usdOut;
                const allowUsdRoll = await Promise.resolve(
                  globalThis.__TENANT_USD_ROLLING_BUDGET__.allow(tenantKey, estUsd)
                );
                if (!allowUsdRoll?.ok) {
                  res.writeHead(429, { 'Content-Type': 'application/json' });
                  res.end(
                    JSON.stringify({
                      error: 'budget_limited',
                      reason: 'tenant_dollars_rolling',
                      scope: 'tenant_dollars_rolling',
                      window_ms: winUsdRoll,
                    })
                  );
                  try {
                    METRICS.inc('budget_prevented_total', { scope: 'tenant_dollars_rolling_http' });
                    METRICS.inc('responses_total', { status: '429' });
                    span?.setAttribute?.('http.status_code', 429);
                  } catch {}
                  return;
            ...
            try {
              ...
              if (Number.isFinite(limTok) && limTok > 0) {
                ...
                const allowTok = await Promise.resolve(
                  globalThis.__TENANT_TOKEN_BUDGET__.allow(tenantKey, estTok)
                );
                if (!allowTok?.ok) {
                  res.writeHead(429, { 'Content-Type': 'application/json' });
                  res.end(
                  ...
                  try {
                    METRICS.inc('budget_prevented_total', { scope: 'tenant_tokens_http' });
                    METRICS.inc('responses_total', { status: '429' });
                    span?.setAttribute?.('http.status_code', 429);
                  } catch {}
                  return;
            ...
            try {
              ...
              if (Number.isFinite(limTokMon) && limTokMon > 0) {
                ...
                const estOut = Math.max(1, Number(process.env.LLM_EXPECTED_TOKENS_OUT || 128));
                const estTok = estIn + estOut;
                const allowTokMon = await Promise.resolve(
                  globalThis.__TENANT_TOKEN_MONTHLY_BUDGET__.allow(tenantKey, estTok)
                );
                if (!allowTokMon?.ok) {
                  res.writeHead(429, { 'Content-Type': 'application/json' });
                  res.end(
                    JSON.stringify({
                      error: 'budget_limited',
                      reason: 'tenant_tokens_monthly',
                      scope: 'tenant_tokens_monthly',
                    })
                  );
                  try {
                    METRICS.inc('budget_prevented_total', { scope: 'tenant_tokens_monthly_http' });
                    METRICS.inc('responses_total', { status: '429' });
                    span?.setAttribute?.('http.status_code', 429);
                  } catch {}
                  return;
            ...
            try {
              ...
              if (
                ...
                const estOut = Math.max(1, Number(process.env.LLM_EXPECTED_TOKENS_OUT || 128));
                const estTok = estIn + estOut;
                const allowTokRoll = await Promise.resolve(
                  globalThis.__TENANT_TOKEN_ROLLING_BUDGET__.allow(tenantKey, estTok)
                );
                if (!allowTokRoll?.ok) {
                  res.writeHead(429, { 'Content-Type': 'application/json' });
                  res.end(
                    JSON.stringify({
                      error: 'budget_limited',
                      reason: 'tenant_tokens_rolling',
                      scope: 'tenant_tokens_rolling',
                      window_ms: winTokRoll,
                    })
                  );
                  try {
                    METRICS.inc('budget_prevented_total', { scope: 'tenant_tokens_rolling_http' });
                    METRICS.inc('responses_total', { status: '429' });
                    span?.setAttribute?.('http.status_code', 429);
                  } catch {}
                  return;
            ...
            try {
              ...
              if (secret && idempotencyKey) {
                const macHdr = String(
                  req.headers['idempotency-mac'] ||
                    req.headers['x-idempotency-mac'] ||
                    req.headers['x-idempotency-sig'] ||
                    ''
                ).trim();
                if (!macHdr) {
                  res.writeHead(401, { 'Content-Type': 'application/json' });
                  ...
                  try {
                    METRICS.inc('responses_total', { status: '401' });
                    METRICS.inc('idempotency_mac_missing_total');
                    span?.setAttribute?.('http.status_code', 401);
                  } catch {}
                  return;
                }
                ...
                if (!ok) {
                  ...
                  try {
                    METRICS.inc('responses_total', { status: '401' });
                    METRICS.inc('idempotency_mac_invalid_total');
                    span?.setAttribute?.('http.status_code', 401);
                  } catch {}
                  return;
                }
            ...
            if (idempotencyKey) {
              ...
              if (!cached) {
                try {
                  cached = await loadIdemFromDisk(idempotencyKey);
                } catch {}
              }
              if (!cached) {
                try {
                  cached = await idemGetRedis(idempotencyKey);
                } catch {}
              }
              if (cached && Date.now() - cached.ts < IDEMPOTENCY_TTL_MS + IDEMPOTENCY_SKEW_MS) {
                ...
                try {
                  ...
                  METRICS.inc('idempotency_cache_hit_total', { path: 'message' });
                  span?.setAttribute?.('http.status_code', 200);
                } catch {}
                try {
                  ...
                  span?.setAttribute?.(
                    'llm.resolved_model',
                    String(cached?.response?.resolved_model || '')
                  );
                  span?.setAttribute?.(
                    'llm.engine_source',
                    String(cached?.response?.engine_source || 'replay')
                  );
                  span?.setAttribute?.('llm.variant_v', String(cached?.response?.variant_v || ''));
                  METRICS.inc('llm_provider_selected_total', {
                    provider: String(cached?.response?.provider || ''),
                    model: String(cached?.response?.model || ''),
                    resolved_model: String(cached?.response?.resolved_model || ''),
                    source: 'replay',
                  });
                } catch {}
                try {
                  sampled('info', { evt: 'idem_replay_served', path: 'message', source: 'replay' });
                } catch {}
                return;
              } else {
                try {
                  METRICS.inc('idempotency_cache_miss_total', { path: 'message' });
                } catch {}
                try {
                  sampled('debug', { evt: 'idem_cache_miss', path: 'message' });
                } catch {}
                ...
                try {
                  ...
                  if (!claimed) {
                    const hedgeWaitMs = Math.max(
                      0,
                      Number(process.env.HEDGE_CUTOVER_MAX_WAIT_MS || 1500)
                    );
                    const t0 = Date.now();
                    let replay = null;
                    ...
                    if (
                      replay &&
                      Date.now() - replay.ts < IDEMPOTENCY_TTL_MS + IDEMPOTENCY_SKEW_MS
                    ) {
                      try {
                        METRICS.inc('hedge_cutover_once_total');
                      } catch {}
                      res.writeHead(200, { 'Content-Type': 'application/json' });
                      ...
                      try {
                        sampled('info', { evt: 'hedge_cutover_replay_served', path: 'message' });
                      } catch {}
                      return;
                    }
                    res.writeHead(409, { 'Content-Type': 'application/json' });
                    res.end(
                      JSON.stringify({ error: 'duplicate_message', ttl_ms: IDEMPOTENCY_TTL_MS })
                    );
                    try {
                      METRICS.inc('responses_total', { status: '409' });
                      span?.setAttribute?.('http.status_code', 409);
                    } catch {}
                    try {
                      sampled('warn', { evt: 'duplicate_message_lock_denied', path: 'message' });
                    } catch {}
                    return;
                  } else {
            ...
            try {
              ...
              try {
                ...
                if (tnorm === '!ultra' || tnorm === '!ultra toggle' || tnorm === '!ultra.toggle') {
                  ...
                  try {
                    METRICS.inc('ultra_toggle_total', { path: 'message' });
                  } catch {}
                ...
                if (tnorm === '!ultra on' || tnorm === '!ultraon' || tnorm === '!ultra.enable') {
                  ...
                  try {
                    METRICS.inc('ultra_state_total', { path: 'message', state: 'on' });
                  } catch {}
                ...
                if (tnorm === '!ultra off' || tnorm === '!ultraoff' || tnorm === '!ultra.disable') {
                  ...
                  try {
                    METRICS.inc('ultra_state_total', { path: 'message', state: 'off' });
                  } catch {}
              ...
              try {
                ...
                if (tnorm === '!fail' || tnorm === '!fail toggle' || tnorm === '!fail.toggle') {
                  ...
                  try {
                    METRICS.inc('failroll_force_toggle_total', { path: 'message' });
                  } catch {}
                ...
                if (tnorm === '!fail on' || tnorm === '!failon' || tnorm === '!fail.enable') {
                  setForceFail(cid, 'on');
                  try {
                    METRICS.inc('failroll_force_set_total', { path: 'message', enabled: 'true' });
                  } catch {}
                ...
                if (tnorm === '!fail off' || tnorm === '!failoff' || tnorm === '!fail.disable') {
                  ...
                  try {
                    METRICS.inc('failroll_force_set_total', { path: 'message', enabled: 'false' });
                  } catch {}
                ...
                if (tnorm === '!fail once' || tnorm === '!fail.once') {
                  ...
                  try {
                    METRICS.inc('failroll_force_once_total', { path: 'message' });
                  } catch {}
              ...
              try {
                ...
                const m = String(text || '')
                  .trim()
                  .match(/^!roll\s+style\s+(\w+)\s*$/i);
                if (m) {
                  const style = String(m[1] || '').toLowerCase();
                  const pref = setRollPref(cid, { style });
                  const roll_meta = computeRollMeta(cid);
                  try {
                    METRICS.inc('roll_pref_changed_total', { path: 'message' });
                  } catch {}
                  try {
                    broadcastAdminMemoryEvent(cid, 'roll.pref', { conv_id: cid, roll_meta });
                  } catch {}
              ...
              try {
                indexTurn({ convId: cid, role: 'user', text });
              } catch {}
              // Watchdog: log user turn (message path)
              try {
                await logTurn(cid, 'user', String(text || ''));
              } catch {}
              // Scene chat commands (optional): !scene tag <name>, !scene goto <name>
              try {
                trySceneChatCommand(cid, text);
              } catch {}

              // Player Commands
              const cmdRemember =
                text.match(/^\s*!remembermessage\s*(\d+)\s*$/i) ||
                text.match(/^\s*!remembermessage(\d+)\s*$/i);
              ...
              if (cmdRemember) {
                ...
                stageBooster({
                  ...
                  id,
                  anchor,
                  range: [anchor - before, anchor + after],
                  text: recap,
                  ttlTurns: ttl,
                  agent: 'bot',
                  source: recap && recap.length >= 40 ? 'heur' : 'llm_or_heur',
                });
                try {
                  METRICS.inc('booster_staged_total', { path: 'message' });
                } catch {}
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(
                  JSON.stringify({
                    ok: true,
                    system: {
                      info: 'memory_boost_staged',
                      anchor,
                      range: [Math.max(1, anchor - before), anchor + after],
                    },
              ...
              if (cmdDel) {
                ...
                try {
                  if (ok) METRICS.inc('booster_deleted_total', { path: 'message' });
                } catch {}
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(
                  JSON.stringify({
              ...
              try {
                ...
                try {
                  activeAgent =
                    String(body?.ctx?.vars?.active_agent || body?.active_agent || '').trim() ||
                    'bot';
                } catch {}
                ...
                if (boost) {
                  ...
                  try {
                    ...
                    res.locals.__booster_text = String(boost);
                  } catch {}
                  try {
                    METRICS.inc('booster_used_total', { path: 'message' });
                  } catch {}
                }
              } catch {}
              ...
              try {
                ...
                const activeAgent = String(
                  body?.ctx?.vars?.active_agent || body?.active_agent || 'bot'
                );
                ...
                if (!hasBooster) {
                  ...
                  if (Array.isArray(dreams) && dreams.length) {
                    ...
                    if (!current.trim().startsWith('(')) {
                      ...
                      try {
                        res.locals = res.locals || {};
                        res.locals.__dream_text = String(dreamLine);
                      } catch {}
                      try {
                        METRICS.inc('dreams_injected_total', { path: 'message' });
                      } catch {}
                    }
                  }
                }
            ...
            const { bytes: reqBytes, hash: reqHash } = assembleForModel([userMsg], {
              persona_v: body?.persona_v,
              prompt_v: body?.prompt_v,
            });
            const runtime = createBotRuntime({
              respond: async (input, ctx) => {
                const text = Array.isArray(input?.content)
                  ? input.content.join('\n')
                ...
                const heuristicsDisabled = String(
                  process.env.LLM_HEURISTICS_DISABLED || ''
                ).toLowerCase();
                let model = 'urga';
                if (engineCandidate) {
                  model = ['echo', 'urga', 'dreams'].includes(engineCandidate)
                    ? engineCandidate
                    : 'urga';
                } else {
                  engineSource =
                    heuristicsDisabled === '1' || heuristicsDisabled === 'true'
                      ? 'default'
                      : 'heuristic';
                  if (engineSource === 'heuristic') {
                    model = /echo|gods|pantheon/i.test(text)
                      ? 'echo'
                      : /dream|sleep|night|hallucin|lucid/i.test(text)
                        ? 'dreams'
                        : 'urga';
                  } else {
                    model = 'urga';
                ...
                try {
                  ctxLocal.vars.engine_source = engineSource;
                } catch {}
                ...
                try {
                  ctxLocal.vars.path = 'message';
                } catch {}
                try {
                  if (cid) ctxLocal.vars.conv_id = cid;
                } catch {}
                // Refusal/policy router: dedicated jailbreak blocker with citations request
                try {
                  const refusalEnabled = String(
                    process.env.POLICY_REFUSAL_ENABLED || '1'
                  ).toLowerCase();
                  const threshold = Math.max(
                    0,
                    Math.min(1, Number(process.env.JAILBREAK_REFUSAL_THRESHOLD || 0.7))
                  );
                  const wantCitations = String(
                    process.env.POLICY_CITATIONS_REQUEST || '1'
                  ).toLowerCase();
                  const signalsPre = computeAbuseSignals(text);
                  const jbLevelPre =
                    signalsPre?.levels?.jailbreak ||
                    (signalsPre &&
                      (signalsPre.jailbreak_signal >= 0.7
                        ? 'high'
                        : signalsPre.jailbreak_signal >= 0.4
                          ? 'med'
                          : 'low'));
                  try {
                    ctxLocal.vars.__abuse_signals_pre = signalsPre;
                  } catch {}
                  const enabled = refusalEnabled === '1' || refusalEnabled === 'true';
                  if (
                    enabled &&
                    (signalsPre.jailbreak_signal >= threshold || jbLevelPre === 'high')
                  ) {
                    const refusalMsg =
                      wantCitations === '1' || wantCitations === 'true'
                    ...
                    const assistant = createMessage({
                      role: 'assistant',
                      turn: Number(input?.turn || 0) + 1,
                      conv_id: input?.conv_id,
                      content: [refusalMsg],
                    });
                    try {
                      METRICS.inc('policy_refusal_total', {
                        class: 'jailbreak',
                        level: jbLevelPre || 'high',
                        source: engineSource,
                      });
                      METRICS.inc('jailbreak_levels_histogram_total', {
                        level: jbLevelPre || 'high',
                        path: 'message',
                      });
                      logAt(
                    ...
                    return {
                      ...
                      variant_v: ctxLocal.vars.abVariant,
                      engine_source: engineSource,
                    };
                  }
                } catch {}
                ...
                if (cid) {
                  if (!variant) {
                    ...
                    if (!existing) {
                      try {
                        existing = await loadAbVariant(cid);
                      } catch {}
                    }
                    if (existing) {
                      variant = existing;
                    ...
                    if (!prior || prior !== variant) {
                      ...
                      try {
                        await saveAbVariant(cid, variant);
                      } catch {}
                    }
                  }
                }
                ...
                const dry = String(
                  process.env.LLM_DRY_RUN || process.env.DRY_RUN || '0'
                ).toLowerCase();
                ...
                try {
                  ctxLocal.vars.toolExecutedPrior = await hasToolExecuted(toolCallId);
                } catch {
                  ctxLocal.vars.toolExecutedPrior = false;
                }
                ...
                try {
                  if (String(process.env.SPINE_ENABLED || '1') === '1') {
                    ...
                    try {
                      ctxLocal.vars.spine = sp;
                    } catch {}
                    try {
                      applySpineToLoopGuard(ctxLocal, sp);
                    } catch {}
                  }
                } catch {}
                ...
                try {
                  await maybeFactGuardNudge(ctxLocal, {
                    conv_id: cid,
                    turnIndex: Number(input?.turn || 0),
                  });
                } catch {}
                ...
                try {
                  ...
                  if (GUARD && cid) {
                    ...
                    if (v && v.text) {
                      try {
                        METRICS.inc('guard_hint_available_total', { path: 'message' });
                      } catch {}
                      const used = consumeGuardHint(cid);
                      if (used) {
                        guardPrefix = String(used || '');
                        try {
                          METRICS.inc('guard_hint_injected_total', { path: 'message' });
                        } catch {}
                        try {
                          METRICS.inc('continuity_guard_used_total', { path: 'message' });
                        } catch {}
                      }
                    }
                  }
                ...
                try {
                  ...
                  if (ARC_ON && cid) {
                    if (!arc) {
                      ...
                      try {
                        const got = getArc(cid);
                        arc = String(got?.arc || '').trim();
                      } catch {}
                      if (!arc) {
                        const inf = inferArcFromText(String(text || '')) || '';
                        if (inf) {
                          try {
                            const resArc = setArc(cid, inf);
                            if (resArc?.ok)
                              METRICS.inc('memory_arc_inferred_total', { path: 'message' });
                          } catch {}
                          arc = inf;
                        }
                      }
                      ...
                      try {
                        const resArc = setArc(cid, arc);
                        if (resArc?.ok) METRICS.inc('memory_arc_set_total', { path: 'message' });
                      } catch {}
                    }
                  }
                  try {
                    ctxLocal.vars.arc = arc;
                  } catch {}
                  const FACTS_ON =
                    String(process.env.FACT_INJECT_ENABLED || '') === '1' ||
                    String(process.env.FACTS_INJECTION_ENABLED || '') === '1';
                  if (FACTS_ON) {
                    const rel = await selectRelevantFactsForTurn(
                      cid,
                      String(text || ''),
                      Number(process.env.FACT_INJECT_MAX || 3),
                      { agentId, arc }
                    );
                    if (Array.isArray(rel) && rel.length) {
                      ...
                      try {
                        ctxLocal.vars.__facts_injected_ids = rel.map((f) => f.id);
                      } catch {}
                      try {
                        for (let i = 0; i < rel.length; i++)
                          METRICS.inc('facts_injected_total', { path: 'message' });
                      } catch {}
                    }
                  }
                } catch {}
                  ...
                  if (String(process.env.SCENE_LINKING_ENABLED || '') === '1') {
                    ...
                    if (scene) {
                      ...
                      if (line) {
                        ...
                        try {
                          METRICS.inc('scene_injected_total', { path: 'message' });
                        } catch {}
                      }
                      try {
                        const ids = Array.isArray(ctxLocal?.vars?.__facts_injected_ids)
                          ? ctxLocal.vars.__facts_injected_ids
                          : [];
                ...
                try {
                  ...
                  if (ultraOn) {
                    const novelty = String(
                      process.env.ULTRA_NOVELTY_HINT ||
                        'Avoid familiar phrasing; vary cadence; add one vivid, specific detail.'
                    );
                    guardPrefix = guardPrefix ? `${guardPrefix} | ${novelty}` : novelty;
                    try {
                      METRICS.inc('ultra_novelty_hint_total', { path: 'message' });
                    } catch {}
                  }
                } catch {}
                ...
                try {
                  if (String(process.env.BELIEFS_ENABLED || '').trim()) {
                    ...
                    try {
                      if (
                        Number(process.env.BELIEFS_CONFLICT_SSE || 0) &&
                        Array.isArray(conflicts) &&
                        conflicts.length
                      ) {
                        for (const c of conflicts) {
                          try {
                            broadcastAdminMemoryEvent(cid, 'memory.belief.conflict', {
                              conv_id: cid,
                              belief_id: c.id,
                              text: c.text,
                            });
                          ...
                          try {
                            METRICS.inc('belief_conflict_total', {
                              path: 'message',
                              char_id: String(ctxLocal?.vars?.agent_id || 'default'),
                            });
                          } catch {}
                        }
                      }
                    ...
                    if (Array.isArray(beliefBoosters) && beliefBoosters.length) {
                      try {
                        for (const b of beliefBoosters) {
                          try {
                            broadcastAdminMemoryEvent(cid, 'memory.belief', {
                              conv_id: cid,
                          ...
                          try {
                            METRICS.inc('belief_injected_total', {
                              path: 'message',
                              char_id: String(ctxLocal?.vars?.agent_id || 'default'),
                            });
                          } catch {}
                      ...
                      try {
                        ctxLocal.vars.__extraBoosters = (
                          Array.isArray(ctxLocal.vars.__extraBoosters)
                            ? ctxLocal.vars.__extraBoosters
                            : []
                        ).concat(beliefBoosters.map((x) => String(x.text || '')));
                      } catch {}
                    }
                  }
                } catch {}
                ...
                try {
                  if (String(process.env.WATCHDOG_ENABLED || '').trim()) {
                    ...
                    try {
                      if (
                        Number(process.env.WATCHDOG_SSE || 0) &&
                        Array.isArray(flags) &&
                        flags.length
                      ) {
                        for (const f of flags) {
                          try {
                            broadcastAdminMemoryEvent(cid, 'memory.contradiction', {
                              conv_id: cid,
                              key: f.key,
                              value: f.value,
                              severity: f.severity,
                    ...
                    try {
                      if (Array.isArray(flags)) {
                        for (const f of flags) {
                          METRICS.inc('contradiction_flag_total', {
                            path: 'message',
                ...
                try {
                  ...
                  try {
                    ctxLocal.vars.__memory_injected_tokens = memoryInjectTokens;
                  } catch {}
                  try {
                    METRICS.inc('memory_inject_tokens_total', {
                      tokens: String(memoryInjectTokens),
                      path: 'message',
                    });
                  } catch {}
                } catch {}
                ...
                try {
                  ...
                  ctxLocal.memory.personality =
                    (await BeliefStore.getPersonality(agentIdMem)) ||
                    ctxLocal.memory.personality ||
                    '';
                } catch {}
                // --- Disagreement Guards: compute and prepend one-liners (message) ---
                try {
                  const dg = await computeDisagreementGuards(ctxLocal, {
                    conv_id: String(cid || ''),
                    turn: Number(input?.turn || 0),
                    userText: String(text || ''),
                  });
                  if (dg && Array.isArray(dg.guards)) {
                    ...
                    for (const g of dg.guards) {
                      if (g.type === 'belief' && g.text) {
                        ...
                        try {
                          METRICS.inc('guard_belief_total', { path: 'message' });
                        } catch {}
                        try {
                          broadcastAdminMemoryEvent(String(cid || ''), 'guard.belief', {
                            conv_id: String(cid || ''),
                            text: String(g.text || ''),
                            style: String(dg.style || ''),
                          });
                        } catch {}
                      } else if (g.type === 'constraint' && g.text) {
                        lines.push(`Constraint: ${String(g.text || '')}`);
                        try {
                          METRICS.inc('guard_constraint_total', { path: 'message' });
                        } catch {}
                        try {
                          broadcastAdminMemoryEvent(String(cid || ''), 'guard.constraint', {
                            conv_id: String(cid || ''),
                            text: String(g.text || ''),
                            style: String(dg.style || ''),
                          });
                        ...
                        try {
                          METRICS.inc('guard_contradiction_total', { path: 'message' });
                        } catch {}
                        try {
                          broadcastAdminMemoryEvent(String(cid || ''), 'guard.contradiction', {
                            conv_id: String(cid || ''),
                            why: String(g.why || ''),
                            fact: String(g.fact || ''),
                          });
                    ...
                    if (lines.length) {
                      ...
                      try {
                        ctxLocal.vars.__guard_hint = String(guardPrefix || block || '');
                      } catch {}
                    }
                    if (dg.failureRoll) {
                      try {
                        METRICS.inc('guard_failure_total', {
                          path: 'message',
                          outcome: String(dg.failureRoll?.outcome || 'none'),
                        });
                      } catch {}
                      try {
                        broadcastAdminMemoryEvent(String(cid || ''), 'guard.failure', {
                          conv_id: String(cid || ''),
                          pct: Number(dg.failureRoll.pct || 0),
                          roll: Number(dg.failureRoll.roll || 0),
                          outcome: String(dg.failureRoll.outcome || ''),
                          verbs: Array.isArray(dg.failureRoll.verbs) ? dg.failureRoll.verbs : [],
                        });
                      } catch {}
                    }
                  }
                  ...
                  if (dg?.hardRefusal) {
                    try {
                      METRICS.inc('refusal_total', {
                        path: 'message',
                        mode: 'hard',
                        reason: 'belief_constraint',
                      });
                    } catch {}
                    const refusalText = renderRefusal({
                      style: String(dg.style || 'firm'),
                      reason: 'belief_constraint',
                      spine: ctxLocal?.vars?.spine,
                      userText: String(text || ''),
                    });
                    const assistant = createMessage({
                      role: 'assistant',
                    ...
                    try {
                      ctxLocal.vars.__guard_hint = String(guardPrefix || '');
                    } catch {}
                    return {
                      ok: true,
                      refused: true,
                      ...
                      model: 'refusal',
                      provider: 'local',
                      resolved_model: 'refusal',
                      reply: assistant,
                      hash: 'refusal',
                      bytes_b64: '',
                      guard_hint: String(ctxLocal?.vars?.__guard_hint || ''),
                    };
                  }
                } catch {}
                ...
                try {
                  ...
                  try {
                    if (String(process.env.BELIEFS_ENABLED || '').trim()) {
                      ...
                      if (profile) {
                        const picks = pickRelevantStateBeliefs(profile, textCore, {
                          max: Number(process.env.BELIEF_MAX_LINES || 3),
                        });
                        if (Array.isArray(picks) && picks.length)
                          beliefLinesCore = picks.map((p) => `Belief: ${String(p.text || '')}`);
                      }
                    }
                  } catch {}
                  try {
                  ...
                  const {
                    ...
                    reasons: disagreeReasons,
                  } = await runDisagreementCore(ctxLocal, {
                    convId: convIdCore,
                    userText: textCore,
                    beliefLines: beliefLinesCore,
                    contradictionLines: contradictionLinesCore,
                  });
                  try {
                    ctxLocal.vars.__disagree_action__ = String(disagreeAction || 'none');
                  } catch {}
                  if (Array.isArray(disagreeReasons) && disagreeReasons.length) {
                    try {
                      for (const r of disagreeReasons)
                        METRICS.inc('disagreement_core_trigger_total', {
                          source: String(r.code || 'other'),
                          path: 'message',
                        });
                    } catch {}
                    try {
                      const sp = ctxLocal?.vars?.spine || {};
                      broadcastAdminMemoryEvent(convIdCore, 'disagree.core', {
                        action: String(disagreeAction || 'none'),
                        reasons: disagreeReasons,
                        mood: String(sp?.mood || ''),
                        trust: Number(sp?.trust ?? 0),
                        suspicion: Number(sp?.suspicion ?? 0),
                      });
                    } catch {}
                  }
                  if (Array.isArray(disagreeLines) && disagreeLines.length) {
                    ...
                    let addTok = 0;
                    try {
                      addTok = Number(TokenCounter.estimate(addText, { model })) || 0;
                    } catch {
                      addTok = 0;
                    }
                    const budget = Number(
                      process.env.MEMORY_INJECT_BUDGET_TOKENS || Number.MAX_SAFE_INTEGER
                    );
                    if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                      ...
                      try {
                        METRICS.inc('memory_inject_tokens_total', {
                          tokens: String(addTok),
                          path: 'message',
                        });
                      } catch {}
                    } else {
                      try {
                        METRICS.inc('disagree_core_skipped_total', {
                          reason: 'budget',
                          path: 'message',
                        });
                      } catch {}
                ...
                try {
                  ...
                  if (criticEnabled) {
                    const beliefsHit = Boolean(
                      ctxLocal?.vars?.__beliefs_conflict ||
                        ctxLocal?.vars?.__contradiction_flag ||
                        ctxLocal?.vars?.__critic_block
                    );
                    const critic = await constraintCritic(
                      { text, beliefsHit },
                      {
                        callLLM:
                          String(process.env.CRITIC_USE_LLM || '') === '1'
                            ? async (uText, { timeoutMs, maxTokens }) => {
                                ...
                                const prompt =
                                  ...
                                  `Input: ${uText}`;
                                try {
                                  const r = await LLMService.call(prompt, {
                                    engine,
                                    timeoutMs,
                                    maxTokens,
                                  });
                    ...
                    if (critic?.violated) {
                      try {
                        ctxLocal.vars.__critic_block = true;
                      } catch {}
                      try {
                        ctxLocal.vars.__critic_reason = critic.reason;
                      } catch {}
                      try {
                        ctxLocal.vars.__critic_class = critic.class;
                      } catch {}
                      ...
                      let addTok = 0;
                      try {
                        addTok = Number(TokenCounter.estimate(refusalCue, { model })) || 0;
                      } catch {
                        addTok = 0;
                      }
                      const budget = Number(
                      ...
                      if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                        ...
                        try {
                          METRICS.inc('constraint_critic_skipped_total', {
                            reason: 'budget',
                            path: 'message',
                          });
                        } catch {}
                      ...
                      try {
                        METRICS.inc('constraint_violations_total', {
                          via: String(critic.via || 'rules'),
                          class: String(critic.class || 'world'),
                          path: 'message',
                          reason: String(critic.reason || ''),
                          count: 1,
                        });
                      } catch {}
                      try {
                        broadcastAdminMemoryEvent(String(cid || ''), 'memory.constraint', {
                          conv_id: String(cid || ''),
                          class: String(critic.class || 'world'),
                          reason: String(critic.reason || ''),
                          via: String(critic.via || 'rules'),
                      ...
                      try {
                        ...
                        if (
                          HARD_REFUSAL.includes(String(critic.class || '')) ||
                          HARD_REFUSAL.includes(key)
                        ) {
                          try {
                            METRICS.inc('refusal_total', {
                              class: String(critic.class || ''),
                              reason: String(critic.reason || ''),
                              path: 'message',
                              mode: 'hard',
                            });
                ...
                try {
                  ...
                  if (disagreeAct !== 'refuse') {
                    failroll = await maybeApplyFailureRoll(ctxLocal, {
                      convId: convIdFR,
                      turn: turnFR,
                      userText: String(text || ''),
                    });
                    // Metrics & admin event
                    try {
                      METRICS.inc('failroll_evaluations_total', {
                        outcome: String(failroll.outcome || 'none'),
                        path: 'message',
                      });
                    } catch {}
                    try {
                    ...
                    try {
                      if (failroll?.eval?.nearMiss)
                        METRICS.inc('failroll_complications_total', {
                          count: 1,
                          path: 'message',
                          beat: String(failroll?.eval?.beat || ''),
                          style: String(failroll?.eval?.styleClass || ''),
                        });
                    } catch {}
                    try {
                      const d = Number(failroll?.eval?.delta || 0);
                      if (d !== 0)
                        METRICS.inc('failroll_tension_adjust_total', {
                          count: 1,
                          path: 'message',
                          outcome: String(failroll?.outcome || 'none'),
                          beat: String(failroll?.eval?.beat || ''),
                    ...
                    try {
                      if (failroll?.eval)
                        broadcastAdminMemoryEvent(convIdFR, 'failroll.eval', {
                          conv_id: convIdFR,
                          ...failroll.eval,
                          outcome: String(failroll.outcome || 'none'),
                        });
                    ...
                    try {
                      if (failroll?.eval?.nearMiss) {
                        ...
                        broadcastAdminMemoryEvent(convIdFR, 'memory.fact', {
                          conv_id: convIdFR,
                          kind: 'complication',
                          vb,
                          beat,
                  ...
                  if (Array.isArray(failroll.lines) && failroll.lines.length) {
                    ...
                    if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                      ...
                      try {
                        METRICS.inc('failroll_booster_skipped_total', {
                          reason: 'budget',
                          path: 'message',
                        });
                      } catch {}
                ...
                try {
                  if (String(process.env.SPINE_ENABLED || '1') === '1') {
                    ...
                    if (sp && sp.boosterLine) {
                      let addTok = 0;
                      try {
                        addTok = Number(TokenCounter.estimate(sp.boosterLine, { model })) || 0;
                      } catch {
                        addTok = 0;
                      }
                      const budget = Number(
                      ...
                      if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                        ...
                        memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${sp.boosterLine}`;
                        try {
                          ctxLocal.vars.__spine_booster_text = String(sp.boosterLine || '');
                        } catch {}
                        try {
                          ctxLocal.vars.__memory_prefix = memoryPrefix;
                        } catch {}
                        ...
                        try {
                          METRICS.inc('spine_booster_skipped_total', {
                            reason: 'budget',
                            path: 'message',
                          });
                        } catch {}
                ...
                try {
                  if (guardPrefix) {
                    let addTok = 0;
                    try {
                      addTok = Number(TokenCounter.estimate(guardPrefix, { model })) || 0;
                    } catch {
                      addTok = 0;
                    }
                    memoryInjectTokens += addTok;
                    ...
                    try {
                      ctxLocal.vars.__guard_hint = guardPrefix;
                    } catch {}
                    try {
                      ctxLocal.vars.__memory_injected_tokens = memoryInjectTokens;
                    } catch {}
                  ...
                  if (cfgLG && cfgLG.enabled) {
                    ...
                    try {
                      const memTokens = Array.isArray(ctxLocal?.memory?.ultra_style_tokens)
                        ...
                        : Array.isArray(ctxLocal?.vars?.ultra_style_tokens)
                          ? ctxLocal.vars.ultra_style_tokens
                          : [];
                    ...
                    if (chosen) {
                      ...
                      let addTok = 0;
                      try {
                        addTok = Number(TokenCounter.estimate(booster, { model })) || 0;
                      } catch {
                        addTok = 0;
                      }
                      const budget = Number(
                      ...
                      if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                        ...
                        memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${booster}`;
                        try {
                          ctxLocal.vars.__loopguard_style_token = String(chosen || '');
                        } catch {}
                        try {
                          ctxLocal.vars.__loopguard_style_nudge = booster;
                        } catch {}
                        try {
                          ctxLocal.vars.__memory_prefix = memoryPrefix;
                        } catch {}
                        ...
                        try {
                          METRICS.inc('loopguard_style_nudge_total', {
                            path: 'message',
                            token: String(chosen || ''),
                          });
                        } catch {}
                        try {
                          broadcastAdminMemoryEvent(convIdLG, 'memory.loopguard_style', {
                            conv_id: convIdLG,
                            token: String(chosen || ''),
                          });
                        } catch {}
                        ...
                        try {
                          METRICS.inc('loopguard_style_nudge_skipped_total', {
                            reason: 'budget',
                            path: 'message',
                          });
                        } catch {}
                  ...
                  if (String(process.env.BELIEFS_ENABLED || '').trim()) {
                    ...
                    if (profile) {
                      ...
                      if (Array.isArray(picks) && picks.length) {
                        ...
                        if (!recent) {
                          recent = new Map();
                          STATE_BELIEF_RECENT.set(convId, recent);
                        }
                        const chosen = [];
                        for (const p of picks) {
                          const h = crypto
                            .createHash('sha256')
                            .update(String(p.text || ''))
                            .digest('hex')
                            .slice(0, 16);
                        ...
                        if (chosen.length) {
                          ...
                          // Admin SSE snapshot
                          try {
                            broadcastAdminMemoryEvent(convId, 'memory.beliefs', {
                              conv_id: convId,
                              lines: chosen.map((x) => ({
                                kind: x.kind,
                                text: x.text,
                ...
                try {
                  const extrasRaw = Array.isArray(ctxLocal?.vars?.__extraBoosters)
                    ? ctxLocal.vars.__extraBoosters
                    : [];
                  ...
                  const extrasUniq = Array.from(
                    new Set(extrasRaw.map((x) => String(x || '').trim()).filter(Boolean))
                  );
                ...
                try {
                  ...
                  let signals = null;
                  try {
                    signals = ctxLocal?.vars?.__abuse_signals_pre || computeAbuseSignals(text);
                  } catch {
                    signals = null;
                  }
                  const threshold = Math.max(
                  ...
                  if (enabled && moderatelyRisky) {
                    ...
                    let addTok = 0;
                    try {
                      addTok = Number(TokenCounter.estimate(hintText, { model })) || 0;
                    } catch {
                      addTok = 0;
                    }
                    const budget = Number(
                    ...
                    if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                      ...
                      memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${hintText}`;
                      try {
                        ctxLocal.vars.__refusal_hint_text = hintText;
                      } catch {}
                      try {
                        ctxLocal.vars.__memory_prefix = memoryPrefix;
                      } catch {}
                      ...
                      try {
                        METRICS.inc('refusal_hint_injected_total', {
                          path: 'message',
                          style: styleR,
                          level: lvl || 'med',
                        });
                      ...
                      try {
                        broadcastAdminRefusalEvent(String(cid || ''), 'refusal.hint', {
                          conv_id: String(cid || ''),
                          text: hintText,
                          style: styleR,
                          level: lvl || 'med',
                      ...
                      try {
                        METRICS.inc('refusal_hint_skipped_total', {
                          reason: 'budget',
                          path: 'message',
                        });
                      } catch {}
                ...
                try {
                  ...
                  if (__style_meta) {
                    ...
                    try {
                      ctxLocal.vars.style = __style_meta;
                    } catch {}
                    try {
                      ctxLocal.vars.model = model;
                    } catch {}
                    const sb = buildStyleBooster(ctxLocal);
                    if (sb && sb.text) {
                      let addTok = 0;
                      try {
                        addTok = Number(TokenCounter.estimate(sb.text, { model })) || 0;
                      } catch {
                        addTok = 0;
                      }
                      memoryInjectTokens += addTok;
                      memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${sb.text}`;
                      try {
                        ctxLocal.vars.__style_booster_text = sb.text;
                      } catch {}
                      try {
                        ctxLocal.vars.__memory_injected_tokens = memoryInjectTokens;
                      } catch {}
                      ...
                      try {
                        METRICS.inc('style_booster_injected_total', {
                          path: 'message',
                          preset: String(__style_meta?.preset || ''),
                        });
                      } catch {}
                      try {
                        broadcastAdminStyleEvent(cid, 'style.booster', {
                          conv_id: cid,
                          text: sb.text,
                          preset: String(__style_meta?.preset || ''),
                        });
                ...
                try {
                  ...
                  const win =
                    getWindowAround(
                      String(cid || ''),
                      Number(anchorSeq || 0),
                      Number(process.env.BEATS_WINDOW_BEFORE || 30),
                      0
                    ) || [];
                  const lastBot = Array.isArray(win)
                    ? [...win].reverse().find((m) => String(m?.role || '') === 'bot')
                    : null;
                  const botPrev = String(lastBot?.text || '');
                  const beatSignals = collectBeatSignals(String(cid || ''), ctxLocal, {
                    userText: String(text || ''),
                    botPrev,
                  });
                  try {
                    ctxLocal.vars.__beat_signals = beatSignals;
                  } catch {}
                  const tensionHint =
                    beatSignals && beatSignals.inputs && beatSignals.inputs.style
                      ? beatSignals.inputs.style.tensionHint
                      : (() => {
                          try {
                            const t = Number(ctxLocal?.vars?.tension ?? ctxLocal?.memory?.tension);
                            return Number.isFinite(t) ? t : null;
                          } catch {
                            return null;
                          }
                        })();
                  const planBeat = detectBeat(String(cid || ''), {
                    userText: String(text || ''),
                    botPrev,
                    tensionHint,
                  });
                  if (planBeat && planBeat.enabled) {
                    try {
                      ctxLocal.vars.__beat_plan = planBeat;
                    } catch {}
                    // Align style hedger preference with beat-suggested preset
                    try {
                      ctxLocal.vars.style = {
                        ...(ctxLocal.vars.style || {}),
                        preset: String(planBeat.styleToken || ''),
                        beat: String(planBeat.beat || ''),
                        tension: Number(planBeat.tension || 0),
                      };
                    } catch {}
                    ...
                    if (cadenceBoost && cadenceBoost.text) {
                      ...
                      try {
                        addTok =
                          Number(TokenCounter.estimate(cadenceBoost.text, { model })) ||
                          Number(cadenceBoost.estTokens) ||
                          0;
                      } catch {
                        addTok = Number(cadenceBoost.estTokens || 0) || 0;
                      }
                      memoryInjectTokens += addTok;
                      memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${cadenceBoost.text}`;
                      try {
                        ctxLocal.vars.__cadence_booster_text = String(cadenceBoost.text || '');
                      } catch {}
                      try {
                        ctxLocal.vars.__memory_injected_tokens = memoryInjectTokens;
                      } catch {}
                      ...
                      try {
                        METRICS.inc('cadence_booster_injected_total', {
                          path: 'message',
                          beat: String(planBeat.beat || ''),
                          style: String(planBeat.styleToken || ''),
                        });
                      } catch {}
                    }
                    try {
                      broadcastAdminStyleEvent(cid, 'beat.tick', {
                        conv_id: cid,
                        tension: Number(planBeat.tension || 0),
                        beat: String(planBeat.beat || ''),
                        style: String(planBeat.styleToken || ''),
                        cadence: String(planBeat.cadence || ''),
                      });
                    } catch {}
                    try {
                      METRICS.inc('beat_ticks_total', {
                        path: 'message',
                        beat: String(planBeat.beat || ''),
                      });
                    } catch {}
                ...
                try {
                  ...
                  try {
                    METRICS.inc('loop_phrase_seen_total', { path: 'message_user_preturn' });
                  } catch {}
                } catch {}
                ...
                try {
                  ...
                  if (plan && plan.enabled) {
                    ...
                    const items = Array.isArray(plan.cooldown)
                      ? plan.cooldown.map((p) => ({
                          hash: String(p.hash || ''),
                    ...
                    if (avoid && avoid.text) {
                      let addTok = 0;
                      try {
                        addTok = Number(TokenCounter.estimate(avoid.text, { model })) || 0;
                      } catch {
                        addTok = Number(avoid.estTokens || 0) || 0;
                      }
                      memoryInjectTokens += addTok;
                      memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${avoid.text}`;
                      try {
                        ctxLocal.vars.__phrase_avoid_text = String(avoid.text || '');
                      } catch {}
                      try {
                        ctxLocal.vars.__memory_injected_tokens = memoryInjectTokens;
                      } catch {}
                    ...
                    if (items.length) {
                      try {
                        ctxLocal.io?.events?.emit?.('loop.phrase.plan', { conv_id: cid, items });
                      } catch {}
                      try {
                        broadcastAdminMemoryEvent(cid, 'loop.phrase.plan', { conv_id: cid, items });
                      } catch {}
                      try {
                        METRICS.inc('loop_phrase_plan_total', {
                          path: 'message',
                ...
                try {
                  ...
                  if (Array.isArray(hot) && hot.length) {
                    const plan2 = {
                      enabled: true,
                      cooldown: hot.map((h) => ({ phrase: String(h.phrase || '') })),
                    };
                    const avoid2 = buildAvoidanceBooster(plan2);
                    if (avoid2 && avoid2.text) {
                      let addTok = 0;
                      try {
                        addTok = Number(TokenCounter.estimate(avoid2.text, { model })) || 0;
                      } catch {
                        addTok = Number(avoid2.estTokens || 0) || 0;
                      }
                      memoryInjectTokens += addTok;
                      memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${avoid2.text}`;
                      try {
                        ctxLocal.vars.__phrase_hot_count = Number(hot.length || 0);
                      } catch {}
                      try {
                        ctxLocal.vars.__memory_injected_tokens = memoryInjectTokens;
                      } catch {}
                ...
                try {
                  const sseEmit = (event, data) => {
                    try {
                      broadcastAdminMemoryEvent(cid, event, { conv_id: cid, ...data });
                    } catch {}
                  };
                  const conc = maybeStageConclusionBooster(
                    ctxLocal,
                ...
                try {
                  if (String(process.env.CONCLUDE_ENABLED || '0') === '1') {
                    ...
                    const loopScore = Number(
                      ctxVars.__loopguard_loopscore || ctxVars.__loopguard_deltaSim || 0
                    );
                    ...
                    try {
                      ctxLocal.vars.__drag_score = drag;
                    } catch {}
                    ...
                    if (drag >= thresh) {
                      ...
                      try {
                        METRICS.inc('scene_conclusion_total', { path: 'message' });
                      } catch {}
                      try {
                        logAt(
                          'info',
                          '[memory.conclude]',
                          JSON.stringify({ drag, next: nextHint || null, conv_id: ctxVars.conv_id })
                        );
                      } catch {}
                    }
                  }
                ...
                try {
                  ctxLocal.vars.tokens_in = TokenCounter.estimate(text, { model });
                } catch {}
                // Tension: compute and store in context prior to provider call
                try {
                  if (tensionEnabled && tensionEnabled()) {
                    const { tension, beat } =
                      updateTension(String(cid || ''), String(text || '')) || {};
                    try {
                      if (typeof tension === 'number') ctxLocal.memory.tension = tension;
                    } catch {}
                    try {
                      if (typeof tension === 'number') ctxLocal.vars.tension = tension;
                    } catch {}
                    try {
                      if (typeof beat === 'string') ctxLocal.vars.tension_beat = beat;
                    } catch {}
                    try {
                      METRICS.set(
                        'tension_level',
                        Number((tension || 0).toFixed?.(2) || tension || 0),
                        { path: 'message' }
                      );
                    } catch {}
                  }
                ...
                if (pen?.penaltyHints?.length) {
                  ...
                  try {
                    METRICS.inc('loopguard_phrase_penalty_total', {
                      path: 'message',
                      count: String(pen.penaltyHints.length),
                    });
                  } catch {}
                  try {
                    ctxLocal.io?.events?.emit?.('loopguard.phrase.penalty', {
                      conv_id: cid,
                      hints: pen.penaltyHints,
                    });
                  } catch {}
                }
                ...
                if (cadence?.style) {
                  ...
                  try {
                    METRICS.inc('loopguard_cadence_hint_total', {
                      path: 'message',
                      beat: cadence.beat,
                      style: cadence.style,
                    });
                  ...
                  try {
                    ctxLocal.io?.events?.emit?.('loopguard.cadence', { conv_id: cid, ...cadence });
                  } catch {}
                }
                let outText;
                ...
                try {
                  if (BEAT_STYLE_ENABLED && BEAT_ENABLED) {
                    __beat_style_info = applyBeatStyleBooster({
                      ctx: ctxLocal,
                      conv_id: cid,
                      userText: textInput,
                      currentBeat: getBeat(cid),
                    });
                    if (
                      __beat_style_info &&
                      __beat_style_info.mode === 'memory' &&
                      __beat_style_info.line
                    ) {
                      let addTok = 0;
                      try {
                        addTok =
                          Number(TokenCounter.estimate(__beat_style_info.line, { model })) || 0;
                      } catch {
                        addTok = 0;
                      }
                      memoryInjectTokens += addTok;
                ...
                try {
                  if (String(ctxLocal?.vars?.__failroll_outcome__ || '').trim()) {
                    ...
                    METRICS.inc?.('failure_roll_skipped_total', {
                      ...
                      try {
                        return Math.max(
                          ...
                          Math.min(
                            ...
                            Number(
                              ctxLocal?.vars?.trust ??
                                ctxLocal?.vars?.trust_score ??
                                ctxLocal?.memory?.trustLevel ??
                                0.5
                            )
                          )
                        );
                      ...
                      try {
                        return Math.max(
                          ...
                          Math.min(
                            1,
                            Number(
                              ctxLocal?.vars?.suspicion ?? ctxLocal?.vars?.suspicion_score ?? 0.0
                            )
                          )
                        );
                      } catch {
                    ...
                    const tension = (() => {
                      try {
                        return Math.max(
                          ...
                          Math.min(
                            1,
                            Number(ctxLocal?.vars?.tension ?? ctxLocal?.memory?.tension ?? 0.5)
                          )
                        );
                      } catch {
                        return 0.5;
                    ...
                    if (useNewFR) {
                      ...
                      if (actionTag || intent) {
                        ...
                        const r100 = d100FR(ctxLocal, {
                          convId: String(cid || ''),
                          turn: Number(ctxLocal?.vars?.turn || 0),
                          userText: userTextX,
                        });
                        const threshold = Math.round(prob * 100);
                        const success = r100 > threshold; // fail on roll ≤ threshold
                        const reason =
                          String(process.env.FAILROLL_SSE_VERBOSE || '0') === '1'
                            ? `d100:${r100} vs ${threshold}`
                            : '';
                        const hint = String(
                          buildOutcomeBooster({
                            style: 'meta',
                            success,
                            verb: (actionTag || 'attempt').replace(/_/g, ' '),
                            reason,
                          })
                        );
                      ...
                      roll = assessRiskyAction({
                        text: userTextX,
                        trust,
                        suspicion,
                        tension,
                        style: getRollStyle(cid),
                      });
                    }
                    if (roll && roll.action && String(roll.hint || '').trim()) {
                      ...
                      let addTok = 0;
                      try {
                        addTok = Number(TokenCounter.estimate(hint, { model })) || 0;
                      } catch {
                        addTok = 0;
                      }
                      const budget = Number(
                      ...
                      if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                        ...
                        memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${hint}`;
                        try {
                          ctxLocal.vars.roll_hint = hint;
                        } catch {}
                        try {
                          ctxLocal.vars.roll = {
                            action: String(roll.action || ''),
                        ...
                        try {
                          METRICS.inc('failure_roll_injected_total', {
                            path: 'message',
                            action: String(roll.action || ''),
                          });
                        } catch {}
                        try {
                          broadcastAdminMemoryEvent(cid, 'memory.roll', {
                            conv_id: cid,
                            hint,
                            action: String(roll.action || ''),
                            chance: Number(roll.chance || 0),
                            success: Boolean(roll.success),
                            style: String(getRollStyle(cid) || ''),
                          });
                        } catch {}
                      } else {
                        try {
                ...
                try {
                  ...
                  try {
                    if (__sceneConclusionStaged) {
                      ...
                      try {
                        broadcastAdminMemoryEvent(cid, 'scene.conclusion.applied', {
                          conv_id: cid,
                          ok: true,
                        });
                      } catch {}
                  ...
                  try {
                    ...
                    if (boosterText) {
                      ...
                      const agent2 = String(
                        ctxLocal?.vars?.active_agent || ctxLocal?.vars?.engine_source || 'bot'
                      );
                      stageBooster({
                        convId: cid2,
                      ...
                      try {
                        METRICS.inc('booster_staged_total', { reason: 'restage', path: 'message' });
                      } catch {}
                    }
                  } catch {}
                  throw err;
                ...
                try {
                  ...
                  let lastSim = 0;
                  let styleUsed = null;
                  const decide0 = await loopGuardDecide({
                    convId: cidLG,
                    candidate: outText,
                    prevBotReplies: prevLG,
                    cfg: cfgLG,
                  });
                  lastSim = decide0.sim;
                  // Emit score to local IO channel for optional stream mirroring
                  try {
                    ctxLocal.io?.events?.emit?.('loopguard.score', {
                      conv_id: cidLG,
                      sim: decide0.sim,
                      reason: decide0.reason,
                    });
                  } catch {}
                  if (decide0.shouldReroll) {
                    ...
                    for (let i = 0; i < Math.max(0, cfgLG.retryLimit); i++) {
                      retries = i + 1;
                      const boostedPrefix = decide0.booster
                        ? `${memoryPrefix}\n${decide0.booster}`
                        : memoryPrefix;
                      const alt = await llm.call(textInput, {
                        critical: true,
                        model,
                        memoryPrefix: boostedPrefix,
                      });
                      const decideN = await loopGuardDecide({
                        convId: cidLG,
                        candidate: alt,
                        prevBotReplies: prevLG,
                        cfg: cfgLG,
                    ...
                    try {
                      ...
                      ctxLocal.io?.events?.emit?.('loopguard.trigger', {
                        conv_id: cidLG,
                        sim: Number(lastSim || 0),
                        style: String(styleUsed || ''),
                        retry: retries,
                      });
                      METRICS.inc('loopguard_trigger_total', { path: 'message' });
                      broadcastAdminMemoryEvent(cidLG, 'loopguard.trigger', {
                        conv_id: cidLG,
                        reason: 'delta_sim',
                        sim: Number(lastSim || 0),
                        style: String(styleUsed || ''),
                  ...
                  try {
                    ...
                    if (ecfg.enabled) {
                      const { maxSim, compared } = emapMaxSim({
                        convId: cidLG,
                        candidate: String(outText || ''),
                        cfg: ecfg,
                      });
                      try {
                        ctxLocal.io?.events?.emit?.('loopguard.emap.score', {
                          conv_id: cidLG,
                          sim: maxSim,
                          compared,
                        });
                      ...
                      if (maxSim >= ecfg.simMax) {
                        ...
                        const { maxSim: afterSim } = emapMaxSim({
                          convId: cidLG,
                          candidate: String(alt || ''),
                          cfg: ecfg,
                        });
                        try {
                          ctxLocal.io?.events?.emit?.('loopguard.trigger', {
                          ...
                          METRICS.inc('loopguard_emap_trigger_total', {
                            path: 'message',
                            reason: 'embed_sim',
                            style: String(chosen || ''),
                          });
                    ...
                    try {
                      METRICS.inc('loopguard_emap_errors_total', { path: 'message' });
                    } catch {}
                  }
                  ...
                  try {
                    ...
                    if (bcfg.enabled) {
                      ...
                      try {
                        ctxLocal.io?.events?.emit?.('loopguard.entropy.score', {
                          conv_id: cidLG,
                          score,
                          charH,
                          wordH,
                      ...
                      const min = turbo
                        ? Math.max(bcfg.min, Number(process.env.LOOP_ENTROPY_MIN_TURBO) || 2.6)
                        : bcfg.min;
                      if (String(outText || '').length >= bcfg.minLen && score < min) {
                        const tokensStr = String(
                          cfgLG?.styleTokens || 'descriptive,poetic,terse,inner-thought'
                        );
                        const styleTokens = tokensStr
                          .split(',')
                          .map((s) => s.trim())
                        ...
                        const boostedPrefix = `${memoryPrefix}\n${booster}`;
                        const reroll = await llm.call(textInput, {
                          critical: true,
                          model,
                          memoryPrefix: boostedPrefix,
                        });
                        ...
                        try {
                          ...
                          METRICS.inc?.('loopguard_entropy_trigger_total', {
                            path: 'message',
                            reason: 'low_entropy',
                            style: String(chosen || ''),
                            turbo: String(turbo),
                    ...
                    try {
                      METRICS.inc?.('loopguard_entropy_errors_total', { path: 'message' });
                    } catch {}
                  }
                  try {
                    LOOP_HISTORY.record(cidLG, String(outText || ''), cfgLG.historyN);
                  } catch {}
                  try {
                ...
                try {
                  const beatState = (() => {
                    try {
                      return String(ctxLocal?.vars?.beat_state || getBeat(cid)?.state || 'lull');
                    } catch {
                      return 'lull';
                    }
                  })();
                  ...
                  try {
                    broadcastAdminStyleEvent(cid, 'cadence.observed', {
                      conv_id: cid,
                      beat: beatState,
                      target,
                      observed,
                  ...
                  if (LOOP_CADENCE_ENFORCE && obs && obs.absDelta > CADENCE_TOLERANCE_WORDS) {
                    ...
                    if (retries < LOOP_CADENCE_RETRY_LIMIT) {
                      try {
                        METRICS.inc('cadence_reroll_total', {
                          reason: obs.delta < 0 ? 'too_short' : 'too_long',
                          beat: String(beatState || 'unknown'),
                          path: 'message',
                        });
                      } catch {}
                      ctxLocal.vars = ctxLocal.vars || {};
                      ...
                      if (CADENCE_STRICT_PREFIX_MODE === 'text') {
                        ...
                        let addTok = 0;
                        try {
                          addTok = Number(TokenCounter.estimate(strictLine, { model })) || 0;
                        } catch {
                          addTok = 0;
                        }
                        memoryInjectTokens += addTok;
                        ...
                        try {
                          ctxLocal.vars.__memory_prefix = boostedPrefix;
                        } catch {}
                        try {
                          METRICS.inc('memory_inject_tokens_total', {
                            tokens: String(addTok),
                      ...
                      const rr = await llm.call(rerollInput, {
                        critical: true,
                        model,
                        memoryPrefix: boostedPrefix,
                      });
                      ...
                      if (rrDelta <= obs.absDelta) {
                        try {
                          METRICS.inc('cadence_reroll_win_total', {
                            beat: String(beatState || 'unknown'),
                            path: 'message',
                          });
                        } catch {}
                        ...
                        try {
                          METRICS.inc('cadence_reroll_lose_total', {
                            beat: String(beatState || 'unknown'),
                            path: 'message',
                          });
                        } catch {}
                  ...
                  try {
                    METRICS.inc('cadence_enforce_errors_total', { path: 'message' });
                  } catch {}
                }
                ...
                try {
                  ...
                  if (COOL_REROLL_ON) {
                    const styleTokensStr = String(
                      getLoopGuardConfig()?.styleTokens || 'descriptive,poetic,terse,inner-thought'
                    );
                    const styleTokens = styleTokensStr
                      .split(',')
                      .map((s) => s.trim())
                      .filter(Boolean);
                  ...
                  try {
                    METRICS.inc('loopguard_cooldown_reroll_errors_total', { path: 'message' });
                  } catch {}
                }
                ...
                try {
                  ...
                  }
                  {
                    ...
                    const cadBotEff = {
                      ...cadBot,
                      enabled: ultraFeatureEnabled(cid, cadBot.enabled),
                    };
                ...
                try {
                  recordFinal(String(cid || ''), String(outText || ''));
                  METRICS.inc('loop_phrase_seen_total', { path: 'message' });
                } catch {}
                // Phrase Decay (pattern-based): update hot phrase tracker
                try {
                  const { hot } =
                    PhraseDecay.update(String(cid || ''), String(outText || '')) || {};
                  if (Array.isArray(hot) && hot.length) {
                    try {
                      METRICS.inc('loop_phrase_hot_total', {
                        path: 'message',
                        count: String(hot.length),
                      });
                    } catch {}
                    try {
                      ctxLocal.io?.events?.emit?.('loopguard.phrase.hot', {
                        conv_id: cid,
                        count: hot.length,
                      });
                    } catch {}
                ...
                try {
                  ...
                  const snap =
                    updateBeat(cidBeat, {
                      botText: String(outText || ''),
                      userText: String(textForModel || ''),
                    }) || null;
                  if (snap && snap.state) {
                    try {
                      res.locals = res.locals || {};
                      res.locals.__beat = snap;
                    } catch {}
                    try {
                      METRICS.inc('beat_ticks_total', {
                        path: 'message',
                        beat: String(snap.state || ''),
                      });
                    } catch {}
                    try {
                      METRICS.inc('scene_beat_state_total', {
                    ...
                    try {
                      ...
                      if (prev && prev !== snap.state)
                        METRICS.inc('scene_beat_switch_total', {
                          from: String(prev || ''),
                          to: String(snap.state || ''),
                        });
                ...
                try {
                  ctxLocal.vars.tokens_out = TokenCounter.estimate(outText, {
                    model: String(ctxLocal.vars.__selected_model || model || ''),
                  });
                } catch {}
                const assistant = createMessage({
                  role: 'assistant',
                ...
                try {
                  ...
                  if (hedgeTriggered) {
                    try {
                      METRICS.inc('llm_hedge_switch_total', {
                        from: primary,
                        to: used,
                        model,
                        source: engineSource,
                  ...
                  try {
                    ...
                    try {
                      METRICS.inc('memory_label_calls_total', {
                        path: 'message',
                        type: String(post?.label_type || 'none'),
                      });
                      for (let i = 0; i < Number(post?.ef_kept || 0); i++) {
                        METRICS.inc('memory_store_kept_total', { path: 'message' });
                      }
                      for (let i = 0; i < Number(post?.ef_pruned || 0); i++) {
                        METRICS.inc('memory_store_pruned_total', { path: 'message' });
                      }
                    } catch {}
                  } catch {}
                  ...
                  try {
                    if (
                      String(process.env.MEMORY_SHAPING_ENABLED || '1') !== '0' &&
                      canShapeNow(cid)
                    ) {
                      ...
                      try {
                        METRICS.inc('facts_shaped_total', {
                          merged: shaped?.merged || 0,
                          dropped: shaped?.dropped || 0,
                        });
                      } catch {}
                    }
                    ...
                    if (typeof upsertWorldState === 'function' && ctxLocal?.vars?.__world_delta) {
                      ...
                      try {
                        METRICS.inc('world_state_updates_total');
                      } catch {}
                    }
                  } catch {}
                  try {
                    scheduleFactsConsolidation(cid, Number(input?.turn || 0));
                  } catch {}
                } catch {}
                return {
                  ok: true,
                  reply: assistant,
                  model,
                  provider: String(ctxLocal.vars.__selected_provider || ''),
                  provider_primary: String(
                    ctxLocal.vars.__primary_provider ||
                      String(ctxLocal.vars.__selected_provider || '')
                  ),
                  provider_used: String(
                    ctxLocal.vars.__used_provider || String(ctxLocal.vars.__selected_provider || '')
                  ),
                  resolved_model: String(ctxLocal.vars.__selected_model || ''),
                  variant_v: ctxLocal.vars.abVariant,
                  engine_source: engineSource,
                };
              },
              limits: {
                max: Math.max(1, Number(process.env.CONV_RATE_MAX || 20)),
                windowCounts: Math.max(1, Number(process.env.CONV_RATE_WINDOW || 40)),
              },
              hooks: {},
            });
            ...
            try {
              ...
              const textForEngine = Array.isArray(userMsg?.content)
                ? userMsg.content.join('\n')
                : String(userMsg?.content || '');
              ...
              const heuristicsDisabledPre = String(
                process.env.LLM_HEURISTICS_DISABLED || ''
              ).toLowerCase();
              if (!engineCandidatePre) {
                engineSourcePre =
                  heuristicsDisabledPre === '1' || heuristicsDisabledPre === 'true'
                    ? 'default'
                    : 'heuristic';
              ...
              if (sd) {
                try {
                  METRICS.inc('rate_limited_total', { reason: 'soft_drop' });
                } catch {}
                emitSoftDropJitterHistogram(sd.jitterMs, {
                  reason: sd.reason,
                  source: engineSourcePre,
                  path: 'message',
                });
                const raSec = Math.max(1, Math.round(sd.jitterMs / 1000));
                res.writeHead(503, {
                  'Content-Type': 'application/json',
                  'Retry-After': String(raSec),
                });
                res.end(
                  JSON.stringify({
                    error: 'soft_drop',
                    reason: sd.reason,
                    retry_after_ms: sd.jitterMs,
                  })
                );
                return;
              }
            ...
            try {
              ...
              if (SHADOW) {
                const ing = await shadowIngest({
                  ...
                  turn: Number(body?.turn || 0),
                  role: 'user',
                  text: Array.isArray(userMsg?.content)
                    ? userMsg.content.join('\n')
                    : String(userMsg?.content || ''),
                ...
                try {
                  for (const f of ing?.factsNew || [])
                    METRICS.inc('shadow_facts_total', { type: String(f.type || 'unknown') });
                } catch {}
              }
            } catch {}
            ...
            try {
              ...
              if (SHADOW) {
                ...
                // ingest bot turn
                {
                  ...
                  try {
                    for (const f of ing2?.factsNew || [])
                      METRICS.inc('shadow_facts_total', { type: String(f.type || 'unknown') });
                  } catch {}
                }
                // detect mismatches and optionally generate nudges
                const { mismatches } = await shadowDetect({
                  convId: cid,
                ...
                if (mismatches?.length) {
                  for (const m of mismatches)
                    try {
                      METRICS.inc('shadow_mismatch_total', {
                        type: m.type,
                        severity: String(m.score || 0),
                      });
                    } catch {}
                  const mode = String(process.env.SHADOW_NUDGE_MODE || 'emit'); // emit|inject|none
                  if (mode !== 'none') {
                    const pov = String(process.env.FACETS_WHO || 'they');
                    const nudges = await shadowNudgeFor({
                      convId: cid,
                      pov,
                      limit: Number(process.env.SHADOW_NUDGE_LIMIT || 2),
                    });
                    try {
                      await shadowStashNudges({ convId: cid, nudges });
                    } catch {}
                    if (mode === 'inject') {
                      ...
                      try {
                        METRICS.inc('shadow_nudge_injected_total', { mode: 'inject' });
                      } catch {}
                    } else {
                      // emit out-of-band advisory for clients/recap
                      try {
                        res.setHeader('X-Shadow-Nudge', '1');
                      } catch {}
                      try {
                        res.locals = res.locals || {};
                        res.locals.__shadow_nudge = nudges;
                      } catch {}
                      try {
                        METRICS.inc('shadow_nudge_injected_total', { mode: 'emit' });
                      } catch {}
                    }
                  }
                }
            ...
            const textForSignals = Array.isArray(userMsg.content)
              ? userMsg.content.join('\n')
              : String(userMsg.content || '');
            ...
            try {
              ...
              if (JUDGE) {
                ...
                try {
                  if (Array.isArray(out?.reply?.content))
                    replyTextForJudge = out.reply.content.join('\n');
                ...
                const { axes, overall } = await judgeContinuity({
                  convId: cidLocal,
                  replyText: String(replyTextForJudge || ''),
                  facetsTopK,
                  weights,
                });
                try {
                  METRICS.set('continuity_score', Number((overall * 100).toFixed(1)), {
                    path: 'message',
                  });
                ...
                try {
                  const GUARD = String(process.env.GUARD_ENABLED || '1') === '1';
                  const guardThresh = Number(
                    process.env.GUARD_SET_THRESHOLD || process.env.JUDGE_LOW_THRESHOLD || 0.6
                  );
                  if (GUARD && overall < guardThresh && cidLocal) {
                    const pov = String(process.env.FACETS_WHO || 'she');
                    const maxChars = Math.max(
                      60,
                      Number(process.env.GUARD_HINT_MAX_CHARS || process.env.GUARD_MAX_CHARS || 180)
                    );
                    const ttlTurns = Math.max(
                      1,
                      Number(process.env.GUARD_HINT_TTL_TURNS || process.env.GUARD_TTL_TURNS || 2)
                    ...
                    if (line) {
                      try {
                        setGuardHint(cidLocal, String(line), { ttlTurns });
                      } catch {}
                      try {
                        METRICS.inc('continuity_guard_set_total', { path: 'message' });
                      } catch {}
                      try {
                        METRICS.inc('guard_hint_stored_total', {
                          path: 'message',
                          reason: 'continuity_low',
                        });
                      } catch {}
                    }
                  }
                ...
                try {
                  res.locals = res.locals || {};
                  res.locals.__continuity = { axes, overall };
                } catch {}
              }
            } catch {}
            ...
            try {
              ...
              try {
                if (Array.isArray(out?.reply?.content))
                  replyTextFinal = out.reply.content.join('\n');
                else replyTextFinal = String(out?.reply?.content || out?.reply || '');
              } catch {}
              try {
                indexTurn({ convId: cidIndex, role: 'bot', text: String(replyTextFinal || '') });
              } catch {}
              // Watchdog: log assistant turn (message path)
              try {
                await logTurn(cidIndex, 'assistant', String(replyTextFinal || ''));
              } catch {}
            } catch {}
              ...
              if (cidReinf) {
                const changed =
                  Array.isArray(ids) && ids.length
                    ? reinforceFacts(cidReinf, ids, undefined, turnIndex)
                    : [];
                const decayed = decayFacts(cidReinf, turnIndex);
                try {
                  if (changed.length) pushFactsUpdatedSSE(res, cidReinf, changed);
                } catch {}
                try {
                  if (req?.sseResponse)
                    emitMemoryShapeSSE(req.sseResponse, {
                      conv_id: cidReinf,
                      reinforced: changed?.length || 0,
                      decayed,
                      turn: turnIndex,
                    });
                } catch {}
                try {
                  METRICS.inc('memory_shape_total', { path: 'message' });
                } catch {}
                try {
                  scheduleFactsConsolidation(cidReinf, turnIndex);
                } catch {}
              }
            } catch {}
            ...
            try {
              ...
              if (cidBeatResp) {
                __beat_snapshot = getBeat(cidBeatResp) || null;
              }
            } catch {}
            const guardHintAudit = String(ctx?.vars?.__guard_hint || '').trim();
            const response = {
              ...out,
              hash: reqHash,
              bytes_b64: reqBytes.toString('base64'),
              ...
              memory_applied: Boolean(
                String(ctx?.vars?.__memory_prefix || '') &&
                  Number(ctx?.vars?.__memory_injected_tokens || 0) > 0
              ),
              ...
              ...(typeof ctx?.vars?.tension === 'number' || typeof ctx?.memory?.tension === 'number'
                ? { tension: Number(ctx?.vars?.tension ?? ctx?.memory?.tension ?? 0) }
                : {}),
              ...(typeof ctx?.vars?.tension_beat === 'string'
                ? { tension_beat: String(ctx?.vars?.tension_beat || '') }
                : {}),
              ...(typeof __beat_snapshot?.state === 'string'
                ? { beat: String(__beat_snapshot.state || '') }
                : {}),
              ...(typeof __beat_snapshot?.tension === 'number'
                ? { beat_tension: Number(__beat_snapshot.tension || 0) }
                : {}),
            ...
            if (idempotencyKey) {
              ...
              try {
                await saveIdemToDisk(idempotencyKey, response, 0);
              } catch {}
              try {
                await idemSetRedis(idempotencyKey, response, 0);
              } catch {}
              // Cleanup expired entries opportunistically
              try {
                for (const [k, v] of Array.from(IDEMPOTENCY_CACHE.entries())) {
                  if (Date.now() - v.ts > IDEMPOTENCY_TTL_MS + IDEMPOTENCY_SKEW_MS)
                    IDEMPOTENCY_CACHE.delete(k);
                }
                await gcIdemDir();
              } catch {}
              ...
              try {
                ...
                if (!alreadyMsg) {
                  await markToolExecuted(toolCallIdMsg, {
                    tenant: String(response?.tenant || ''),
                    tool: 'conv.message',
                  });
                }
              } catch {}
              try {
                if (claimedIdemLock) await idemReleaseLock(idempotencyKey);
              } catch {}
            }
            res.writeHead(200, { 'Content-Type': 'application/json' });
            try {
              ...
              if (dbg === '1' || dbg === 'true') {
                console.info(
                  JSON.stringify({ evt: 'message_response_debug', path: 'message', response })
                );
              }
            } catch {}
            try {
              const payload = JSON.stringify(response);
              await sendMessageWithTick(async (p) => {
                res.end(p);
                return true;
              }, payload);
            } catch {
    ...
    if (
      (req.url?.startsWith('/conv/ultra') || req.url?.startsWith('/v1/conv/ultra')) &&
      String(req.method || 'GET').toUpperCase() === 'GET'
    ) {
      try {
        ...
        try {
          ...
          if (corsList.length > 0 && isProd && !corsList.includes(origin)) {
            ...
            try {
              METRICS.inc('responses_total', { status: '403' });
              METRICS.inc('auth_blocked_total', { reason: 'cors', path: 'ultra:get' });
              span?.setAttribute?.('http.status_code', 403);
            } catch {}
            return;
          }
        ...
        try {
          ...
          if (criticEnabled) {
            const beliefsHit = Boolean(
              ctx?.vars?.__beliefs_conflict ||
                ctx?.vars?.__contradiction_flag ||
                ctx?.vars?.__critic_block
            );
            const critic = await constraintCritic(
            ...
            if (critic?.violated) {
              try {
                ctx.vars.__critic_block = true;
              } catch {}
              try {
                ctx.vars.__critic_reason = critic.reason;
              } catch {}
              try {
                ctx.vars.__critic_class = critic.class;
              } catch {}
              ...
              if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                ...
                memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}(${refusalCue})`;
                try {
                  ctx.vars.__memory_prefix = memoryPrefix;
                } catch {}
                try {
                  ctx.vars.__memory_injected_tokens = memoryInjectTokens;
                } catch {}
                try {
                  METRICS.inc('memory_inject_tokens_total', {
                    tokens: String(addTok),
              ...
              try {
                ...
                res.write(
                  `data: ${JSON.stringify({ class: String(critic.class || 'world'), reason: String(critic.reason || ''), via: String(critic.via || 'rules') })}\n\n`
                );
              } catch {}
              ...
              try {
                ...
                if (
                  ...
                  try {
                    ...
                    res.write(
                      `data: ${JSON.stringify({ refused: true, reason: String(critic.reason || ''), model: 'refusal', provider: 'local', resolved_model: 'refusal' })}\n\n`
                    );
                  } catch {}
                  try {
                  ...
                  try {
                    res.write(`event: end\n`);
                    res.write(
                      `data: ${JSON.stringify({ final: refusalText, refused: true, reason: String(critic.reason || '') })}\n\n`
                    );
                  } catch {}
                  try {
                    await sendMessageWithTick(async () => {
        ...
        try {
          ...
          const hasToken = token.length > 0 && (tokenFromHdr === token || tokenFromQuery === token);
          const reqTs = Number(
            u.searchParams.get('ts') || Number(req.headers['x-request-ts'] || 0)
          );
          const replayWinMs = Math.max(0, Number(process.env.REPLAY_WINDOW_MS || 0));
          const skewToleranceMs = Math.max(100, Number(process.env.REPLAY_SKEW_TOLERANCE_MS || 0));
          if (isProd && replayWinMs > 0) {
          ...
          try {
            if (hasToken)
              METRICS.inc('auth_accepted_total', { method: 'token', path: 'ultra:get' });
            else if (macOk)
              METRICS.inc('auth_accepted_total', {
                method: 'hmac',
                path: 'ultra:get',
    ...
    if (
      (req.url?.startsWith('/conv/beliefs') || req.url?.startsWith('/v1/conv/beliefs')) &&
      String(req.method || 'GET').toUpperCase() === 'GET'
    ) {
      try {
        // Strict CORS in production
        try {
          ...
          if (corsList.length > 0 && isProd && !corsList.includes(origin)) {
            ...
            try {
              METRICS.inc('responses_total', { status: '403' });
              METRICS.inc('auth_blocked_total', { reason: 'cors', path: 'beliefs:get' });
              span?.setAttribute?.('http.status_code', 403);
            } catch {}
            return;
          }
        ...
        try {
          ...
          try {
            if (hasToken)
              METRICS.inc('auth_accepted_total', { method: 'token', path: 'beliefs:get' });
            else if (macOk)
              METRICS.inc('auth_accepted_total', {
                method: 'hmac',
                path: 'beliefs:get',
      ...
      try {
        ...
        try {
          ...
          if (corsList.length > 0 && isProd && !corsList.includes(origin)) {
            ...
            try {
              METRICS.inc('responses_total', { status: '403' });
              METRICS.inc('auth_blocked_total', { reason: 'cors', path: 'beliefs:post' });
              span?.setAttribute?.('http.status_code', 403);
            } catch {}
            return;
          }
        ...
        req.on('end', async () => {
          try {
            ...
            try {
              ...
              try {
                if (hasToken)
                  METRICS.inc('auth_accepted_total', { method: 'token', path: 'beliefs:post' });
                else if (macOk)
                  METRICS.inc('auth_accepted_total', {
                    method: 'hmac',
                    path: 'beliefs:post',
            ...
            if (op === 'add') {
              ...
              try {
                if (item) METRICS.inc('belief_added_total', { path: 'beliefs:post', char_id });
              } catch {}
            } else if (op === 'delete') {
              ...
              result = { deleted: !!ok };
              try {
                if (ok) METRICS.inc('belief_deleted_total', { path: 'beliefs:post', char_id });
              } catch {}
            } else {
              res.writeHead(400, { 'Content-Type': 'application/json' });
            ...
            res.writeHead(500, { 'Content-Type': 'application/json' });
            res.end(
              JSON.stringify({ error: 'beliefs_post_failed', msg: String((e && e.message) || e) })
            );
            try {
              METRICS.inc('responses_total', { status: '500' });
            } catch {}
      ...
      try {
        ...
        try {
          ...
          if (corsList.length > 0 && isProd && !corsList.includes(origin)) {
            ...
            try {
              METRICS.inc('responses_total', { status: '403' });
              METRICS.inc('auth_blocked_total', { reason: 'cors', path: 'ultra:post' });
              span?.setAttribute?.('http.status_code', 403);
            } catch {}
            return;
          }
        ...
        req.on('end', async () => {
          try {
            ...
            try {
              ...
              try {
                if (hasToken)
                  METRICS.inc('auth_accepted_total', { method: 'token', path: 'ultra:post' });
                else if (macOk)
                  METRICS.inc('auth_accepted_total', {
                    method: 'hmac',
                    path: 'ultra:post',
            ...
            res.writeHead(500, { 'Content-Type': 'application/json' });
            res.end(
              JSON.stringify({ error: 'ultra_post_failed', msg: String((e && e.message) || e) })
            );
            try {
              METRICS.inc('responses_total', { status: '500' });
            } catch {}
    ...
    if (
      (req.url?.startsWith('/conv/stream') || req.url?.startsWith('/v1/conv/stream')) &&
      String(req.method || 'GET').toUpperCase() === 'GET'
    ) {
      try {
        // Strict CORS: allow only configured origins in production
        try {
          ...
          if (corsList.length > 0 && isProd && !corsList.includes(origin)) {
            ...
            try {
              METRICS.inc('responses_total', { status: '403' });
              METRICS.inc('auth_blocked_total', { reason: 'cors', path: 'stream' });
              span?.setAttribute?.('http.status_code', 403);
            } catch {}
            return;
          }
        ...
        try {
          ...
          try {
            if (hasToken) METRICS.inc('auth_accepted_total', { method: 'token', path: 'stream' });
              ...
              METRICS.inc('auth_accepted_total', { method: 'hmac', path: 'stream', mac_id: macId });
          } catch {}
        } catch {}
        // Transcript: index user message early (raw text)
        try {
          indexTurn({ convId: String(conv_id || 'conv'), role: 'user', text });
        } catch {}
        // Watchdog: log user turn (stream path)
        try {
          await logTurn(String(conv_id || 'conv'), 'user', String(text || ''));
        } catch {}
        // Scene chat commands (optional): !scene tag <name>, !scene goto <name>
        try {
          trySceneChatCommand(conv_id, text);
        } catch {}
          ...
          if (!isProbe && !isControl && Number.isFinite(TENANT_LIMIT) && TENANT_LIMIT > 0) {
            ...
            if (!out?.ok) {
              ...
              if (out.internal_error && tenantInternalOnce) {
                ...
                try {
                  METRICS.inc('rate_limited_total', { reason: 'internal_error', scope: 'tenant' });
                  METRICS.inc('responses_total', { status: '503' });
                } catch {}
                return;
              }
        ...
        try {
          ...
          if (secret && idempotencyKey) {
            ...
            if (!macHdr) {
              ...
              try {
                METRICS.inc('responses_total', { status: '401' });
                METRICS.inc('idempotency_mac_missing_total', { path: 'stream' });
                span?.setAttribute?.('http.status_code', 401);
              } catch {}
              return;
            }
            ...
            if (!ok) {
              ...
              try {
                METRICS.inc('responses_total', { status: '401' });
                METRICS.inc('idempotency_mac_invalid_total', { path: 'stream' });
                span?.setAttribute?.('http.status_code', 401);
              } catch {}
              return;
            }
        ...
        const wantsReplay = ['1', 'true', 'yes'].includes(
          String(
            u.searchParams.get('reconnect') ||
              u.searchParams.get('replay') ||
              req.headers['x-reconnect'] ||
              ''
          ).toLowerCase()
        );
        try {
        ...
        if (streamKey) {
          ...
          if (active && Date.now() - active.started < IDEMPOTENCY_TTL_MS + IDEMPOTENCY_SKEW_MS) {
            ...
            try {
              sampled('warn', { evt: 'duplicate_stream_active', path: 'stream' });
            } catch {}
            return;
          }
          let cached =
            touchLRU(IDEMPOTENCY_CACHE, streamKey) ||
            (idempotencyKey ? touchLRU(IDEMPOTENCY_CACHE, idempotencyKey) : null);
          if (!cached) {
            try {
              cached =
                (await loadIdemFromDisk(streamKey)) ||
                (idempotencyKey ? await loadIdemFromDisk(idempotencyKey) : null);
            } catch {}
          }
          if (!cached) {
            try {
              cached =
                (await idemGetRedis(streamKey)) ||
                (idempotencyKey ? await idemGetRedis(idempotencyKey) : null);
            } catch {}
          }
          ...
          if (cached && Date.now() - cached.ts < IDEMPOTENCY_TTL_MS + IDEMPOTENCY_SKEW_MS) {
            ...
            if (wantsReplay && Number(cached.replayCount || 0) >= 1) {
              ...
              try {
                METRICS.inc('responses_total', { status: '409' });
                METRICS.inc('idempotent_replay_denied_total', {
                  reason: 'exhausted',
                  path: 'stream',
                });
              } catch {}
              try {
                sampled('warn', { evt: 'replay_unavailable', path: 'stream', reason: 'exhausted' });
              } catch {}
              return;
            }
            ...
            try {
              const ridStore = ALS?.getStore?.() || {};
              const ridVal =
                String(ridStore?.rid || req.headers['x-request-id'] || '').trim() ||
                String(rid || '');
              let __tension = 0;
              let __tensionBeat = '';
              try {
              ...
              try {
                if (__style_meta)
                  broadcastAdminStyleEvent(conv_id, 'style.pref', {
                    conv_id,
                    style_meta: __style_meta,
                  });
              } catch {}
              const __style_field = __style_meta
                ? {
                    style: {
                      preset: String(__style_meta.preset || ''),
                      token_count: Array.isArray(__style_meta.tokens)
                        ? __style_meta.tokens.length
                        : 0,
                    },
                  }
                : {};
              ...
              try {
                maybeTagCooledPhrases(
                  ctx,
                  String(conv_id || ''),
                  String(cached?.response?.final || '')
                );
              } catch {}
              res.write(
            ...
            try {
              sampled('info', { evt: 'idem_replay_served', path: 'stream', source: 'replay' });
            } catch {}
            ...
            try {
              if (wantsReplay) {
                ...
                if (k) {
                  IDEMPOTENCY_CACHE.delete(k);
                  IDEMPOTENCY_CACHE.set(k, cached);
                }
                pruneCaches();
                try {
          ...
          try {
            if (!cached && !wantsReplay) {
              ...
              if (!claimed) {
                ...
                try {
                  sampled('warn', { evt: 'duplicate_stream_lock_denied', path: 'stream' });
                } catch {}
                return;
              }
        ...
        if (engineCandidate) {
          ...
          engineSource =
            heuristicsDisabled === '1' || heuristicsDisabled === 'true' ? 'default' : 'heuristic';
          if (engineSource === 'heuristic') {
            model = /echo|gods|pantheon/i.test(text)
              ? 'echo'
              : /dream|sleep|night|hallucin|lucid/i.test(text)
          ...
          if (!recap || recap.length < 40) {
            try {
              ...
              const windowText = Array.isArray(msgs)
                ...
                : '';
              const ctxForBooster = {
                vars: { path: 'stream', purpose: 'booster' },
                io: { events: new EventEmitter() },
              };
          ...
          if (recap && recap.length > 0) {
            ...
            try {
              stageBooster({
                ...
                ttlTurns: ttl,
                agent: 'bot',
                source: recap.length >= 40 ? 'heur' : 'llm_or_heur',
              });
              __booster_staged_id = id;
              __booster_staged_text = recap;
              try {
                METRICS.inc('booster_staged_total', { reason: 'auto', path: 'stream' });
              } catch {}
            } catch {}
          }
        } catch {}
        try {
          ...
          try {
            ...
            activeAgent =
              String(
                uAA.searchParams.get('agent') || uAA.searchParams.get('active_agent') || ''
              ).trim() || 'bot';
          } catch {}
          ...
          if (boost) {
            ...
            try {
              METRICS.inc('booster_used_total', { path: 'stream' });
            } catch {}
          }
        } catch {}
        ...
        try {
          ...
          if (
            ...
            if (!__booster_text) {
              ...
              try {
                METRICS.inc('dreams_injected_total', { path: 'stream' });
              } catch {}
            } else {
              try {
                METRICS.inc('dreams_injected_total', {
                  path: 'stream',
                  skipped: 'booster_present',
                });
              } catch {}
            }
          }
          ...
          if (enabled) {
            ...
            const threshold = Math.max(
              ...
              Math.min(
                1,
                Number(
                  process.env.JAILBREAK_REFUSAL_THRESHOLD || process.env.REFUSAL_THRESHOLD || 0.7
                )
              )
            );
            const jb = Number(signals?.jailbreak_signal || 0);
            ...
            if (moderatelyRisky) {
              ...
              try {
                METRICS.inc('refusal_staged_total', {
                  path: 'stream',
                  style: styleR,
                  level: __refusal_hint_level,
                });
              } catch {}
              try {
                broadcastAdminRefusalEvent(String(conv_id || ''), 'refusal.staged', {
                  conv_id: String(conv_id || ''),
                  text: __refusal_hint_text,
                  style: styleR,
                  level: __refusal_hint_level,
                  reason: __refusal_hint_reason,
                });
              } catch {}
            }
          }
        ...
        try {
          if (tensionEnabled && tensionEnabled()) {
            ...
            res.write(
              `data: ${JSON.stringify({ conv_id: String(conv_id || ''), tension: t, beat: b })}\n\n`
            );
            try {
              METRICS.inc('memory_tension_emitted_total', { path: 'stream' });
            } catch {}
            ...
            try {
              ...
              try {
                METRICS.inc('memory_beat_emitted_total', { path: 'stream' });
              } catch {}
            } catch {}
          }
        } catch {}
        ...
        try {
          if (Number.isFinite(SSE_HEARTBEAT_MS) && SSE_HEARTBEAT_MS > 0) {
            __hb = setInterval(() => {
              try {
                res.write(':keepalive\n\n');
                METRICS.inc('sse_heartbeat_total');
              } catch {}
            }, SSE_HEARTBEAT_MS);
          }
        } catch {}
            ...
            try {
              if (!gotFirstDelta && __booster_text) {
                ...
                try {
                  ...
                  try {
                    METRICS.inc('booster_staged_total', { reason: 'restage', path: 'stream' });
                  } catch {}
                } catch {}
              }
            } catch {}
        ...
        const ctx = {
          ...
          flags: {},
          io: { events: new EventEmitter() },
          memory: { toolResultsCache: new Map() },
        };
        ...
        try {
          ctx.vars.engine_source = engineSource;
        } catch {}
        // Propagate mac_id for lineage into metrics/ledger
        try {
          const macId = String(req.headers['x-mac-id'] || '').trim();
          if (macId) ctx.vars.mac_id = macId;
        } catch {}
        // Propagate request_id, path and conv_id for usage ledger context
        try {
          const ridStore = ALS?.getStore?.() || {};
        ...
        try {
          ctx.vars.path = 'stream';
        } catch {}
        try {
          if (conv_id) ctx.vars.conv_id = conv_id;
        } catch {}
        ...
        // Bind A/B variant stickiness by conv_id with strict precedence and persistence
        const variantExplicit = String(
          u.searchParams.get('ab_variant') || req.headers['x-ab-variant'] || ''
        ).trim();
        ...
        if (conv_id) {
          if (!variant) {
            ...
            if (!existing) {
              try {
                existing = await loadAbVariant(conv_id);
              } catch {}
            }
            if (existing) {
              variant = existing;
            ...
            if (!prior || prior !== variant) {
              ...
              try {
                await saveAbVariant(conv_id, variant);
              } catch {}
            }
          }
        }
        ...
        const toolCallId = String(
          idempotencyKey || (conv_id ? `${conv_id}:${turn}` : `rid:${Date.now()}`)
        );
        ctx.vars.toolCallId = toolCallId;
        try {
          ctx.vars.toolExecutedPrior = await hasToolExecuted(toolCallId);
        } catch {
          ctx.vars.toolExecutedPrior = false;
        }
        ...
        try {
          if (tensionEnabled && tensionEnabled()) {
            const { tension, beat } =
              updateTension(String(conv_id || ''), String(text || '')) || {};
            try {
              if (typeof tension === 'number') ctx.memory.tension = tension;
            } catch {}
            try {
              if (typeof tension === 'number') ctx.vars.tension = tension;
            } catch {}
            try {
              if (typeof beat === 'string') ctx.vars.tension_beat = beat;
            } catch {}
            try {
              METRICS.set('tension_level', Number((tension || 0).toFixed?.(2) || tension || 0), {
                path: 'stream',
              });
            } catch {}
          }
        ...
        try {
          if (String(process.env.SPINE_ENABLED || '1') === '1') {
            ...
            try {
              applySpineToLoopGuard(ctx, sp);
            } catch {}
            // Stash refusal likelihood and broadcast admin spine.update
            try {
              ctx.vars.spine_refusal = Number(sp?.refusalLikelihood || 0);
            } catch {}
            try {
              ...
              if (convId) {
                const trust = Math.max(
                  ...
                  Math.min(
                    1,
                    Number(ctx.stats?.trustMA ?? ctx.stats?.trust ?? ctx.memory?.trustLevel ?? 0.5)
                  )
                );
                const suspicion = Math.max(
                  0,
                  Math.min(1, Number(ctx.stats?.suspicionMA ?? ctx.stats?.suspicion ?? 0.1))
                );
                const spine_meta = {
                  mood: String(sp?.mood || ''),
                  tone: String(sp?.tone || ''),
        ...
        try {
          if (String(process.env.LOOP_STREAM_PROBE_ENABLED || '0') === '1') {
            ...
            const reason = [];
            if (ecfg?.enabled) {
              const { maxSim } = emapMaxSim({
                convId: conv_id,
                candidate: String(preview || ''),
                cfg: ecfg,
              });
              if (Number(maxSim || 0) >= Number(ecfg.simMax || 0)) {
                risky = true;
                reason.push('emap');
              }
            }
            ...
            if (risky) {
              ...
              textInput = `${booster}\n\n${textInput}`;
              try {
                ctx.io?.events?.emit?.('loopguard.stream.primed', {
                  conv_id: String(conv_id),
                  reason,
                  style: chosen,
                });
              ...
              try {
                METRICS.inc('loopguard_stream_primed_total', {
                  path: 'stream',
                  style: chosen,
          ...
          try {
            METRICS.inc('loopguard_stream_probe_errors_total', { path: 'stream' });
          } catch {}
        }
          ...
          if (enabled) {
              ...
              (signalsPre &&
                (signalsPre.jailbreak_signal >= 0.7
                  ...
                  : signalsPre.jailbreak_signal >= 0.4
                    ...
                    : 'low'));
            try {
              ctx.vars.__abuse_signals_pre = signalsPre;
            } catch {}
            if (signalsPre.jailbreak_signal >= threshold || jbLevelPre === 'high') {
              ...
              try {
                ...
                if (__style_meta) {
                  const tokenCount = Array.isArray(__style_meta.tokens)
                    ? __style_meta.tokens.length
                    : 0;
                  startPayload.style = {
                    preset: String(__style_meta.preset || ''),
              ...
              try {
                maybeTagCooledPhrases(ctx, String(conv_id || ''), String(refusalMsg || ''));
              } catch {}
              try {
                res.write(
              ...
              try {
                if (streamKey) idemReleaseLock(streamKey).catch(() => {});
              } catch {}
              return;
            }
          }
        ...
        ctx.io.events.on('stream.start', (info) => {
          ...
          try {
            ctx.vars.__stream_start_ms = Date.now();
            ctx.vars.__first_token_recorded = false;
          } catch {}
          ...
          setImmediate(() => {
                  ...
                  if (ctx?.vars?.spine) {
                    ...
                    const trust = Math.max(
                      ...
                      Math.min(
                        1,
                        Number(
                          ctx.stats?.trustMA ?? ctx.stats?.trust ?? ctx.memory?.trustLevel ?? 0.5
                        )
                      )
                    );
                    const suspicion = Math.max(
                    ...
                    const threshold = Number(
                      process.env.SPINE_REFUSAL_THRESHOLD ?? process.env.REFUSAL_THRESHOLD ?? 0.65
                    );
                    if (Number(sp?.refusalLikelihood || 0) >= threshold) {
                      res.write('event: spine.refusal\n');
                      res.write(
                        `data: ${JSON.stringify({ conv_id, refusal_likelihood: Number(sp?.refusalLikelihood || 0), threshold })}\n\n`
                      );
                    }
                  }
                } catch {}
            ...
            try {
              if (__booster_staged_id && __booster_staged_text) {
                ...
                res.write(
                  `data: ${JSON.stringify({ id: __booster_staged_id, text: __booster_staged_text })}\n\n`
                );
              }
            } catch {}
            try {
              if (__refusal_hint_text) {
                ...
                res.write(
                  `data: ${JSON.stringify({ text: __refusal_hint_text, style: __refusal_hint_style, level: __refusal_hint_level, reason: __refusal_hint_reason })}\n\n`
                );
                try {
                  METRICS.inc('refusal_staged_emitted_total', {
                    path: 'stream',
                    style: __refusal_hint_style,
                    level: __refusal_hint_level,
                    reason: __refusal_hint_reason,
                  });
                } catch {}
            ...
            try {
              ...
              if (gh) {
                ...
                try {
                  METRICS.inc('guard_hint_emitted_total', { path: 'stream' });
                } catch {}
              }
            } catch {}
            try {
              const sbt = String(
                ctx?.vars?.__style_booster_text || __style_booster_text || ''
              ).trim();
              if (sbt) {
                ...
                try {
                  estTokens = Number(
                    ctx?.vars?.__style_booster_est_tokens ||
                      TokenCounter.estimate(sbt, { model }) ||
                      0
                  );
                } catch {
                  estTokens = 0;
                ...
                try {
                  tokenBudget = Number(
                    ctx?.vars?.__style_booster_token_budget ||
                      process.env.STYLE_BOOSTER_MAX_TOKENS ||
                      40
                  );
                } catch {}
                res.write('event: memory.style.booster\n');
                res.write(
                  `data: ${JSON.stringify({ text: sbt, preset: sbPreset, estTokens, tokenBudget })}\n\n`
                );
                try {
                  METRICS.inc('style_booster_emitted_total', { path: 'stream' });
                } catch {}
              }
            } catch {}
            ...
            try {
              ...
              if (r && String(r?.action || '').trim()) {
                const payload = {
                  ...
                  style: String(getRollStyle(String(conv_id || '')) || ''),
                };
                res.write('event: memory.roll\n');
                res.write(`data: ${JSON.stringify(payload)}\n\n`);
                try {
                  broadcastAdminMemoryEvent(conv_id, 'memory.roll', { conv_id, ...payload });
                } catch {}
                try {
                  METRICS.inc('memory_roll_emitted_total', { path: 'stream' });
                } catch {}
              }
            } catch {}
            try {
              const items = Array.isArray(ctx?.vars?.__phrase_plan_items)
                ? ctx.vars.__phrase_plan_items
                : [];
            ...
            try {
              ...
              if (hotCount > 0) {
                ...
                try {
                  METRICS.inc('loop_phrase_cooldown_total', {
                    path: 'stream',
                    count: String(hotCount),
                  });
                } catch {}
              }
            } catch {}
            ...
            try {
              ...
              span?.setAttribute?.('llm.resolved_model', resolvedModel);
              span?.setAttribute?.('llm.engine_source', engineSource);
              span?.setAttribute?.('llm.variant_v', String(ctx.vars.abVariant || ''));
            } catch {}
          });
          ...
          try {
            if (__dream_text) {
              ...
              try {
                broadcastAdminMemoryEvent(conv_id, 'memory.dream', { conv_id, text: __dream_text });
              } catch {}
              try {
                METRICS.inc('dreams_emitted_total', { path: 'stream' });
              } catch {}
            }
          } catch {}
        ...
        ctx.io.events.on('stream.delta', (d) => {
          ...
          try {
            ...
            if (
              !hedgeSwitchEmitted &&
              (currentProvider !== providerAtStart || currentModel !== modelAtStart)
            ) {
          ...
          try {
            ...
            try {
              if (
                !ctx.vars.__first_token_recorded &&
                Number.isFinite(Number(ctx.vars.__stream_start_ms || 0))
              ) {
                const ms = Math.max(0, Date.now() - Number(ctx.vars.__stream_start_ms));
                let le = 'gt';
                for (const b of FIRST_TOKEN_MS_BUCKETS) {
                  if (ms <= b) {
                    le = String(b);
                    break;
                  }
        ...
        try {
          ctx.io.events.on('narrative.event', (p) => {
            try {
              res.write('event: narrative.event\n');
              res.write(`data: ${JSON.stringify({ conv_id, ...p })}\n\n`);
            } catch {}
            try {
              broadcastAdminMemoryEvent(conv_id, 'narrative.event', { conv_id, ...p });
            } catch {}
            try {
        ...
        try {
          ctx.io.events.on('loopguard.score', (p) => {
            try {
              res.write('event: loopguard.score\n');
              res.write(`data: ${JSON.stringify(p)}\n\n`);
            } catch {}
          });
          ctx.io.events.on('loopguard.trigger', (p) => {
            try {
              res.write('event: loopguard.trigger\n');
              res.write(`data: ${JSON.stringify(p)}\n\n`);
            } catch {}
          });
          ctx.io.events.on('loopguard.emap.score', (p) => {
            try {
              res.write('event: loopguard.emap.score\n');
              res.write(`data: ${JSON.stringify(p)}\n\n`);
            } catch {}
          });
          ctx.io.events.on('loopguard.entropy.score', (p) => {
            try {
              res.write('event: loopguard.entropy.score\n');
              res.write(`data: ${JSON.stringify(p)}\n\n`);
            } catch {}
          });
          ctx.io.events.on('loopguard.stream.primed', (p) => {
            try {
              res.write('event: loopguard.stream.primed\n');
              res.write(`data: ${JSON.stringify(p)}\n\n`);
            } catch {}
          });
          ctx.io.events.on('loopguard.cooldown', (p) => {
            try {
              res.write('event: loopguard.cooldown\n');
              res.write(`data: ${JSON.stringify(p)}\n\n`);
            } catch {}
          });
          ctx.io.events.on('loopguard.cooldown.hit', (p) => {
            try {
              res.write('event: loopguard.cooldown.hit\n');
              res.write(`data: ${JSON.stringify(p)}\n\n`);
            } catch {}
          });
        } catch {}
        ctx.io.events.on('stream.end', async () => {
          ...
          try {
            try {
              ctx.vars.tokens_out = TokenCounter.estimate(String(final || ''), {
                model: String(ctx.vars.__selected_model || model || ''),
              });
            } catch {}
          } catch {}
          ...
          try {
            recordFinal(String(conv_id || ''), String(final || ''));
            METRICS.inc('loop_phrase_seen_total', { path: 'stream' });
          } catch {}
          // Phrase Decay (pattern-based): update hot phrase tracker
          try {
            ...
            if (Array.isArray(hot) && hot.length) {
              try {
                METRICS.inc('loop_phrase_hot_total', { path: 'stream', count: String(hot.length) });
              } catch {}
              try {
                ctx.io?.events?.emit?.('loopguard.phrase.hot', { conv_id, count: hot.length });
              } catch {}
            }
          } catch {}
            ...
            if (SHADOW && conv_id) {
              ...
              try {
                ...
                if (Array.isArray(mismatches) && mismatches.length) {
                  ...
                  try {
                    await shadowStashNudges({ convId: conv_id, nudges });
                  } catch {}
                  try {
            ...
            if (JUDGE && conv_id) {
              ...
              try {
                ...
                if (GUARD && overall < guardThresh && conv_id) {
                  ...
                  if (line) {
                    try {
                      setGuardHint(conv_id, String(line), { ttlTurns });
                    } catch {}
                    try {
                      METRICS.inc('continuity_guard_set_total', { path: 'stream' });
                    } catch {}
                    try {
                      METRICS.inc('guard_hint_stored_total', {
                        path: 'stream',
          ...
          try {
            ...
            try {
              maybeTagCooledPhrases(ctx, String(conv_id || ''), String(final || ''));
            } catch {}
            const endPayload = {
              final: sanitizeUtf8Text(String(final || '')),
              request_id: String(rid || ''),
            };
            if (lastSuspicion && typeof lastSuspicion.level === 'number')
              endPayload.suspicion = lastSuspicion;
          ...
          try {
            ...
            if (durMs > 0) {
              let le = 'inf';
              for (const b of STREAM_DURATION_MS_BUCKETS) {
                if (durMs <= b) {
                  le = String(b);
                  break;
                }
          ...
          try {
            const provider = String(ctx.vars.__selected_provider || '');
            const resolvedModel = String(ctx.vars.__selected_model || '');
            METRICS.inc('llm_provider_outcome_total', {
              provider,
              model,
              resolved_model: resolvedModel,
              source: engineSource,
          ...
          try {
            ...
            try {
              await logTurn(String(conv_id || 'conv'), 'assistant', String(final || ''));
            } catch {}
            const resolvedModel = String(ctx.vars.__selected_model || '');
            const post = await postTurnMemory({
              convId: conv_id,
            ...
            try {
              ...
              for (let i = 0; i < Number(post?.ef_kept || 0); i++) {
                METRICS.inc('memory_store_kept_total', { path: 'stream' });
              }
              for (let i = 0; i < Number(post?.ef_pruned || 0); i++) {
                METRICS.inc('memory_store_pruned_total', { path: 'stream' });
              }
            } catch {}
          } catch {}
          // === Memory: shaping + world snapshot
          ...
          // Facts reinforcement + decay + SSE memory.shape
          try {
            const ids = Array.isArray(ctx?.vars?.__facts_injected_ids)
              ? ctx.vars.__facts_injected_ids
              : [];
            const turnIndex = Number(turn || 0);
            if (conv_id) {
              const changed =
                Array.isArray(ids) && ids.length
                  ? reinforceFacts(conv_id, ids, undefined, turnIndex)
                  : [];
              const decayed = decayFacts(conv_id, turnIndex);
              try {
                if (changed.length) pushFactsUpdatedSSE(res, conv_id, changed);
              } catch {}
              try {
                emitMemoryShapeSSE(res, {
                  conv_id,
                  reinforced: changed?.length || 0,
                  decayed: decayed,
                  turn: turnIndex,
              ...
              try {
                METRICS.inc('memory_shape_total', { path: 'stream' });
              } catch {}
            }
          } catch {}
          try {
            scheduleFactsConsolidation(conv_id, Number(turn || 0));
          } catch {}
          ...
          try {
            if (streamKey) {
              ...
              const persisted = {
                ...
                tenant: String(ctx.vars.tenant || ''),
                request_id: String(rid || ''),
                tension: persistedTension,
                tension_beat: persistedBeat,
                style_meta: computeStyleMeta(conv_id),
              };
              ...
              try {
                ...
                if (!already) {
                  await markToolExecuted(toolCallId, {
                    tenant: String(persisted?.tenant || ''),
                    tool: 'conv.stream',
                  });
                }
              } catch {}
              try {
                await idemReleaseLock(streamKey);
              } catch {}
            }
          } catch {}
          try {
        ...
        try {
          // Fact-driven pre-injection nudge based on continuity and facts
          try {
            await maybeFactGuardNudge(ctx, { conv_id, turnIndex: Number(turn || 0), sseRes: res });
          } catch {}
          // --- Guard: next-turn hint pre-injection ---
          let guardPrefix = '';
          try {
            ...
            if (GUARD && conv_id) {
              ...
              if (v && v.text) {
                try {
                  METRICS.inc('guard_hint_available_total', { path: 'stream' });
                } catch {}
                const used = consumeGuardHint(conv_id);
                if (used) {
                  guardPrefix = String(used || '');
                  try {
                    METRICS.inc('guard_hint_injected_total', { path: 'stream' });
                  } catch {}
                  try {
                    METRICS.inc('continuity_guard_used_total', { path: 'stream' });
                  } catch {}
                }
              }
            }
          ...
          try {
            ...
            let agentId = 'bot';
            try {
              agentId = String(ctx?.vars?.active_agent || '').trim() || 'bot';
            } catch {}
            let arc = '';
            const ARC_ON = String(process.env.ARC_LINKING_ENABLED || '') === '1';
            if (ARC_ON && conv_id) {
              ...
              try {
                const uQ = new URL(`http://localhost${req.url}`);
                arcManual = String(uQ.searchParams.get('arc') || '').trim();
              } catch {}
              if (arcManual) {
                try {
                  const resArc = setArc(conv_id, arcManual);
                  if (resArc?.ok) METRICS.inc('memory_arc_set_total', { path: 'stream' });
                } catch {}
                arc = arcManual;
                try {
                  pushMemoryArcSSE(res, conv_id, arcManual);
                } catch {}
              } else {
                try {
                  const got = getArc(conv_id);
                  arc = String(got?.arc || '').trim();
                } catch {}
                if (!arc) {
                  ...
                  if (inf) {
                    try {
                      const resArc = setArc(conv_id, inf);
                      if (resArc?.ok) METRICS.inc('memory_arc_inferred_total', { path: 'stream' });
                    } catch {}
                    arc = inf;
                    try {
                      pushMemoryArcSSE(res, conv_id, inf);
                    } catch {}
                  }
                }
              }
            ...
            if (FACTS_ON2) {
              ...
              if (Array.isArray(rel) && rel.length) {
                ...
                guardPrefix = guardPrefix ? `${guardPrefix} | ${booster}` : booster;
                try {
                  ctx.vars.__facts_injected_ids = rel.map((f) => f.id);
                } catch {}
                try {
                  for (let i = 0; i < rel.length; i++)
                    METRICS.inc('facts_injected_total', { path: 'stream', reason: 'select_rel' });
                } catch {}
                try {
                  pushMemoryFactSSE(
                    res,
                    conv_id,
                    rel.map((f) => ({ id: f.id, text: String(f.text || '') }))
                  );
                } catch {}
              } else {
                ...
                try {
                  if (String(process.env.TEST_MEMORY_API || '').trim() === '1') {
                    const top =
                      selectTopFacts(conv_id, Number(process.env.FACT_SELECT_LIMIT || 3)) || [];
                    if (Array.isArray(top) && top.length) {
                      const booster = formatFactBooster(top.map((f) => String(f.text || '')));
                      guardPrefix = guardPrefix ? `${guardPrefix} | ${booster}` : booster;
                      try {
                        ctx.vars.__facts_injected_ids = top.map((f) => f.id);
                      } catch {}
                      try {
                        for (let i = 0; i < top.length; i++)
                          METRICS.inc('facts_injected_total', {
                            path: 'stream',
                            reason: 'fallback_top',
                          });
                      } catch {}
                      try {
                        pushMemoryFactSSE(
                          res,
                          conv_id,
                          top.map((f) => ({ id: f.id, text: String(f.text || '') }))
                        );
                      } catch {}
                    }
                  }
          ...
          try {
            ...
            await tryPromoteDreams(conv_id, String(textInput || ''), {
              agentId: activeAgent,
              arc: String(ctx?.vars?.arc || ''),
            });
          ...
          try {
            if (String(process.env.SCENE_LINKING_ENABLED || '') === '1') {
              ...
              try {
                const uQ = new URL(`http://localhost${req.url}`);
                sceneManual = String(uQ.searchParams.get('scene') || '').trim();
              } catch {}
              ...
              if (scene) {
                ...
                if (line) {
                  guardPrefix = guardPrefix ? `${line} ${guardPrefix}` : line;
                  try {
                    METRICS.inc('scene_injected_total', { path: 'stream' });
                  } catch {}
                }
                try {
                  const ids = Array.isArray(ctx?.vars?.__facts_injected_ids)
          ...
          try {
            ...
            if (ultraOn) {
              ...
              guardPrefix = guardPrefix ? `${guardPrefix} | ${novelty}` : novelty;
              try {
                METRICS.inc('ultra_novelty_hint_total', { path: 'stream' });
              } catch {}
            }
          } catch {}
          ...
          try {
            ...
            const fear = Math.max(
              0,
              Math.min(1, Number(ctx?.vars?.tension ?? ctx?.memory?.tension ?? 0))
            );
            try {
              ctx.memory.trust = trust;
            } catch {}
            try {
              ctx.memory.suspicion = suspicion;
            } catch {}
            try {
              ctx.memory.mood = mood;
            } catch {}
            try {
              ctx.memory.fear = fear;
            } catch {}
            ...
            try {
              ...
              ctx.memory.personality =
                (await BeliefStore.getPersonality(agentIdMem)) || ctx.memory.personality || '';
            } catch {}
              ...
              try {
                ...
                if (prof) {
                  ...
                  if (!Array.isArray(ctx.memory.beliefs) || ctx.memory.beliefs.length === 0) {
                    ctx.memory.beliefs = Array.isArray(prof.beliefs)
                      ? prof.beliefs.slice(0, maxBel)
                      : [];
                  }
                  if (
                    !Array.isArray(ctx.memory.logicConstraints) ||
                    ctx.memory.logicConstraints.length === 0
                  ) {
                    ctx.memory.logicConstraints = Array.isArray(prof.logic_constraints)
                      ? prof.logic_constraints.slice(0, maxCon)
                      : [];
                  }
                }
              } catch {}
            ...
            try {
              ...
              ctx.memory.recentFacts = recent.map((f) => ({
                id: f.id,
                text: String(f.text || ''),
              }));
            } catch {}
          ...
          try {
            if (typeof DisagreementCore?.buildGuard === 'function') {
              ...
              if (dg) {
                ...
                try {
                  METRICS.inc('disagreement_guard_total', { path: 'stream' });
                } catch {}
              }
            }
          } catch {}
          ...
          try {
            if (String(process.env.BELIEFS_ENABLED || '').trim()) {
                ...
                if (
                  ...
                  for (const c of conflicts) {
                    ...
                    try {
                      METRICS.inc('belief_conflict_total', {
                        path: 'stream',
                        char_id: String(ctx?.vars?.agent_id || 'default'),
                      });
                    } catch {}
                  }
                }
              ...
              if (Array.isArray(beliefBoosters) && beliefBoosters.length) {
                try {
                  for (const b of beliefBoosters) {
                    try {
                      ...
                      res.write(
                        `data: ${JSON.stringify({ belief_id: b.belief_id, text: b.text })}\n\n`
                      );
                    } catch {}
                    try {
                      METRICS.inc('belief_injected_total', {
                ...
                try {
                  ctx.vars.__extraBoosters = (
                    Array.isArray(ctx.vars.__extraBoosters) ? ctx.vars.__extraBoosters : []
                  ).concat(beliefBoosters.map((x) => String(x.text || '')));
                } catch {}
              }
            }
                ...
                if (Array.isArray(picks) && picks.length) {
                  ...
                  if (chosen.length) {
                    ...
                    try {
                      ...
                      res.write(
                        `data: ${JSON.stringify({ lines: chosen.map((x) => ({ kind: x.kind, text: x.text, hash: x.hash })), style: stylePref, max: maxLines, avoid_min_turns: avoidTurns })}\n\n`
                      );
                    } catch {}
                    try {
                      METRICS.inc('belief_injected_total', {
                ...
                if (Number(process.env.WATCHDOG_SSE || 0) && Array.isArray(flags) && flags.length) {
                  for (const f of flags) {
                    try {
                      ...
                      res.write(
                        `data: ${JSON.stringify({ key: f.key, value: f.value, severity: f.severity })}\n\n`
                      );
                    } catch {}
                  }
                }
          ...
          try {
            ...
            ctx.memory.personality =
              (await BeliefStore.getPersonality(agentIdMem)) || ctx.memory.personality || '';
            const dg = await computeDisagreementGuards(ctx, {
              conv_id: String(conv_id || ''),
              turn: Number(turn || 0),
              userText: String(textInput || ''),
            });
            if (dg && Array.isArray(dg.guards)) {
              ...
              for (const g of dg.guards) {
                if (g.type === 'belief' && g.text) {
                  lines.push(`Belief: ${String(g.text || '')}`);
                  try {
                    METRICS.inc('guard_belief_total', { path: 'stream' });
                  } catch {}
                  try {
                    emitGuardSSE(res, 'guard.belief', {
                      conv_id: String(conv_id || ''),
                      text: String(g.text || ''),
                      style: String(dg.style || ''),
                    });
                  ...
                  lines.push(`Constraint: ${String(g.text || '')}`);
                  try {
                    METRICS.inc('guard_constraint_total', { path: 'stream' });
                  } catch {}
                  try {
                    emitGuardSSE(res, 'guard.constraint', {
                      conv_id: String(conv_id || ''),
                      text: String(g.text || ''),
                      style: String(dg.style || ''),
                    });
                  ...
                  lines.push(`Contradiction: ${String(g.why || '')}`);
                  try {
                    METRICS.inc('guard_contradiction_total', { path: 'stream' });
                  } catch {}
                  try {
                    emitGuardSSE(res, 'guard.contradiction', {
                      conv_id: String(conv_id || ''),
                      why: String(g.why || ''),
                      fact: String(g.fact || ''),
                    });
              ...
              if (lines.length) {
                ...
                guardPrefix = guardPrefix ? `${block} ${guardPrefix}` : block;
                try {
                  ctx.vars.__guard_hint = String(guardPrefix || block || '');
                } catch {}
              }
              if (dg.failureRoll) {
                try {
                ...
                try {
                  emitGuardSSE(res, 'guard.failure', {
                    conv_id: String(conv_id || ''),
                    pct: Number(dg.failureRoll.pct || 0),
                    roll: Number(dg.failureRoll.roll || 0),
                    outcome: String(dg.failureRoll.outcome || ''),
            ...
            if (dg?.hardRefusal) {
              ...
              try {
                res.write(`event: start\n`);
                res.write(
                  `data: ${JSON.stringify({ ok: true, refused: true, reason: 'belief_constraint', guard_hint: String(ctx?.vars?.__guard_hint || ''), style: String(dg.style || 'firm') })}\n\n`
                );
              } catch {}
              const refusalText = renderRefusal({
                style: String(dg.style || 'firm'),
                reason: 'belief_constraint',
                spine: ctx?.vars?.spine,
                userText: String(textInput || ''),
              });
              try {
                res.write(`event: delta\n`);
              ...
              try {
                res.write(`event: end\n`);
                res.write(
                  `data: ${JSON.stringify({ ok: true, refused: true, reason: 'belief_constraint' })}\n\n`
                );
              } catch {}
              try {
                await sendMessageWithTick(async () => {
          ...
          try {
            ...
            try {
              ctx.vars.__disagree_action__ = String(disagreeAction || 'none');
            } catch {}
            if (Array.isArray(disagreeReasons) && disagreeReasons.length) {
              try {
                for (const r of disagreeReasons)
              ...
              try {
                ...
                res.write(
                  `data: ${JSON.stringify({ action: String(disagreeAction || 'none'), reasons: disagreeReasons, mood: String(sp?.mood || ''), trust: Number(sp?.trust ?? 0), suspicion: Number(sp?.suspicion ?? 0) })}\n\n`
                );
              } catch {}
            }
            if (Array.isArray(disagreeLines) && disagreeLines.length) {
              ...
              if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                ...
                try {
                  METRICS.inc('disagree_core_skipped_total', { reason: 'budget', path: 'stream' });
                } catch {}
              }
            }
          } catch {}
          ...
          try {
            ...
            let failroll = { lines: [], outcome: 'none', eval: null };
            if (disagreeAct !== 'refuse') {
              failroll = await maybeApplyFailureRoll(ctx, {
                convId: convIdFR,
                turn: turnFR,
                userText: String(textInput || ''),
              });
              ...
              try {
                if (failroll?.eval) {
                  ...
                  res.write(
                    `data: ${JSON.stringify({ conv_id: convIdFR, ...failroll.eval, outcome: String(failroll.outcome || 'none') })}\n\n`
                  );
                }
              } catch {}
              try {
                if (failroll?.eval) {
                  const payloadFR = {
                    risk: Number(failroll.eval?.pFail || 0),
                    roll: Number(failroll.eval?.roll || 0),
                    threshold: Number(failroll.eval?.threshold || 0),
                    outcome: String(failroll.outcome || 'none'),
                  };
              ...
              try {
                if (failroll?.eval) {
                  const actionTag =
                    detectActionTag(String(textInput || '')) ||
                    String(failroll.eval?.verb || 'attempt');
                  const payload = {
                    hint: '',
                    action: String(actionTag || 'attempt'),
                    chance: Number(failroll.eval?.pFail || 0),
                    success: !Boolean(failroll.eval?.fail),
                    style: String(process.env.FAILROLL_STYLE || ''),
                  };
                  res.write('event: memory.roll\n');
                  res.write(`data: ${JSON.stringify(payload)}\n\n`);
                }
              ...
              try {
                if (failroll?.eval?.nearMiss) {
                  ...
                  res.write('event: memory.fact\n');
                  res.write(
                    `data: ${JSON.stringify({ conv_id: convIdFR, kind: 'complication', vb, beat })}\n\n`
                  );
                }
              } catch {}
            }
          ...
          try {
            if (guardPrefix) {
              ...
              memoryPrefix = `${guardPrefix}\n${memoryPrefix || ''}`;
              try {
                ctx.vars.__guard_hint = guardPrefix;
              } catch {}
              try {
                ctx.vars.__memory_prefix = memoryPrefix;
              } catch {}
            ...
            if (cfgLG && cfgLG.enabled) {
              ...
              try {
                const memTokens = Array.isArray(ctx?.memory?.ultra_style_tokens)
                  ...
                  : Array.isArray(ctx?.vars?.ultra_style_tokens)
                    ? ctx.vars.ultra_style_tokens
                    : [];
                if (Array.isArray(memTokens) && memTokens.length) {
                  tokensStr = memTokens
                    .map((s) => String(s || '').trim())
              ...
              if (chosen) {
                ...
                if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                  ...
                  memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${booster}`;
                  try {
                    ctx.vars.__loopguard_style_token = String(chosen || '');
                  } catch {}
                  try {
                    ctx.vars.__loopguard_style_nudge = booster;
                  } catch {}
                  try {
                    ctx.vars.__memory_prefix = memoryPrefix;
                  } catch {}
          ...
          try {
            const extrasRaw = Array.isArray(ctx?.vars?.__extraBoosters)
              ? ctx.vars.__extraBoosters
              : [];
            const existing = new Set(
              String(memoryPrefix || '')
            ...
            if (String(process.env.SPINE_ENABLED || '1') === '1') {
              ...
              if (sp && sp.boosterLine) {
                ...
                if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                  ...
                  memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${sp.boosterLine}`;
                  try {
                    ctx.vars.__spine_booster_text = String(sp.boosterLine || '');
                  } catch {}
                  try {
                    ctx.vars.__memory_prefix = memoryPrefix;
                  } catch {}
          ...
          try {
            ...
            if (__style_meta) {
              ...
              if (sb && sb.text) {
                ...
                try {
                  ctx.vars.__style_booster_preset = String(
                    sb.preset || String(__style_meta?.preset || '')
                  );
                } catch {}
                try {
                  ctx.vars.__style_booster_est_tokens = Number(sb.estTokens || addTok || 0);
                } catch {}
                try {
                  ctx.vars.__style_booster_token_budget = Number(
                    sb.tokenBudget || Number(process.env.STYLE_BOOSTER_MAX_TOKENS || 40)
                  );
                } catch {}
                try {
                  ctx.vars.__memory_prefix = memoryPrefix;
                ...
                try {
                  broadcastAdminStyleEvent(conv_id, 'style.booster', {
                    conv_id,
                    text: __style_booster_text,
                    preset: String(__style_meta?.preset || ''),
                  });
                } catch {}
              }
          ...
          try {
            ...
            if (hintText) {
              let addTokR = 0;
              try {
                addTokR = Number(TokenCounter.estimate(hintText, { model })) || 0;
              } catch {
                addTokR = 0;
              }
              const budget = Number(
              ...
              if (!Number.isFinite(budget) || memoryInjectTokens + addTokR <= budget) {
                ...
                try {
                  METRICS.inc('refusal_hint_skipped_total', { reason: 'budget', path: 'stream' });
                } catch {}
              }
            }
          } catch {}
            ...
            if (String(ctx?.vars?.__failroll_outcome__ || '').trim()) {
                ...
                try {
                  return Math.max(
                    ...
                    Math.min(
                      1,
                      Number(
                        ctx?.vars?.trust ?? ctx?.vars?.trust_score ?? ctx?.memory?.trustLevel ?? 0.5
                      )
                    )
                  );
                } catch {
              ...
              const suspicion = (() => {
                try {
                  return Math.max(
                    0,
                    Math.min(1, Number(ctx?.vars?.suspicion ?? ctx?.vars?.suspicion_score ?? 0.0))
                  );
                } catch {
                  return 0.0;
                }
              ...
              const tension = (() => {
                try {
                  return Math.max(
                    0,
                    Math.min(1, Number(ctx?.vars?.tension ?? ctx?.memory?.tension ?? 0.5))
                  );
                } catch {
                  return 0.5;
                }
              ...
              if (useNewFR) {
                ...
                roll = assessRiskyAction({
                  ...
                  suspicion,
                  tension,
                  style: getRollStyle(String(conv_id || '')),
                });
              }
              if (roll && roll.action && String(roll.hint || '').trim()) {
                const hint = String(roll.hint || '').trim();
                ...
                if (!Number.isFinite(budget) || memoryInjectTokens + addTok <= budget) {
                  ...
                  try {
                    METRICS.inc('failure_roll_skipped_total', { reason: 'budget', path: 'stream' });
                  } catch {}
                }
              }
            }
          ...
          try {
            if (conv_id) {
              ...
              try {
                METRICS.inc('loop_phrase_seen_total', { path: 'stream_user_preturn' });
              } catch {}
            }
          } catch {}
          ...
          try {
            ...
            if (plan && plan.enabled) {
              ...
              if (avoid && avoid.text) {
                ...
                memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${avoid.text}`;
                try {
                  ctx.vars.__phrase_avoid_text = String(avoid.text || '');
                } catch {}
                try {
                  ctx.vars.__phrase_avoid_est_tokens = Number(avoid.estTokens || addTok || 0);
                } catch {}
                try {
                  ctx.vars.__memory_prefix = memoryPrefix;
                } catch {}
          ...
          try {
            ...
            if (Array.isArray(hot) && hot.length) {
              ...
              if (avoid2 && avoid2.text) {
                ...
                memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${avoid2.text}`;
                try {
                  ctx.vars.__phrase_hot_count = Number(hot.length || 0);
                } catch {}
                try {
                  ctx.vars.__memory_prefix = memoryPrefix;
                } catch {}
          ...
          try {
            ...
            const lastBot = Array.isArray(win)
              ...
              : null;
            const botPrev = String(lastBot?.text || '');
            const beatSignals = collectBeatSignals(String(conv_id || ''), ctx, {
              userText: String(text || ''),
              botPrev,
            });
            try {
              ctx.vars.__beat_signals = beatSignals;
            } catch {}
            const tensionHint =
              beatSignals && beatSignals.inputs && beatSignals.inputs.style
                ? beatSignals.inputs.style.tensionHint
                : (() => {
                    try {
                      const t = Number(ctx?.vars?.tension ?? ctx?.memory?.tension);
                      return Number.isFinite(t) ? t : null;
                    } catch {
                      return null;
                    }
                  })();
            const planBeat = detectBeat(String(conv_id || ''), {
              userText: String(text || ''),
              botPrev,
              tensionHint,
            });
            if (planBeat && planBeat.enabled) {
              ...
              if (cadenceBoost && cadenceBoost.text) {
                ...
                memoryPrefix = `${memoryPrefix ? `${memoryPrefix}\n` : ''}${cadenceBoost.text}`;
                try {
                  ctx.vars.__cadence_booster_text = String(cadenceBoost.text || '');
                } catch {}
                try {
                  ctx.vars.__memory_prefix = memoryPrefix;
                } catch {}
          ...
          try {
            ...
            try {
              if (conv_id) {
                ...
                const cadUserEff = {
                  ...cadUser,
                  enabled: ultraFeatureEnabled(conv_id, cadUser.enabled),
                };
            ...
            try {
              ...
              if (Array.isArray(pen?.penaltyHints) && pen.penaltyHints.length) {
                textInput = pen.penaltyHints.join('\n') + '\n\n' + textInput;
                try {
                  METRICS.inc('loopguard_phrase_penalty_total', { path: 'stream' });
                } catch {}
              }
            } catch {}
            ...
            try {
              ...
              const cadNextEff = {
                ...cadNext,
                enabled: ultraFeatureEnabled(conv_id, cadNext.enabled),
              };
          ...
          try {
            if (String(process.env.CONCLUDE_ENABLED || '0') === '1') {
              ...
              if (drag >= thresh) {
                ...
                try {
                  METRICS.inc('scene_conclusion_total', { path: 'stream' });
                } catch {}
                try {
          ...
          try {
            ctx.vars.tokens_in = TokenCounter.estimate(textInput, { model });
          } catch {}
          ...
          try {
            if (BEAT_STYLE_ENABLED && BEAT_ENABLED) {
              __beat_style_info_stream = applyBeatStyleBooster({
                ctx,
                conv_id,
                userText: textInput,
                currentBeat: getBeat(conv_id),
              });
              if (
                __beat_style_info_stream &&
                __beat_style_info_stream.mode === 'memory' &&
                __beat_style_info_stream.line
              ) {
                let addTok = 0;
                try {
                  addTok =
                    Number(TokenCounter.estimate(__beat_style_info_stream.line, { model })) || 0;
                } catch {
                  addTok = 0;
                }
                memoryInjectTokens += addTok;
                ...
                __beat_style_info_stream &&
                __beat_style_info_stream.mode === 'text' &&
                __beat_style_info_stream.line
              ) {
          ...
          try {
            ...
            if (plan && plan.booster && plan.booster.text) {
              try {
                ctx.vars.__style_backup_booster = String(plan.booster.text || '');
              } catch {}
              try {
                ctx.vars.__style_backup_preset = String(plan.altPreset || '');
              } catch {}
              try {
                ctx.vars.__style_backup_tokens = Array.isArray(plan.tokens) ? plan.tokens : [];
              } catch {}
            }
          } catch {}
          ...
          const forwardStream = (ctxLocal, tag) => {
            ...
            const onStart = (p) => {
              if (!startWritten) {
                ...
                write('start', {
                  ...p,
                  style_hedge: tag === 'backup',
                  style:
                    tag === 'backup' ? String(ctx?.vars?.__style_backup_preset || '') : undefined,
                });
              }
            };
            const onDelta = (payload) => {
              if (!winnerTag) {
                ...
                const meta = {
                  provider: String(ctxLocal?.vars?.__selected_provider || ''),
                  model: String(ctxLocal?.vars?.__selected_model || ''),
                  style: tag === 'backup' ? String(ctx?.vars?.__style_backup_preset || '') : '',
                };
                // Reflect winner provider/model on route ctx for downstream metrics
                if (tag === 'backup') {
                  try {
                    ctx.vars.__selected_provider = String(
                      ctxBackup?.vars?.__selected_provider || ctx.vars.__selected_provider || ''
                    );
                  } catch {}
                  try {
                    ctx.vars.__selected_model = String(
                      ctxBackup?.vars?.__selected_model || ctx.vars.__selected_model || ''
                    );
                  } catch {}
                  try {
                    ctx.vars.__used_provider = String(
                      ctxBackup?.vars?.__selected_provider || ctx.vars.__used_provider || ''
                    );
                  } catch {}
                } else {
                  try {
                    ctx.vars.__used_provider = String(
                      ctxPrimary?.vars?.__selected_provider || ctx.vars.__used_provider || ''
                    );
                  } catch {}
                }
                try {
                  notifyRef.fn(tag, meta);
                } catch {}
                ...
                try {
                  if (ctx.vars.__scene_conclusion_staged && !ctx.vars.__scene_conclusion_applied) {
                    ...
                    // Mirror applied to admin memory SSE channel
                    try {
                      broadcastAdminMemoryEvent(conv_id, 'scene.conclusion.applied', {
                        conv_id,
                        ok: true,
                      });
                    } catch {}
            ...
            const onNarrative = (p) => {
              if (winnerTag === tag) write('narrative.event', p);
            };
            const onEnd = (p) => {
              if (!endWritten && winnerTag === tag) {
                endWritten = true;
            ...
            unsub.push(() => {
              try {
                ctxLocal.io.events.off('stream.start', onStart);
              } catch {}
            });
            unsub.push(() => {
              try {
                ctxLocal.io.events.off('stream.delta', onDelta);
              } catch {}
            });
            unsub.push(() => {
              try {
                ctxLocal.io.events.off('narrative.event', onNarrative);
              } catch {}
            });
            unsub.push(() => {
              try {
                ctxLocal.io.events.off('stream.end', onEnd);
              } catch {}
            });
            return () =>
              unsub.forEach((fn) => {
            ...
            try {
              if (event === 'start') {
                ...
                const provider = String(
                  (payload && payload.provider) || ctx.vars.__selected_provider || ''
                );
                const resolvedModel = String(
                  (payload && payload.resolved_model) || ctx.vars.__selected_model || ''
                );
                providerAtStart = provider;
                modelAtStart = resolvedModel;
                const primary = String(
                  (payload && payload.provider_primary) || ctx.vars.__primary_provider || provider
                );
                const used = String(
                  (payload && payload.provider_used) || ctx.vars.__used_provider || provider
                );
                const hedgeTriggered = Boolean(
                  (payload && payload.hedge_triggered) || (primary && used && primary !== used)
                );
                const startSignals = computeAbuseSignals(textInput);
                emitAndRecordSignals(startSignals, { engine_source: engineSource });
                const __style_meta = computeStyleMeta(conv_id);
                try {
                  ...
                  try {
                    if (__style_meta) {
                      ...
                      METRICS.inc('style_pref_sse_total', { path: 'stream' });
                    }
                  } catch {}
                } catch {}
                ...
                try {
                  ...
                  if (bp && bp.enabled) {
                    ...
                    try {
                      broadcastAdminStyleEvent(conv_id, 'beat.tick', beatPayload);
                    } catch {}
                ...
                try {
                  ...
                  if (bp && bp.enabled) {
                    ...
                    if (cb) {
                      const estTokens =
                        Number(cb.estTokens || 0) ||
                        Number(TokenCounter.estimate(String(cb.text || ''), { model })) ||
                        0;
                      const payload = {
                        conv_id,
                        beat: String(bp.beat || ''),
                      ...
                      res.write(`data: ${JSON.stringify(payload)}\n\n`);
                      try {
                        broadcastAdminStyleEvent(conv_id, 'cadence.plan', payload);
                      } catch {}
                    }
                  }
                } catch {}
                ...
                try {
                  if (__dream_text) {
                    ...
                    res.write(`data: ${JSON.stringify({ text: __dream_text })}\n\n`);
                    try {
                      broadcastAdminMemoryEvent(conv_id, 'memory.dream', {
                        conv_id,
                        text: __dream_text,
                      });
                    } catch {}
                ...
                try {
                  ...
                  if (sbt) {
                    const sbPreset = String(
                      ctx?.vars?.__style_booster_preset ||
                        (__style_meta && __style_meta.preset) ||
                        ''
                    );
                    let estTokens = 0;
                    try {
                ...
                try {
                  ...
                  if (backupText) {
                    const primary = String(
                      (__style_meta && __style_meta.preset) || ctx?.vars?.style?.preset || ''
                    );
                    const backupPreset = String(ctx?.vars?.__style_backup_preset || '');
                    let estTokens = 0;
                    try {
                      estTokens = Number(TokenCounter.estimate(backupText, { model }) || 0);
                    } catch {
                      estTokens = 0;
                    }
                    let tokenBudget = 40;
                    ...
                    res.write(
                      `data: ${JSON.stringify({ conv_id, preset_primary: primary, preset_backup: backupPreset, estTokens, tokenBudget })}\n\n`
                    );
                    try {
                      broadcastAdminStyleEvent(conv_id, 'style.hedge.plan', {
                        conv_id,
                        preset_primary: primary,
                        preset_backup: backupPreset,
                        estTokens,
                      });
                    } catch {}
                    try {
                      METRICS.inc('style_hedge_started_total', { path: 'stream' });
                    } catch {}
                  }
                } catch {}
                ...
                try {
                  const snap =
                    updateBeat(String(conv_id || ''), {
                      botText: String(final || ''),
                      userText: String(textForModel || ''),
                    }) || null;
                  if (snap && snap.state) {
                    try {
                    ...
                    try {
                      ctx.vars.memory = ctx.vars.memory || {};
                      ctx.vars.memory.tension = Number(snap.tension || 0);
                      ctx.vars.beat_state = String(snap.state || '');
                    } catch {}
                    ...
                    try {
                      if (BEAT_SSE) {
                        ...
                        res.write(
                          `data: ${JSON.stringify({ conv_id, state: String(snap.state || ''), tension: Number(snap.tension || 0), stats: snap.stats || {} })}\n\n`
                        );
                      }
                    } catch {}
                    ...
                    try {
                      res.write('event: memory.beat\n');
                      res.write(
                        `data: ${JSON.stringify({ conv_id, tension: Number(snap.tension || 0), beat: String(snap.state || '') })}\n\n`
                      );
                      try {
                        METRICS.inc('memory_beat_emitted_total', { path: 'stream_end' });
                      } catch {}
                    } catch {}
                  }
                } catch (e) {
                  try {
                    METRICS.inc('beat_errors_total', { path: 'stream' });
                  } catch {}
                }
                // Cadence observed for streams (no reroll here)
                try {
                  const beatState = String(
                    ctx?.vars?.beat_state || getBeat(String(conv_id || ''))?.state || 'lull'
                  );
                  const target = getCadenceForBeat(beatState);
                ...
                const payloadOut = {
                  reason: 'style_hedge',
                  style: String((payload && payload.style) || ''),
                  from_provider: providerAtStart,
                  from_resolved_model: modelAtStart,
                  to_provider: toProvider,
                  to_resolved_model: toModel,
                };
                try {
                  res.write(`event: hedge.switch\ndata: ${JSON.stringify(payloadOut)}\n\n`);
                } catch {}
                try {
                  METRICS.inc('llm_hedge_switch_total', {
                    from: providerAtStart,
                ...
                try {
                  METRICS.inc('style_hedge_switch_total', { path: 'stream' });
                } catch {}
                ...
                try {
                  broadcastAdminMemoryEvent(conv_id, 'narrative.event', { conv_id, ...payload });
                } catch {}
                try {
          ...
          const ctxBackup = initBotContext({
            ...ctx,
            vars: {
              ...(ctx.vars || {}),
              conv_id,
              __style_hedge: String(ctx?.vars?.__style_backup_preset || ''),
            },
          });
          ...
          const startPrimary = async () => {
            ...
            (async () => {
              try {
                await llm.stream(textForModel, { model, signal: ac.signal }, ctxPrimary);
              } catch {
              } finally {
                try {
                  primaryUnsub();
          ...
          const startBackup = async () => {
            ...
            if (ALT_PROVIDER) {
              try {
                ctxBackup.vars.__provider_override = ALT_PROVIDER;
              } catch {}
              // Keep legacy visibility for observability; provider will overwrite on selection
              try {
                ctxBackup.vars.__selected_provider = ALT_PROVIDER;
              } catch {}
            }
            (async () => {
              try {
                await llm.stream(backupText, { model, signal: ac.signal }, ctxBackup);
              } catch {
              } finally {
                try {
                  backupUnsub();
          ...
          const resOrEmit = (evt, payload) => {
            if (evt === 'hedge.style.start') {
              try {
                res.write('event: style.hedge.start\n');
              } catch {}
              try {
                res.write(
                  `data: ${JSON.stringify({ preset_backup: ctx?.vars?.__style_backup_preset || 'unknown', ...payload })}\n\n`
                );
              } catch {}
              try {
                METRICS.inc('style_hedge_started_total', { path: 'stream' });
              ...
              try {
                res.write('event: style.hedge.switch\n');
              } catch {}
              try {
                res.write(
                  `data: ${JSON.stringify({ preset_backup: ctx?.vars?.__style_backup_preset || 'unknown', ...payload })}\n\n`
          ...
          const hedger = await runStyleHedge(
            ctx,
            startPrimary,
            startBackup,
            (from, meta) => {
              ...
              write('hedge.switch', {
                reason: 'style_hedge',
                style: String(ctx?.vars?.__style_backup_preset || ''),
              });
            },
            resOrEmit
          );
    ...
    if (String(req.method || 'GET').toUpperCase() === 'GET') {
      ...
      if (__path === '/memory/boosters') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_boosters_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/memory/arc') {
        try {
          ...
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ ok: true, conv_id: convId, arc: arc || null, at: Number(at || 0) })
          );
          try {
            METRICS.inc('responses_total', { status: '200' });
          } catch {}
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_arc_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path.startsWith('/memory/boosters/')) {
        try {
          ...
          res.end(
            JSON.stringify({
              error: 'memory_booster_get_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
      ...
      if (__path === '/memory/facets') {
        try {
          ...
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ ok: true, conv_id: convId, ...(char ? { char } : {}), facets: out })
          );
          try {
            METRICS.inc('responses_total', { status: '200' });
          } catch {}
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_facets_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/memory/shadow') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_shadow_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/memory/facts') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_facts_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/memory/beliefs') {
        try {
          ...
          try {
            const uQ = new URL(`http://localhost${req.url}`);
            charId = String(
              uQ.searchParams.get('char_id') || uQ.searchParams.get('agent_id') || ''
            ).trim();
          } catch {}
          if (!charId) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_beliefs_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/memory/audit') {
        try {
          ...
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ ok: true, conv_id: convId || undefined, count: items.length, items })
          );
          try {
            METRICS.inc('responses_total', { status: '200' });
          } catch {}
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_audit_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/billing/export') {
        try {
          ...
          try {
            globalThis.__ensureUsageLedger__?.();
          } catch {}
          ...
          let limit = Math.max(
            ...
            Math.min(
              5000,
              Number(new URL(`http://localhost${req.url}`).searchParams.get('limit') || 1000)
            )
          );
          try {
            const uTmp = new URL(`http://localhost${req.url}`);
          ...
          try {
            console.log(
              JSON.stringify({
                evt: 'billing_export_debug',
                startIndex,
                limit,
                buffer_len: Array.isArray(led.buffer) ? led.buffer.length : 0,
            ...
            appendTestOutput(
              JSON.stringify({
                ...
                buffer_len: Array.isArray(led.buffer) ? led.buffer.length : 0,
                lines_len: lines.length,
                last_cursor: String(led.lastCursor || ''),
              })
            );
          } catch {}
          res.writeHead(200, {
            'Content-Type': 'application/x-ndjson',
            'X-Resume-Token': String(led.lastCursor || ''),
            'X-Checksum': String(checksum || ''),
          });
          try {
            for (const ln of lines) res.write(ln + '\n');
          } catch {}
          try {
            res.end();
          } catch {}
      ...
      if (__path === '/memory/preview') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'memory_preview_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/memory/tuner') {
        try {
          ...
          let body = '';
          try {
            body = await AsyncFS.readFile(file, 'utf8');
          } catch {}
          if (!body) {
            res.writeHead(404, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: 'not_found' }));
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'serve_tuner_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/memory') {
        try {
          ...
          let body = '';
          try {
            body = await AsyncFS.readFile(fileSpa, 'utf8');
          } catch {}
          if (!body) {
            try {
              body = await AsyncFS.readFile(fileLegacy, 'utf8');
            } catch {}
          }
          if (!body) {
            res.writeHead(404, { 'Content-Type': 'application/json' });
          ...
          res.end(
            JSON.stringify({
              error: 'serve_admin_memory_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
      ...
      if (
        String(req.method || 'GET').toUpperCase() === 'GET' &&
        __path.startsWith('/admin/conv/') &&
        __path.endsWith('/phrases')
      ) {
        try {
          if (!adminGuard(req, res)) return;
          const parts = __path.split('/');
          ...
          try {
            ...
            res.writeHead(500, { 'Content-Type': 'application/json' });
            res.end(
              JSON.stringify({ error: 'phrase_inspector_failed', msg: String(e?.message || e) })
            );
            try {
              METRICS.inc('responses_total', { status: '500' });
            } catch {}
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'phrase_inspector_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/ultra') {
        try {
          ...
          try {
            const uTmp = new URL(`http://localhost${req.url}`);
            wantsUI = String(uTmp.searchParams.get('ui') || '') === '1';
          } catch {}
...
$('#off').onclick = async () => {
  ...
  s('Ultra['+id+'] = '+(j.ultra?'ON':'OFF'),'ok');
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'ultra_get_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
    ...
    if (String(req.method || 'GET').toUpperCase() === 'DELETE') {
      ...
      if (__path === '/state') {
        try {
          ...
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(
            JSON.stringify({ error: 'state_delete_failed', msg: String((e && e.message) || e) })
          );
          try {
            METRICS.inc('responses_total', { status: '500' });
          } catch {}
      ...
      if (__path === '/admin/beliefs/line') {
        try {
          ...
          res.end(
            JSON.stringify({
              error: 'beliefs_delete_line_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
      ...
      if (__path.startsWith('/admin/beliefs/') && __path !== '/admin/beliefs/line') {
        try {
          ...
          try {
            emitAdminMemoryEvent('beliefs.delete', { agentId, belief: hashOrText });
          } catch {}
          res.writeHead(200, { 'Content-Type': 'application/json' });
          ...
          res.end(
            JSON.stringify({
              error: 'agent_belief_delete_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
      ...
      if (__path === '/admin/constraints') {
        try {
          ...
          try {
            const uQ = new URL(`http://localhost${req.url}`);
            text = String(
              uQ.searchParams.get('text') || uQ.searchParams.get('constraint') || ''
            ).trim();
            hash = String(uQ.searchParams.get('hash') || '').trim();
          } catch {}
          const hashOrText = text || hash;
          ...
          try {
            emitAdminMemoryEvent('constraints.delete', { constraint: hashOrText });
          } catch {}
          res.writeHead(200, { 'Content-Type': 'application/json' });
          ...
          res.end(
            JSON.stringify({
              error: 'constraint_delete_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
      ...
      if (__path && __path.startsWith('/memory/facts/')) {
        try {
          ...
          res.end(
            JSON.stringify({
              error: 'memory_facts_delete_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
      ...
      if (__path && __path.startsWith('/memory/beliefs/')) {
        try {
          ...
          res.end(
            JSON.stringify({
              error: 'memory_beliefs_delete_failed',
              msg: String((e && e.message) || e),
            })
          );
          try {
    ...
    if (req.url === '/config/snapshot') {
      ...
      try {
        ...
        let safe = {};
        try {
          safe = JSON.parse(JSON.stringify(cfg));
        } catch {
          safe = {};
        }
        let configHash = '';
        ...
        res.writeHead(200, {
          'Content-Type': 'application/json',
          'X-Config-Hash': String(configHash || ''),
        });
        res.end(
          JSON.stringify({
            ok: true,
        ...
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(
          JSON.stringify({ error: 'config_snapshot_failed', msg: String((e && e.message) || e) })
        );
        try {
          METRICS.inc('responses_total', { status: '500' });
          span?.setAttribute?.('http.status_code', 500);
    ...
    if (req.url === '/heap/snapshot') {
      ...
      if (requireAuth) {
        ...
        try {
          console.log(
            JSON.stringify({
              evt: 'heap_snapshot_auth_debug',
              hdr_present: hdr.length > 0,
              token_hdr_len: tokenFromHdr.length,
              token_query_len: tokenFromQuery.length,
            })
          );
        } catch {}
        if (!ok) {
      ...
      try {
        ...
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(
          JSON.stringify({ error: 'heap_snapshot_failed', msg: String((e && e.message) || e) })
        );
        try {
          METRICS.inc('responses_total', { status: '500' });
          span?.setAttribute?.('http.status_code', 500);
    ...
    try {
      span?.setAttribute?.('http.status_code', res.statusCode || 404);
    } catch {}
    try {
      span?.end?.();
    } catch {}
    ...
    try {
      logAt(
        ...
        JSON.stringify({
          evt: 'service_listen',
          healthz: url,
          readyz: `http://localhost:${port}/readyz`,
          port,
        })
      );
    } catch {
  ...
  try {
    ctx.io.events.on('style.hedge.start', (p) => {
      ...
      try {
        res.write(
          `data: ${JSON.stringify({ preset_backup: ctx?.vars?.__style_backup_preset || 'unknown', ...p })}\n\n`
        );
      } catch {}
    });
  ...
  try {
    ...
    try {
      logAt(
        ...
        JSON.stringify({
          evt: 'service_error',
          msg: 'Failed to register graceful shutdown',
        ...
        () => {
          try {
            ...
            if (rssMb > ceilingMb) {
              ...
              logAt(
                'warn',
                JSON.stringify({ evt: 'rss_ceiling_breach', rss_mb: rssMb, ceiling_mb: ceilingMb })
              );
            }
          } catch {}
        },
  ...
  try {
    ...
    const debugHeap =
      String(process.env.DEBUG_HEAP || '').toLowerCase() === 'true' ||
      String(process.env.DEBUG_HEAP || '') === '1';
    if (debugHeap || !isProd) {
      const handler = (sig) => {
        try {
