...
const CANONICAL_CONVS = [
  ...
  { name: 'golden-urga-2', text: 'Count to three: 1, 2, 3.', engine: 'urga', persona_v: 'v1', prompt_v: 'v2' },
  { name: 'golden-urga-3', text: 'Echo the word: moonlight', engine: 'urga', persona_v: 'v2', prompt_v: 'v1' },
  { name: 'golden-echo-1', text: 'Plain echo test.', engine: 'echo' },
  { name: 'golden-echo-2', text: 'Symbols: !@#$%^&*()', engine: 'echo', persona_v: 'v1', prompt_v: 'v3' },
  ...
  { name: 'golden-echo-3', text: 'Whitespace     preserved', engine: 'echo', persona_v: 'v2', prompt_v: 'v2' },
  { name: 'golden-dreams-3', text: 'Final golden case.', engine: 'dreams', persona_v: 'v3', prompt_v: 'v3' }
];
...
function freezeOne({ name, text, engine, persona_v, prompt_v }) {
  ...
  const userForCompile = createMessage({ role: 'user', content: [text], turn: 0, ts: 0, id: 'msg_user_0' });
  const { bytes: compileBytes, hash: compileHash } = assembleForModel([userForCompile], { persona_v, prompt_v });

  // Message step: mirrors service stability defaults (conv:0:user id, ts=0).
  const userForMessage = createMessage({ role: 'user', content: [text], turn: 0, ts: 0, id: 'conv:0:user', conv_id: 'conv' });
  const { bytes: messageBytes, hash: messageHash } = assembleForModel([userForMessage], { persona_v, prompt_v });

  // Stream final from stub providers is deterministic: stub:<provider>:<model>.
  const provider = engine === 'urga' ? 'stub-urga' : (engine === 'echo' ? 'stub-echo' : 'stub-dreams');
