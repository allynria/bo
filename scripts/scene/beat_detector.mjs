...
const EMO_WORDS = (process.env.BEAT_EMO_WORDS || [
  'love','hurt','rage','fear','afraid','tremble','cry','tears','ache',
  'yearn','jealous','betray','panic','bleed','pulse','thrill','shiver'
]).toString().split(',').map(s=>s.trim().toLowerCase()).filter(Boolean);
...
function emoScore(t){
  ...
  let hits=0;
  for (const w of EMO_WORDS) { if (lw.includes(w)) hits++; }
  return hits;
}
...
function decideState(stats){
  ...
  if (lastPace >= CLIMAX_MIN_PACE && lastEmo >= CLIMAX_MIN_EMO && lastRatio <= REPET_MAX_RATIO && lastNovel >= 0.35) {
    return 'climax';
  }
...
function stateToTension(state){
  switch(state){
    ...
    case 'rising':  return 0.72;
    case 'falling': return 0.44;
    case 'lull':    return 0.22;
    default:        return 0.3;
  }
}
...
export function updateBeat(convId, { botText, userText }) {
  ...
  const prevAvgPace = rec.turns.length ? rec.turns.reduce((a,t)=>a+t.pace,0)/rec.turns.length : 0;
  ...
  const state = decideState({ avgPace, lastPace: pace, lastEmo: emo, lastRatio: ratio, lastNovel: novelty, prevAvgPace });
...
export function forceBeat(convId, state){
  try {
    ...
    const rec = STORE.get(convId) || { turns: [], lastWords: [], state: 'lull', tension: 0.22, ts: 0 };
