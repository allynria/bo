...
const CFG = {
  ...
  // Cadence text nudges
  CADENCE_RISING: (process.env.BEATS_CADENCE_RISING || 'Build momentum; vivid sensory detail, short-medium sentences.'),
  CADENCE_PEAK: (process.env.BEATS_CADENCE_PEAK || 'High tension; clipped, urgent lines; avoid filler.'),
  CADENCE_FALLING: (process.env.BEATS_CADENCE_FALLING || 'Let breath in; decompress the scene with controlled pacing.'),
  CADENCE_LULL: (process.env.BEATS_CADENCE_LULL || 'Slow, lyrical beats; interiority and atmosphere.')
};
...
function scoreTextTension(text='') {
  ...
  const strong = (t.match(/\b(bleed|shiver|gasp|scream|tremble|rush|pound|panic|surge|crack|snap|burn|clench)\b/gi) || []).length;
...
export function detectBeat(convId, { userText='', botPrev='', tensionHint=null } = {}) {
  ...
  // State machine
  let beat = st.beat;
  const R = CFG.RISING_THRESH, P = CFG.PEAK_THRESH, F = CFG.FALLING_THRESH;
  ...
  const styleToken =
    beat === 'peak' ? CFG.STYLE_PEAK :
    beat === 'rising' ? CFG.STYLE_RISING :
    beat === 'falling' ? CFG.STYLE_FALLING :
    CFG.STYLE_LULL;

  const cadence =
    beat === 'peak' ? CFG.CADENCE_PEAK :
    beat === 'rising' ? CFG.CADENCE_RISING :
    beat === 'falling' ? CFG.CADENCE_FALLING :
    CFG.CADENCE_LULL;
