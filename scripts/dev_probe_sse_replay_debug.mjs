  ...
  return new Promise((resolve, reject) => {
    const tick = () => {
      http.get(`${base}/healthz`, (res) => {
        if (res.statusCode === 200) resolve(); else if (Date.now() > deadline) reject(new Error('timeout'));
      }).on('error', () => { if (Date.now() > deadline) reject(new Error('timeout')); else setTimeout(tick, 100); });
    };
    tick();
  });
}
...
async function main() {
  ...
  const activeGauge = (Array.isArray(metrics?.counters) ? metrics.counters : []).find((c) => c.name === 'active_streams_current');
  console.log(JSON.stringify({ agg, fullStart: full.start, fullEnd: full.end, replayStart: replay.start, replayEnd: replay.end, replayStatus: replayRaw.status, replayBody: replayRaw.body, activeStreamsGauge: activeGauge }, null, 2));
  try { console.log('LOGS:\n' + getLogs()); } catch {}
  try { child.kill('SIGTERM'); } catch {}
}

main().catch((e) => { console.error('debug_error', e); process.exitCode = 1; });
