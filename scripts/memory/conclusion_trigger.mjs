...
export function computeDragScore({ loopScore = 0, deltaSim = 0, dwellMs = 0, tensionVar = 0 }) {
  ...
  const dwell = Math.min(1, dwellMs / (Number.isFinite(dwellTarget) && dwellTarget > 0 ? dwellTarget : 120000));
  const lowVar = 1 - Math.min(1, Math.max(0, Number(tensionVar)));
  const score = 0.35 * Number(loopScore || 0) + 0.25 * Number(deltaSim || 0) + 0.25 * Number(dwell || 0) + 0.15 * Number(lowVar || 0);
