...
function tokCount(s){ return Math.ceil(String(s||'').length/4); } // rough
...
export function extractFacts(text, { role='bot', turn=0 }={}) {
  const facts = [];
  const t = String(text||'');
  const add = (type, key, val, pol=+1) => facts.push({ type, who: role, key, val: norm(val), polarity: pol, turn, ts: nowMs() });
  // name
  const mName = t.match(RX.name); if (mName) add('name','name',mName[1],+1);
  // promise (store verb phrase)
  const mPr = t.match(RX.promise); if (mPr) add('promise','promise',mPr[1],+1);
  // location (coarse; prefer Proper Nouns)
  const mLoc = t.match(RX.location); if (mLoc) add('location','location',mLoc[2],+1);
  // fear/like/dislike/injury/secret cues
  const mFear = t.match(RX.fear); if (mFear) add('fear','fear',mFear[1],+1);
  const mLike = t.match(RX.like); if (mLike) add('like','like',mLike[1],+1);
  const mDisl = t.match(RX.dislike); if (mDisl) add('dislike','dislike',mDisl[1],+1);
  const mInj = t.match(RX.injury); if (mInj) add('injury','injury',mInj[1],+1);
  const mSec = t.match(RX.secret); if (mSec) add('secret','secret','shared secret',+1);
  return facts;
}

// Dedup/merge fact list to compact shadow memory
function mergeFacts(oldFacts=[], newFacts=[], { maxFacts=128 }={}) {
  const keyOf = f => `${f.who}|${f.type}|${(f.key||'').toLowerCase()}|${(f.val||'').toLowerCase()}`;
  const map = new Map();
...
const CONTRA = {
  fear_vs_like: (facts, text) => {
    ...
    const claimsLike = likes.filter(x => new RegExp(`\\b${escapeRegExp(x)}\\b`).test(text.toLowerCase()));
    const claimsNoFear = fears.filter(x => new RegExp(`\\b(?:not afraid of|no fear of)\\s+${escapeRegExp(x)}\\b`).test(text.toLowerCase()));
    const hits = [...claimsLike.map(x=>({kind:'fear_like_conflict', what:x})), ...claimsNoFear.map(x=>({kind:'fear_denial', what:x}))];
  ...
  name_flip: (facts, text) => {
    ...
    if (m && names.length) {
      ...
      if (m[1].toLowerCase() !== last) return [{ type:'name_conflict', score:0.9, detail:`was "${last}", now "${m[1]}"`}];
    }
    return [];
  },
  broken_promise: (facts, text) => {
    ...
    const hits = [];
    for (const p of ps) {
      const token = String(p.val||'').toLowerCase().split(/\s+/).slice(0,3).join(' ');
      if (!token) continue;
      if (lower.includes('break') && lower.includes(token)) hits.push({ type:'promise_conflict', score:0.8, detail:token });
      if (lower.includes('ignore') && lower.includes(token)) hits.push({ type:'promise_conflict', score:0.75, detail:token });
    }
    return hits;
  },
};
...
export function detectMismatches(facts=[], replyText='') {
  ...
  for (const [k, fn] of Object.entries(CONTRA)) {
    try { out.push(...fn(facts, replyText)); } catch {}
  }
  // dedupe by type+detail; keep highest score
  const best = new Map();
  for (const m of out) {
    const key = `${m.type}|${m.detail||''}`;
    const prev = best.get(key); if (!prev || (m.score||0) > (prev.score||0)) best.set(key, m);
  }
...
// ---------- Public API ----------
export async function shadowIngest({ convId, turn, role, text, maxTimeline=400, maxFacts=128 }) {
...
export async function shadowStashNudges({ convId, nudges = [] }) {
  const s = await loadShadow(convId);
  try { s.__last_nudges = Array.isArray(nudges) ? nudges : [String(nudges || '')]; } catch { s.__last_nudges = []; }
  await saveShadow(convId, s);
  return { snapshot: s };
}

export async function shadowSnapshot(convId){ return loadShadow(convId); }
export async function shadowRebuild({ convId, full=false }) {
  ...
  for (const t of s.turns) rebuilt.facts = mergeFacts(rebuilt.facts, extractFacts(t.text, { role: t.role, turn: t.turn }), { maxFacts: 256 });
