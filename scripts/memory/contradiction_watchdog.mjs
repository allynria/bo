...
function simpleNegationPair(u, key, val){
  ...
  const UNLOCK_OK = /(unlock|use (the )?key|pick (the )?lock|lockpick|jimmy|force (it|open)|break (the )?lock)/;

  if (val === 'locked') {
    if (!nounHit) return false; // require noun overlap with the state key
    const actionPhrases = [
      'open', 'push', 'pull', 'swing', 'slide', 'pry',
      'walk through', 'go through', 'step through', 'pass through', 'enter'
    ];
    const hasAction = actionPhrases.some(p => t.includes(p));
    const mentionsOpenObject = nouns.some(n => (
...
export function runWatchdog({ convId, userText }) {
  ...
  for (const s of states) {
    if (HARD.includes(s.value) && simpleNegationPair(userText, s.key, s.value)) {
      ...
      if (userText.toLowerCase().includes(kt) && /undo|ignore|as if|retcon/.test(userText.toLowerCase())) {
