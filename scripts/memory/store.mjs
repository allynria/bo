...
function makeLRU(capacity = 64) {
  ...
  return {
    ...
    set(k, v) { if (m.has(k)) m.delete(k); m.set(k, v); if (m.size > capacity) { const fk = m.keys().next().value; m.delete(fk); } },
    delete(k) { m.delete(k); },
    values() { return Array.from(m.values()); },
    size() { return m.size; },
  };
}
...
function isFresh(ts, ttl) { return (now() - (ts || 0)) <= ttl; }
function pruneListByTTL(items, ttl) { const t = now(); return items.filter(x => (t - (x.ts || 0)) <= ttl); }

async function rget(key) {
  if (!redisClient) return null;
  try { const s = await redisClient.get(key); if (!s) return null; return JSON.parse(s); } catch { return null; }
}
...
export async function pushEF(convId, item) {
  ...
  efLRU.set(k, obj); await rset(k, obj, EF_TTL);
  return obj;
}
...
export async function upsertCF(convId, name, patch) {
  ...
  if (names.length > CF_MAXC) {
    ...
    const sorted = names.sort((a,b) => (next.chars[a].last_update||0) - (next.chars[b].last_update||0));
    const drop = sorted.slice(0, names.length - CF_MAXC);
    for (const d of drop) delete next.chars[d];
  }
  cfLRU.set(k, next); await rset(k, next, CF_TTL);
  return next;
}
