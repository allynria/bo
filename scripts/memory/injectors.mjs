...
const MAX_RECAP_CHARS = Math.max(40, Math.min(240, Number(process.env.MEMORY_RECAP_MAX_CHARS || 200)));
const MAX_CUE_CHARS = Math.max(40, Math.min(140, Number(process.env.MEMORY_CUE_MAX_CHARS || 90)));
const MAX_FACET_CHARS = Math.max(40, Math.min(220, Number(process.env.MEMORY_FACET_MAX_CHARS || 160)));
...
export function buildRecapLine(strRecap) {
  ...
  const out = `(${clean})\n`;
  if (clean.length < String(strRecap || '').length) sampled('debug', 0.02, '[injectors] recap truncated/sanitized');
  return out;
}
...
export function renderEpisodicCue(item) {
  ...
  if (out.length < base.length) sampled('debug', 0.02, '[injectors] episodic cue truncated/sanitized');
  return out;
}
...
export function renderFacetEcho(facet) {
  ...
  if (s.length < joined.length) sampled('debug', 0.02, '[injectors] facet echo truncated/sanitized');
