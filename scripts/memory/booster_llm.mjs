...
function emitMetric(name, n=1, labels={}) {
  try {
    ...
    else if (globalThis?.UrgaCoreDeps?.Metrics?.inc) globalThis.UrgaCoreDeps.Metrics.inc(name, n, labels);
  } catch {}
}
...
export async function tryLLMBooster(ctx, convId, windowText, persona='') {
  ...
  if (String(process.env.BOOSTER_DEV_ONLY ?? '1') === '1' && process.env.NODE_ENV === 'production') return null;

  // Select model (favor echo/openai config; fallback to stub in tests)
  const model = process.env.BOOSTER_LLM_MODEL
  ...
  if (cached) { emitMetric('booster_llm_cache_total', 1, { hit: '1' }); return cached; }
  ...
  try {
    ...
    if (!line || line.length < 20) { emitMetric('booster_llm_yield_total', 1, { ok: '0' }); return null; }
    lruSet(key, line);
