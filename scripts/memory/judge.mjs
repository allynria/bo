...
const toWords = s => String(s||'').toLowerCase().split(/[^a-z0-9]+/).filter(Boolean);
...
function settingScore({ facts=[], replyText }) {
  const locs = facts.filter(f=>f.type==='location').slice(0,3).map(f=>String(f.val||'').toLowerCase());
  if (!locs.length) return 0.6;
  const reply = String(replyText||'').toLowerCase();
  let hits = 0;
  for (const l of locs) {
    if (!l) continue;
    if (reply.includes(l)) { hits++; continue; }
    // Partial noun token anchor (e.g., "Black Harbor" -> token "harbor")
    const toks = l.split(/[^a-z0-9]+/).filter(w=>w.length>=4);
    for (const t of toks) { if (reply.includes(t)) { hits++; break; } }
  }
...
function emotionScore({ lastUserText, replyText }) {
  ...
  const uPos = hasAny(lastUserText, POS), uNeg = hasAny(lastUserText, NEG);
  const rPos = hasAny(replyText, POS), rNeg = hasAny(replyText, NEG);
 ...
 */
export async function judgeContinuity({ convId, replyText, facetsTopK=2, recentTurns=6, weights }) {
  ...
  const chosenFacets = list.length ? (pickFacets({ list, nowTurn: (snap?.lastTurn||0), k: facetsTopK }) || []) : [];
...
export function parseWeights(envStr) {
  ...
  for (const part of String(envStr).split(',').map(s=>s.trim()).filter(Boolean)) {
