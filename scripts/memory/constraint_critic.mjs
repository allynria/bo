...
const RULES = [
  ...
  {
    re: /(jump|fall|leap).*(cliff|tower|roof).*(unharmed|no (?:harm|damage)|land safely)/i,
    class: 'world',
    reason: 'gravity_exists',
  },
  {
    re: /(walk|phase|pass).*(through|thru).*(wall|locked door)/i,
    class: 'world',
    reason: 'solid_barriers',
  },
  // Resurrection/world rules
  {
    re: /(resurrect|revive|bring back).*(dead|corpse)/i,
    class: 'world',
    reason: 'no_resurrection_magic',
  },
