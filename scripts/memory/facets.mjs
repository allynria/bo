...
export async function upsertFacet({
  convId,
  turn = 0,
  charId = 'bot',
  key,
  ...
  const idx = list.findIndex(
    (f) =>
      (f.key || '') === key && String(f.val || '').toLowerCase() === String(val || '').toLowerCase()
  );
  if (idx >= 0) {
    const f = list[idx];
    ...
    list.push({
      key,
      val,
      strength: Math.min(cap, Number(delta || 0)),
      lastTurn: turn,
      pinned: !!pin,
      createdTs: now,
      updatedTs: now,
...
export function scoreFacetForTurn(f, nowTurn, { halfLife }) {
  const s = halfLifeDecay(
    Number(f.strength || 0),
    Math.max(0, nowTurn - Number(f.lastTurn || 0)),
    halfLife
  );
...
export function pickFacets({
  ...
  const toks = (s) =>
    String(s || '')
      ...
      .match(/[a-z0-9]+/g) || [];
  const sim = (a, b) => {
    const A = new Set(toks(`${a.key}: ${a.val}`)),
      B = new Set(toks(`${b.key}: ${b.val}`));
  ...
  for (let step = 0; step < k; step++) {
    ...
    const adj = scored.map(({ f, rel }, i) => {
      ...
      let div = 0;
      for (const j of chosen) div = Math.max(div, sim(scored[i].f, j.f));
...
export function facetToNarrative(f, who = 'they') {
  ...
  switch (k) {
    case 'fear':
      return `(${who} still dreaded ${v}.)`;
    case 'bond':
      return `(${who} felt the bond: ${v}.)`;
    case 'goal':
      return `(${who} held to a goal: ${v}.)`;
    case 'scar':
      return `(${who} touched the old scar — ${v}.)`;
    case 'secret':
      return `(${who} kept a secret: ${v}.)`;
    default:
      return `(${who} remembered ${k}: ${v}.)`;
  }
}
