...
async function main() {
  ...
  console.log('backpressure_statuses', results.map(r => r.status));
  console.log('backpressure_retry_after', results.map(r => r.headers && r.headers['retry-after']));
  const m2 = await fetchJson(`${base2}/metrics`);
  console.log('metrics2', m2.status, JSON.stringify(m2.json));
  try { child2.kill('SIGTERM'); } catch {}
}

main().catch((e) => { console.error('taxonomy_repro_error', e && e.stack || e); process.exitCode = 1; });
