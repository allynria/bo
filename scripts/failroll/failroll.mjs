...
export function detectRiskIntent(userText, pattern = process.env.FAILROLL_RISK_REGEX) {
  ...
  let rx = null;
  if (pat) {
    try { rx = new RegExp(pat, 'i'); } catch { rx = null; }
    return !!(userText && rx && rx.test(String(userText)));
  }
  // Fallback: common risky actions so tests work without FAILROLL_RISK_REGEX
  const fallbackRx = /\b(sneak|stealth|slip\s+past|pick\s+the?\s*lock|lockpick|lock\s*pick|lie|deceive|bluff|fabricate|pretend|intimidate|threaten|coerce|charm|steal|pickpocket|snatch|lift|dodge|evade|sidestep|parry|deflect|shoot|fire\b|take\s+(?:an|the)\s*shot|hack|bypass|override|climb|scale\b|ascend|jump\s+(?:the\s*)?gap|leap\s+across)\b/i;
...
export function computeFailProb({ base, trust=0.5, suspicion=0.0, tension=0.0 }) {
  ...
  const sw  = Number(process.env.FAILROLL_SUSPICION_WEIGHT ?? 0.30);
  const aw  = Number(process.env.FAILROLL_TENSION_WEIGHT ?? 0.20);
  const min = Number(process.env.FAILROLL_MIN ?? 0.05);
  const max = Number(process.env.FAILROLL_MAX ?? 0.90);
...
export function buildOutcomeBooster({ style='diegetic', success, verb='attempt', reason='' }) {
  if (style === 'meta') {
