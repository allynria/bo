    ...
    const req = http.request({ method: 'GET', hostname: u.hostname, port: u.port, path: u.pathname + (u.search || '') }, (res) => {
      ...
      res.on('data', (d) => {
        ...
        for (const chunk of chunks) {
          ...
          if (evt === 'memory.tension') { try { tensionEvt = JSON.parse(dataStr); } catch {} }
          else if (evt === 'start') { try { startPayload = JSON.parse(dataStr); } catch {} }
      ...
      res.on('error', reject);
      res.on('end', () => { if (!endPayload) resolve({ tension: tensionEvt, start: startPayload, end: null }); });
    });
    req.on('error', reject);
    req.end();
  });
