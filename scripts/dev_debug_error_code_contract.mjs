...
function postJson(url, body, headers = {}) { return postRaw(url, JSON.stringify(body || {}), { 'Content-Type': 'application/json', ...headers }); }
...
async function main() {
  ...
  const log = (label, r) => { const j = r.json || {}; console.log(`${label}: status=${r.status} error=${j.error || ''} reason=${j.reason || ''}`); };
  ...
  const headers409 = { origin: 'http://ok.test', authorization: 'Bearer test-token', 'Idempotency-Key': idemKey };
  const body409 = { text: 'dup-stream', conv_id: 'c409m', turn: 0, engine: 'urga', ts: Date.now() };
  const [dm1, dm2] = await Promise.all([ postJson(url409, body409, headers409), postJson(url409, body409, headers409) ]);
  log('409-dm1', dm1);
  log('409-dm2', dm2);
