  ...
  return new Promise((resolve, reject) => {
    ...
    const tryOnce = () => {
      ...
      const req = http.get({ hostname: u.hostname, port: u.port, path: '/healthz' }, (res) => {
        ...
        if (Date.now() < end) setTimeout(tryOnce, 100); else reject(new Error(String(res.statusCode)));
      });
      req.on('error', () => { if (Date.now() < end) setTimeout(tryOnce, 100); else reject(new Error('error')); });
    };
    tryOnce();
  });
}
...
async function main() {
  ...
  await waitForUp(base, { timeout: 3000 }).catch((e) => { console.error('waitForUp error:', e); });

  // Trigger a simple message alias to produce llm_cost metrics (stub provider)
  await new Promise((resolve, reject) => {
    const body = JSON.stringify({ message: { role: 'user', content: 'hello' }, ctx: { vars: { tenant: 'test-tenant' } } });
    const req = http.request(`${base}/message`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-tenant': 'test-tenant' } }, (res) => {
      let raw = '';
      res.setEncoding('utf8');
      res.on('data', (c) => { raw += c; });
      res.on('end', () => {
        console.log('MESSAGE_ALIAS_STATUS:', res.statusCode);
        try { console.log('MESSAGE_ALIAS_BODY:', raw.slice(0, 160)); } catch {}
        resolve();
      });
    });
    req.on('error', reject);
  ...
  const ndj = await fetchNdjson(`${base}/billing/export`, { Authorization: `Bearer ${token}` }).catch((e) => ({ status: 0, body: '', headers: {} }));
...
main().catch((e) => { console.error('debug error:', e); process.exit(1); });
