...
async function main() {
  ...
  const cases = [
    ...
    {
      name: 'dreams_fallback_urga',
      model: 'dreams',
      env: { DREAMS_PROVIDER: '', URGA_PROVIDER: 'stub-urga', ECHO_PROVIDER: 'stub-echo' },
    },
    {
      name: 'dreams_fallback_echo',
      model: 'dreams',
      env: { DREAMS_PROVIDER: 'unknown-provider', URGA_PROVIDER: '', ECHO_PROVIDER: 'stub-echo' },
    },
    // echo
    {
      name: 'echo_direct',
    ...
    {
      name: 'echo_fallback_urga',
      model: 'echo',
      env: { DREAMS_PROVIDER: 'stub-dreams', URGA_PROVIDER: 'stub-urga', ECHO_PROVIDER: '' },
    },
    {
      name: 'echo_fallback_dreams',
      model: 'echo',
      env: { DREAMS_PROVIDER: 'stub-dreams', URGA_PROVIDER: '', ECHO_PROVIDER: 'unknown-provider' },
    },
    // default
    {
      name: 'default_direct_urga',
      model: '',
      env: {
        DREAMS_PROVIDER: 'stub-dreams',
        URGA_PROVIDER: 'stub-urga',
    ...
    {
      name: 'default_fallback_echo',
      model: '',
      env: { DREAMS_PROVIDER: 'stub-dreams', URGA_PROVIDER: '', ECHO_PROVIDER: 'stub-echo' },
    },
    {
      name: 'default_fallback_dreams',
      model: '',
      env: { DREAMS_PROVIDER: 'stub-dreams', URGA_PROVIDER: 'unknown-provider', ECHO_PROVIDER: '' },
    },
  ];
  ...
  if (!allPass) {
    console.error(
      JSON.stringify({ ok: false, reason: 'env precedence assertions failed', results })
    );
    process.exitCode = 1;
    return;
  }
