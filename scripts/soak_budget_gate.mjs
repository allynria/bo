...
async function main() {
  ...
  const p99TargetMs = Number(
    process.env.RESPOND_P99_TARGET_MS || (process.platform === 'win32' ? 20 : 10)
  );
  ...
  const child = allowServiceSpawn
    ? startService({
        ...
        PORT: String(port),
        QUEUE_MAX: '0',
        ADMIN_TOKEN: String(process.env.ADMIN_TOKEN || 'ci'),
  ...
  const metrics = await fetchJson(`${base}/metrics`).catch(() => ({
    status: 0,
    body: { counters: [] },
  }));
  const counters = Array.isArray(metrics?.body?.counters) ? metrics.body.counters : [];
  const openCircuits = counters
    .filter((c) => c.name.includes('circuit') || c.labels?.reason === 'circuit_open')
  ...
  console.log(
    JSON.stringify({
      ...
      heap_start_bytes: heapStartBytes,
      heap_end_bytes: heapEndBytes,
    })
  );
  ...
  if (p99 > p99TargetMs) {
    fail = true;
    reasons.push(`p99_ms>${p99TargetMs}`);
  }
  if (baselineP99Ms > 0) {
    const maxAllowed = baselineP99Ms * (1 + allowedRegressionPct / 100);
    if (p99 > maxAllowed) {
      fail = true;
      reasons.push(`p99_regression>${allowedRegressionPct}%`);
    }
  }
  const rssBudgetPct = Number(process.env.SOAK_RSS_BUDGET_PCT || 5);
  if (baselineMb > 0 && rssEndMb > Math.ceil(baselineMb * (1 + rssBudgetPct / 100))) {
    fail = true;
    reasons.push(`rss_growth>${rssBudgetPct}%`);
  }
  ...
  if (
    heapStartBytes > 0 &&
    heapEndBytes > Math.ceil(heapStartBytes * (1 + retainedBudgetPct / 100))
  ) {
    fail = true;
    reasons.push(`retained_objects_diff>${retainedBudgetPct}%`);
  }
  // Require no circuit openings
  if (openCircuits > 0) {
    fail = true;
    reasons.push('open_circuits>0');
  }
  // require zero error spikes
  if (errors.spikes > 0) {
    fail = true;
    reasons.push('error_spikes>0');
  }
...
main().catch((e) => {
  console.error('soak_budget_gate_error', (e && e.stack) || e);
  process.exitCode = 1;
});
