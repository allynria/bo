...
test('Shared RL: recreates shard directories after purge mid-run', async () => {
  ...
  const tmpBase = path.join(os.tmpdir(), `trae_rl_purge_${process.pid}_${Math.random().toString(36).slice(2)}`);
  ...
  const exists1 = await fsp.stat(filePath).then(() => true).catch(() => false);
  ...
  const existsBaseAfterPurge = await fsp.stat(path.join(tmpBase, 'urga_rl')).then(() => true).catch(() => false);
  ...
  const exists2 = await fsp.stat(filePath).then(() => true).catch(() => false);
  assert.equal(exists2, true, 'backend file should be recreated after purge');

  // Cleanup
  try { await fsp.rm(path.join(tmpBase, 'urga_rl'), { recursive: true, force: true }); } catch {}
