...
test('AB variant stickiness persists across restart for message and stream', async () => {
  ...
  const env = { PORT: String(port), QUEUE_MAX: '0', LLM_TURN_BUDGET: '5', LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga', TMPDIR: tmpRoot, TEMP: tmpRoot, CONV_RATE_MAX: '1000', CONV_RATE_WINDOW: '2000', CONV_SOFT_MAX: '5000', CONV_SOFT_WINDOW_MS: '2000' };
  ...
  for (let start = 0; start < TOTAL; start += BATCH) {
    ...
    for (let j = start; j < Math.min(TOTAL, start + BATCH); j++) {
      ...
      tasks.push(postJson(`${base}/conv/message`, makeBody(j, cid)).then((r) => {
  ...
  for (let i = 0; i < 2000; i++) {
    ...
    tasks2.push(postJson(`${base}/conv/message`, body).then((r) => {
      assert.equal(r.status, 200);
      const v = String(r.json.variant_v || '');
  ...
  for (let i = 0; i < convs.length; i++) {
    ...
    assert.equal(String(s.startEvent.variant_v || ''), expected, `Stream variant changed for ${conv_id}`);
  }
