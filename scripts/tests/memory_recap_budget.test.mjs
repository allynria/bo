...
async function getJSON(port, path) {
  return new Promise((resolve, reject) => {
    http.get({ host: '127.0.0.1', port, path }, (res) => {
      let data=''; res.on('data', d => data += String(d));
...
async function main() {
  ...
  const j = await getJSON(PORT, q({ conv_id: convId, text: 'We made a solemn promise on the bridge, under rain.', model: 'o200k_base', turn: '4', token: 't' }));
