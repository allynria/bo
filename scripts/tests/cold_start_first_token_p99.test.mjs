...
function fetchSSEFirstToken(url) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.get({ hostname: u.hostname, port: u.port, path: u.pathname + u.search }, (res) => {
      ...
      res.on('end', () => { if (firstDeltaMs == null) resolve(Math.max(0, Date.now() - t0)); });
    });
    req.on('error', reject);
  });
}
...
test('Cold-start p99 time-to-first-token below target with prewarm', async () => {
  ...
  const env = { PORT: String(port), QUEUE_MAX: '0', LLM_TURN_BUDGET: '5', LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga', PREWARM_MODELS: 'gpt-4o-mini,echo-small' };
