...
function runWorkerEnv(env, opts = {}) {
  ...
  return new Promise((resolve) => {
    const to = setTimeout(() => { try { child.kill('SIGKILL'); } catch {}; resolve({ code: -1, stdout, stderr, timeout: true }); }, Math.max(500, Number(env.TOOL_TIMEOUT_MS || 3000)));
...
test('Worker enforces per-request network timeout', async () => {
  ...
  const server = http.createServer((_req, _res) => { /* hang intentionally */ });
