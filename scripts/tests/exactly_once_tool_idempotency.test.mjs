...
test('Exactly-once tool idempotency: /conv/message and /conv/stream mark once per key', async () => {
  ...
  const r1 = await postJsonWithHeaders(`${base}/conv/message`, body, {
    'Idempotency-Key': idemMsgKey,
  });
  ...
  const r2 = await postJsonWithHeaders(`${base}/conv/message`, body, {
    'Idempotency-Key': idemMsgKey,
  });
  ...
  const s1 = await fetchSSE(
    `${base}/conv/stream?text=Stream%20once&conv_id=conv-idem-s1&turn=0&engine=urga`,
    { 'Idempotency-Key': idemStreamKey }
  );
  ...
  assert.ok(
    ...
    'SSE end carries idempotent_replay'
  );
