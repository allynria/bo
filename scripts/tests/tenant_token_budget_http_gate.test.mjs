...
test('Token budget gate returns 429 and increments budget_prevented_total on both endpoints', async () => {
  ...
  const r2 = await fetchStatus(
    `${base}/conv/stream?text=hello&conv_id=tok-gate-stream&turn=0&engine=urga`
  );
  ...
  for (let i = 0; i < 12 && count < 2; i++) {
    ...
    count = counters
      .filter(
        (c) => c.name === 'budget_prevented_total' && c.labels?.scope === 'tenant_tokens_http'
      )
  ...
  assert.ok(
    count >= 2,
    'budget_prevented_total for tenant_tokens_http should increment for both denials'
  );
