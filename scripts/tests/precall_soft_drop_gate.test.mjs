...
test('Pre-call soft-drop gate triggers under CPU override and recovers', async () => {
  ...
  const env = {
    ...
    CPU_SOFT_DROP_FORCE: '1',
    PRECALL_SHED_JITTER_MS: '300',
  };
  ...
  const res1 = await fetchJson(`${base}/conv/message`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body,
  });
  console.log(
    'CPU soft-drop message status=',
    res1.status,
    'headers.retry-after=',
    res1.headers?.['retry-after'],
    'json=',
    res1.json
  );
  ...
  console.log(
    'CPU soft-drop metrics status=',
    metrics1.status,
    'counters len=',
    Array.isArray(metrics1.json?.counters) ? metrics1.json.counters.length : 0
  );
  ...
  assert.ok(
    !!cs1.find((c) => c.name === 'precall_soft_drop_total' && String(c.labels?.reason) === 'cpu'),
    'precall_soft_drop_total counter seen for cpu'
  );
  ...
  const env2 = {
    ...
    LLM_TEST_STUBS: '1',
    URGA_PROVIDER: 'stub-urga',
    CPU_SOFT_DROP_FORCE: '0',
  };
  const { child: child2, getLogs: getLogs2 } = startService(env2);
  const base2 = `http://localhost:${port2}`;
  try {
    await waitForUp(base2, { timeout: 5000 });
    console.log('CPU recovery: service up at', base2);
  } catch (e) {
    console.log('CPU recovery: service failed to start. Logs=', getLogs2());
    throw e;
  }
  ...
  for (let i = 0; i < 20; i++) {
    const t0 = Date.now();
    const r = await fetchJson(`${base2}/conv/message`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        text: `probe ${i}`,
        conv_id: `softdrop-cpu-recover-${i}`,
        turn: 0,
        engine: 'urga',
      }),
    });
...
test('Pre-call soft-drop gate triggers under RSS override', async () => {
  ...
  assert.ok(
    !!cs.find((c) => c.name === 'precall_soft_drop_total' && c.labels?.reason === 'rss'),
    'precall_soft_drop_total counter seen for rss'
  );
