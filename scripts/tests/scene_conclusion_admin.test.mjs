...
test('admin forced scene conclusion applies on next turn without duplicate staged SSE', async () => {
  ...
  const stagedAdmin = (m1.counters || []).find(
    (c) => c.name === 'scene_conclusion_staged_total' && String(c.labels?.path || '') === 'admin'
  );
  ...
  const res = await fetch(
    `${BASE}/conv/stream?engine=urga&conv_id=${encodeURIComponent(conv_id)}&turn=1&text=${encodeURIComponent('Hello')}`,
    { headers: { Accept: 'text/event-stream', Authorization: `Bearer ${convToken}` } }
  );
  ...
  const appliedStream = (m2.counters || []).find(
    (c) => c.name === 'scene_conclusion_applied_total' && String(c.labels?.path || '') === 'stream'
  );
  assert.ok(appliedStream, 'scene_conclusion_applied_total (stream) present');
  assert.equal(Number(appliedStream.value || 0), 1);

  const stagedStream = (m2.counters || []).find(
    (c) => c.name === 'scene_conclusion_staged_total' && String(c.labels?.path || '') === 'stream'
  );
