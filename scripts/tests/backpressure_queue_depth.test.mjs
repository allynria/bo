...
test('Queue depth tracks inflight within ±5% and backpressure gates', async () => {
  ...
  const { child, getLogs } = startService({ NODE_ENV: 'test', PORT: String(port), LOG_JSON: '1', QUEUE_MAX: '5' });
  ...
  const diffs = samples.filter((s) => s.inflight > 0).map((s) => Math.abs(s.depth - s.inflight) / s.inflight);
  ...
  let saw503 = false; let retryHeader = '';
