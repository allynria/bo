...
function fetchSSE(url, headers = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({ method: 'GET', hostname: u.hostname, port: u.port, path: u.pathname + (u.search || ''), headers }, (res) => {
      ...
      res.on('end', () => resolve({ status: res.statusCode, headers: res.headers, startEvent, endEvent }));
    });
    req.on('error', reject);
    req.end();
  });
...
test('Tenant surfaces in replayed /conv/stream start payload', async () => {
  ...
  const env = { PORT: String(port), QUEUE_MAX: '0', LLM_TURN_BUDGET: '5', LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga', IDEMPOTENCY_TTL_MS: '300000' };
  ...
  assert.equal(s2.startEvent.tenant, tenantVal.replace(/[^a-zA-Z0-9_\.\-]/g, ''), 'tenant surfaced in replay start');
