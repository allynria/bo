...
test('conv endpoints block unauth and enforce CORS in prod', async () => {
  ...
  // message: no auth -> 401
  const mNoAuth = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'c1', ts: nowTs },
    { origin: allowed }
  );
  ...
  const mBadCors = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'c1', ts: nowTs },
    { origin: 'http://evil.test', authorization: `Bearer ${token}` }
  );
  ...
  // stream: no auth -> 401
  const sNoAuth = await getRaw(`${base}/conv/stream?text=hi&conv_id=c2&turn=0&ts=${nowTs}`, {
    origin: allowed,
  });
  ...
  // stream: bad CORS -> 403
  const sBadCors = await getRaw(`${base}/conv/stream?text=hi&conv_id=c2&turn=0&ts=${nowTs}`, {
  ...
  // stream: valid -> 200
  const sOk = await getRaw(`${base}/conv/stream?text=hi&conv_id=c2&turn=0&ts=${nowTs}`, {
    origin: allowed,
    authorization: `Bearer ${token}`,
  });
...
test('replay window enforced for message and stream', async () => {
  ...
  const mStale = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'c3', ts: staleTs },
    { origin: allowed, authorization: `Bearer ${token}` }
  );
  assert.equal(mStale.status, 401, 'stale ts blocked');
  assert.ok(mStale.text.includes('replay_window_exceeded'));

  const mFresh = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'c3', ts: freshTs },
    { origin: allowed, authorization: `Bearer ${token}` }
  );
  assert.equal(mFresh.status, 200, 'fresh ts accepted');

  const sStale = await getRaw(`${base}/conv/stream?text=hi&conv_id=c3&turn=0&ts=${staleTs}`, {
    origin: allowed,
    authorization: `Bearer ${token}`,
  });
  ...
  const sFresh = await getRaw(`${base}/conv/stream?text=hi&conv_id=c3&turn=0&ts=${freshTs}`, {
    origin: allowed,
    authorization: `Bearer ${token}`,
  });
...
test('HMAC signatures accepted; invalid or missing rejected', async () => {
  ...
  const macMsg = crypto
    ...
    .update(`POST:message:${String(ts)}`)
    .digest('hex');
  const macStr = crypto
    .createHmac('sha256', secret)
    .update(`GET:stream:${String(ts)}`)
    .digest('hex');

  const mNoMac = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'c4', ts },
    { origin: allowed }
  );
  ...
  const mOk = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'c4', ts },
    { origin: allowed, 'x-client-mac': macMsg }
  );
  assert.equal(mOk.status, 200, 'message with mac accepted');

  const sNoMac = await getRaw(`${base}/conv/stream?text=hi&conv_id=c4&turn=0&ts=${ts}`, {
    origin: allowed,
  });
  assert.equal(sNoMac.status, 401, 'stream missing mac blocked');

  const sOk = await getRaw(`${base}/conv/stream?text=hi&conv_id=c4&turn=0&ts=${ts}`, {
    origin: allowed,
    'x-client-mac': macStr,
  });
