...
test('Tenant surfaces in /conv/stream start payload', async () => {
  const port = 4350 + Math.floor(Math.random() * 200);
  ...
  const conv_id = 'tenant-stream-1';
  const tenantVal = 'Tenant-ABC_01';
  const u = `${base}/conv/stream?conv_id=${encodeURIComponent(conv_id)}&turn=0&engine=urga&tenant=${encodeURIComponent(tenantVal)}&text=${encodeURIComponent('plain text')}`;
  const sse = await fetchSSE(u);
  assert.ok(sse.start, 'start event present');
  assert.equal(sse.start.provider, 'stub-urga');
  assert.equal(sse.start.resolved_model, 'urga');
  assert.equal(sse.start.tenant, tenantVal.replace(/[^a-zA-Z0-9_\.\-]/g, ''), 'tenant surfaced in start');
