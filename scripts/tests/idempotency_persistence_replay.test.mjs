...
test('Idempotent message replay persists across restarts and 1,000 replays', async () => {
  ...
  const env = { PORT: String(port), QUEUE_MAX: '0', LLM_TURN_BUDGET: '5', LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga', IDEMPOTENCY_TTL_MS: '300000', CONV_SOFT_MAX: '5000', MAX_HEADER_BYTES: '8192' };
  ...
  assert.ok(first.json && first.json.hash && first.json.bytes_b64, 'first response includes determinism artifacts');
