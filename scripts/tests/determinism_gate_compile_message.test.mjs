...
function describeDiffB64(a, b, label = 'bytes_b64') {
  ...
  let idx = -1;
  for (let i = 0; i < minLen; i++) { if (A[i] !== B[i]) { idx = i; break; } }
...
test('Determinism gate: /conv/compile returns stable hash/bytes_b64', async () => {
  ...
  assert.ok(r1.json && r1.json.hash && r1.json.bytes_b64, 'first compile provides determinism artifacts');
  assert.ok(r2.json && r2.json.hash && r2.json.bytes_b64, 'second compile provides determinism artifacts');
...
test('Determinism gate: /conv/message returns stable hash/bytes_b64 across restarts', async () => {
  ...
  const env = { PORT: String(port), QUEUE_MAX: '0', LLM_TURN_BUDGET: '5', LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga', MAX_HEADER_BYTES: '8192' };
  const { child } = startService(env);
  const base = `http://localhost:${port}`;
  await waitForUp(base, { timeout: 5000 });
  ...
  if (r1.json.bytes_b64 !== r2.json.bytes_b64) {
    assert.fail(describeDiffB64(r1.json.bytes_b64, r2.json.bytes_b64, 'message.bytes_b64.in_session'));
  }
  try { child.kill('SIGTERM'); } catch {}
  await new Promise((r) => child.on('exit', r));
  ...
  if (r3.json.bytes_b64 !== r1.json.bytes_b64) {
    assert.fail(describeDiffB64(r3.json.bytes_b64, r1.json.bytes_b64, 'message.bytes_b64.cross_restart'));
  }
