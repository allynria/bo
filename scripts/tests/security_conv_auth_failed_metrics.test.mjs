...
test('auth_failed_total includes method and mac_id on 401s', async () => {
  ...
  const { child } = startService({ NODE_ENV: 'production', LOG_JSON: '1', PORT: String(port), CONV_AUTH: token, CONV_HMAC_SECRET: secret, CORS_ALLOWLIST: allowed, REPLAY_WINDOW_MS: '5000', METRICS_AUTH: metricsToken, LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga' });
  await waitForUp(base, { timeout: 3000 });

  // message: wrong token -> 401
  const ts1 = Date.now();
  const mBadTok = await postJson(`${base}/conv/message`, { text: 'hi', conv_id: 'mX', ts: ts1 }, { origin: allowed, authorization: 'Bearer wrong' });
  assert.equal(mBadTok.status, 401);

  // stream: wrong hmac -> 401
  const ts2 = Date.now();
  const badMac = crypto.createHmac('sha256', 'other').update(`GET:stream:${String(ts2)}`).digest('hex');
  const sBadMac = await getRaw(`${base}/conv/stream?text=hi&conv_id=sX&turn=0&ts=${ts2}`, { origin: allowed, 'x-client-mac': badMac, 'x-mac-id': 'kidA' });
  ...
  const hasMsgTokenFail = labels.some((l) => l.reason === 'token_invalid' && l.path === 'message' && l.method === 'token');
  const hasStrHmacFail = labels.some((l) => l.reason === 'hmac_invalid' && l.path === 'stream' && l.method === 'hmac' && l.mac_id === 'kidA');
...
test('auth_missing increments auth_failed_total for missing credentials', async () => {
  ...
  const metricsToken = 'metrics123';
  const token = 'topsecret';
  const { child } = startService({ NODE_ENV: 'production', LOG_JSON: '1', PORT: String(port), CONV_AUTH: token, CORS_ALLOWLIST: allowed, REPLAY_WINDOW_MS: '5000', METRICS_AUTH: metricsToken, LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga' });
  await waitForUp(base, { timeout: 3000 });

  const ts = Date.now();
  const r = await postJson(`${base}/conv/message`, { text: 'hi', conv_id: 'mY', ts }, { origin: allowed });
  assert.equal(r.status, 401);

  const m = await getRaw(`${base}/metrics`, { authorization: `Bearer ${metricsToken}` });
  assert.equal(m.status, 200);
  const parsed = JSON.parse(m.text);
  const labels = (parsed?.counters || []).filter((c) => c.name === 'auth_failed_total').map((c) => c.labels).filter(Boolean);
  const hasMissing = labels.some((l) => l.reason === 'auth_missing' && l.path === 'message' && l.method === 'none');
