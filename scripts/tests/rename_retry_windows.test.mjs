...
test('Rename retry succeeds on simulated EPERM and emits metrics', async () => {
  ...
  globalThis.UrgaCoreDeps.Metrics = {
    ...
    gauge: () => {},
    histogramMs: (_ctx, name, ms, labels = {}) => { captured.push({ event: `${name}.hist`, ms, labels }); },
  };
  ...
  assert.ok(retryMetric && Number(retryMetric.delta || 0) >= 2, 'rename retry metric should reflect attempts');
});
