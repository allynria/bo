...
import { summarizeWindow, stageBooster, listBoosters, consumeOne, makeBoosterId } from '../memory/booster.mjs';

const cid = 'booster-recall';
for (let i=1;i<=50;i++) {
  indexTurn({ convId: cid, role: i%2? 'user':'bot', text: `msg ${i} — ${i%5===0?'confession: i care':''}` });
}
const anchor = 30;
const recap = summarizeWindow({ convId: cid, anchor, before:20, after:20, pov:'she', maxChars: 400 });
assert.ok(recap && recap.length <= 400, 'recap length capped');
stageBooster({ convId: cid, id: makeBoosterId(anchor), anchor, range:[anchor-20,anchor+20], text:recap, ttlTurns: 2 });
