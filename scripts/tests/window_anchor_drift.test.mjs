...
test('Window anchor updates only after 2×window + 2×skew guard; no drift', async () => {
  ...
  const tmpBase = path.join(os.tmpdir(), `trae_rl_drift_${process.pid}_${Math.random().toString(36).slice(2)}`);
  ...
  Date.now = () => t0;
  try { await rl.allow(key); } finally { Date.now = realNow; }
  ...
  for (let i = 1; i <= iterations; i++) {
    ...
    Date.now = () => fakeNow;
    try { await rl.allow(key); } finally { Date.now = realNow2; }
    ...
    if (elapsed >= resetThreshold) {
      ...
      assert.equal(winStart, expectedAnchor, 'windowStart should advance to next anchor after threshold');
