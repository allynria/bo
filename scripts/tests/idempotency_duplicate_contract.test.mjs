...
const runRedisLockTests = ['1', 'true', 'yes'].includes(
  String(
    process.env.RUN_IDEMPOTENCY_REDIS_LOCK_TESTS || process.env.RUN_REDIS_LOCK_TESTS || ''
  ).toLowerCase()
);

(runRedisLockTests ? test : test.skip)(
  '409 duplicate_message: concurrent POST with same Idempotency-Key are rejected via lock',
  async () => {
