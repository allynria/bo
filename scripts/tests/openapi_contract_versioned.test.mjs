...
test('OpenAPI: /openapi.json includes /v1 paths and schemas validate /v1 responses', async () => {
  ...
  assert.ok(
    MessageRequest && MessageResponse && CompileRequest && CompileResponse,
  ...
  const msgOk = await postJson(`${base}/v1/conv/message`, {
    text: 'hello',
    engine: 'echo',
    conv_id: 'oapi-msg',
    turn: 0,
  ...
  const compOk = await postJson(`${base}/v1/conv/compile`, {
...
test('OpenAPI contract: unknown fields fail for /v1 routes with schema_invalid', async () => {
  ...
  const r1 = await postJson(`${base}/v1/conv/message`, {
    text: 'hi',
    conv_id: 'c-oapi-1',
    turn: 0,
    engine: 'urga',
  ...
  const r2 = await postJson(`${base}/v1/conv/compile`, {
