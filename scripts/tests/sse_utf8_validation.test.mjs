      ...
      res.on('data', (d) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (line.startsWith('event: ')) {
            ...
            const dataLine = lines[i + 1] && lines[i + 1].startsWith('data: ') ? lines[i + 1].slice(6) : '';
            if (evt === 'start') {
              try { startPayload = JSON.parse(dataLine); } catch {}
            } else if (evt === 'delta') {
              try { const p = JSON.parse(dataLine); if (typeof p?.text === 'string') deltas.push(p.text); } catch {}
            } else if (evt === 'end') {
              try { finalPayload = JSON.parse(dataLine); } catch {}
            }
          }
        }
        if (finalPayload) { resolve({ start: startPayload, deltas, end: finalPayload }); }
      });
      res.on('end', () => { if (!finalPayload) resolve({ start: startPayload, deltas, end: null }); });
    });
    req.on('error', reject);
  });
}
...
test('SSE sanitizer replaces invalid UTF-8 in deltas and final', async () => {
  ...
  const env = { PORT: String(port), QUEUE_MAX: '0', LLM_TURN_BUDGET: '5', LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-invalid' };
  ...
  assert.ok(agg.includes('\uFFFD') || agg.includes('�'), 'delta aggregator contains replacement character');
  assert.ok(sse.end && typeof sse.end.final === 'string', 'end has final');
  assert.ok(sse.end.final.includes('\uFFFD') || sse.end.final.includes('�'), 'final contains replacement character');
