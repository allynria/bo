...
test('auth_accepted_total includes mac_id labels for HMAC lineage', async () => {
  ...
  const macMsg = crypto
    ...
    .update(`POST:message:${String(ts1)}`)
    .digest('hex');
  const r1 = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'm1', ts: ts1 },
    { origin: allowed, 'x-client-mac': macMsg, 'x-mac-id': 'new' }
  );
  ...
  const macStr = crypto
    ...
    .digest('hex');
  const r2 = await getRaw(`${base}/conv/stream?text=hi&conv_id=s1&turn=0&ts=${ts2}`, {
    origin: allowed,
    'x-client-mac': macStr,
    'x-mac-id': 'old',
  });
  ...
  const hasNew = labels.some(
    (l) => l.method === 'hmac' && l.mac_id === 'new' && l.path === 'message'
  );
  const hasOld = labels.some(
    (l) => l.method === 'hmac' && l.mac_id === 'old' && l.path === 'stream'
  );
