...
function connectAbortQuick(base, conv_id) {
  return new Promise((resolve) => {
    const u = new URL(`${base}/conv/stream?conv_id=${encodeURIComponent(conv_id)}&turn=0&engine=urga&text=quick`);
    const req = http.request({ hostname: u.hostname, port: u.port, path: u.pathname + u.search, method: 'GET' }, (res) => {
      ...
      res.on('data', (d) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          const line = lines[i];
          if (line.startsWith('event: delta')) { seenDelta = true; try { req.destroy(); } catch {} resolve(); break; }
        }
      });
      res.on('end', () => resolve());
    });
...
test('SSE memory stays flat under repeated forced disconnects', async () => {
  ...
  const env = { PORT: String(port), QUEUE_MAX: '0', LLM_TURN_BUDGET: '5', LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga', SSE_HEARTBEAT_MS: '1000', IDEMPOTENCY_TTL_MS: '1200', ACTIVE_STREAMS_MAX_ITEMS: '100' };
  ...
  assert.ok(activeGauge.value >= 0 && activeGauge.value <= 10, 'active_streams_current bounded after soaks');
