...
test('Tenant propagates into engine_selected log via context', async () => {
  ...
  const env = {
    ...
    MAX_HEADER_BYTES: String(100_000),
  };
  ...
  try {
    ...
    if (headerChecks.length > 0) {
      ...
      assert.ok(
        last.headerBytes <= last.MAX_HEADER_BYTES,
        `headerBytes ${last.headerBytes} > cap ${last.MAX_HEADER_BYTES}`
      );
    }
    ...
    assert.equal(
      logEntry.tenant,
      sanitizedTenant,
      'tenant should be surfaced and sanitized in engine_selected log'
    );
  } finally {
    try {
      child.kill('SIGTERM');
