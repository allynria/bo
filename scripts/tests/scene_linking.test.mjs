...
test('scene tagging + revisit injects scene memory', async ()=>{
  ...
  try {
    ...
    const f = await (await fetch(`${BASE}/__test/fact`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ conv_id, text:'They confessed their secret in the Old Harbor library', weight:0.9, score:0.8 })})).json();
    const fid = f.fact.id;

    // link scene explicitly
    const sceneResp = await (await fetch(`${BASE}/memory/scene`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ conv_id, scene:'Old Harbor Library', fact_ids:[fid], booster:'(You are back in the Old Harbor library.)' })})).json();
    assert.equal(sceneResp.ok, true);

    // visit scene in stream
    const res = await fetch(`${BASE}/conv/stream?engine=urga&conv_id=${conv_id}&turn=2&text=${encodeURIComponent('We walk into the Old Harbor Library once more')}`, { headers:{ Accept:'text/event-stream' } });
