...
async function postJSON(path, body, headers = {}) {
  ...
  const opts = { hostname:'127.0.0.1', port:3456, path, method:'POST',
...
async function getMetrics() {
  return new Promise((resolve,reject)=>{
    http.get('http://127.0.0.1:3456/metrics', res=>{
...
(async () => {
  ...
  async function waitForReady(maxMs = 2000) {
    ...
    while (Date.now() - start < maxMs) {
      const ok = await new Promise((resolve) => {
        try {
          http.get('http://127.0.0.1:3456/healthz', res => { resolve(res.statusCode === 200); }).on('error', () => resolve(false));
        } catch { resolve(false); }
      });
      if (ok) return;
  ...
  const a = await postJSON('/v1/conv/message', { conv_id: conv, text: 'She smiles softly and nods.', ts: Date.now() }, { origin: 'http://ok.test', authorization: 'Bearer t' });
  assert.equal(a.status, 200);
  const b = await postJSON('/v1/conv/message', { conv_id: conv, text: 'She smiles softly and nods again.', ts: Date.now() }, { origin: 'http://ok.test', authorization: 'Bearer t' });
  assert.equal(b.status, 200);
  const m = await getMetrics();
  const hit = (m.counters||[]).some(c => c.name === 'loopguard_trigger_total' && c.labels?.path === 'message');
