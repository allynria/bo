...
test('Shared rate limiter fairness under ±250ms skew with anchored windows', async () => {
  ...
  const internalTotal =
    Number(w1.internal || 0) + Number(w2.internal || 0) + Number(w3.internal || 0);
  const rlBlockedTotal =
    Number(w1.rlBlocked || 0) + Number(w2.rlBlocked || 0) + Number(w3.rlBlocked || 0);
  ...
  assert.equal(
    totalRequests,
    okTotal + internalTotal + rlBlockedTotal,
