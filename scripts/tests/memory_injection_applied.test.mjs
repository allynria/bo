...
function onceReady() {
  return new Promise((resolve) => {
    const t = setInterval(() => {
      http
        .get({ host: '127.0.0.1', port: PORT, path: '/healthz' }, (r) => {
          if (r.statusCode === 200) {
            clearInterval(t);
            resolve();
          }
...
async function main() {
  ...
  // Message path
  const m = await postJSON('/v1/conv/message', {
    conv_id: conv,
    engine: 'urga',
    text: 'We crossed the bridge again.',
    ts: Date.now(),
  });
    ...
    const req = http.request(
      {
        ...
        method: 'GET',
        headers: { origin: ' `http://ok.test` ', authorization: 'Bearer t' },
      },
      (res) => {
        let startLine = '';
        res.on('data', (c) => {
