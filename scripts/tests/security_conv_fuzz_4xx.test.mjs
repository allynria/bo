...
test('fuzz invalid headers and ts skew taxonomy', async () => {
  ...
  const { child } = startService({
    ...
    REPLAY_WINDOW_MS: '50',
    REPLAY_SKEW_TOLERANCE_MS: '10000',
    LLM_TEST_STUBS: '1',
  ...
  const cases = [
    ...
    () =>
      postJson(
        `${base}/conv/message`,
        { text: 'hi', conv_id: 'm4', ts: now - 20000 },
        { origin: allowed, authorization: `Bearer ${token}` }
      ), // stale beyond tolerance
    // Stream variations
    () => getRaw(`${base}/conv/stream?text=hi&conv_id=s1&turn=0&ts=${now}`, { origin: allowed }), // no auth
    () =>
      getRaw(`${base}/conv/stream?text=hi&conv_id=s2&turn=0&ts=${now}`, {
        origin: 'http://evil.test',
        authorization: `Bearer ${token}`,
      }), // bad CORS
    () =>
      getRaw(`${base}/conv/stream?text=hi&conv_id=s3&turn=0`, {
        origin: allowed,
        authorization: `Bearer ${token}`,
      }), // missing ts
    () =>
      getRaw(`${base}/conv/stream?text=hi&conv_id=s4&turn=0&ts=${now - 20000}`, {
        origin: allowed,
        authorization: `Bearer ${token}`,
      }), // stale beyond tolerance
  ];
  ...
  const okMsg = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'm5', ts: now - 500 },
    { origin: allowed, authorization: `Bearer ${token}` }
  );
  assert.equal(okMsg.status, 200);
  const okStr = await getRaw(`${base}/conv/stream?text=hi&conv_id=s5&turn=0&ts=${now - 500}`, {
    origin: allowed,
    authorization: `Bearer ${token}`,
  });
...
test('HMAC rotation supports multiple secrets', async () => {
  ...
  const { child } = startService({
    ...
    LOG_JSON: '1',
    PORT: String(port),
    CONV_HMAC_SECRETS: `${oldSec},${newSec}`,
    CORS_ALLOWLIST: allowed,
  ...
  const ts = Date.now();
  const macOldMsg = crypto
    .createHmac('sha256', oldSec)
    .update(`POST:message:${String(ts)}`)
    .digest('hex');
  const macNewStr = crypto
    .createHmac('sha256', newSec)
    .update(`GET:stream:${String(ts)}`)
    .digest('hex');
  const macBadMsg = crypto
    .createHmac('sha256', 'bad')
    .update(`POST:message:${String(ts)}`)
    .digest('hex');

  const okMsg = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'mm', ts },
    { origin: allowed, 'x-client-mac': macOldMsg }
  );
  assert.equal(okMsg.status, 200, 'message accepts old secret');
  const okStr = await getRaw(`${base}/conv/stream?text=hi&conv_id=ss&turn=0&ts=${ts}`, {
    origin: allowed,
    'x-client-mac': macNewStr,
  });
  assert.equal(okStr.status, 200, 'stream accepts new secret');

  const badMsg = await postJson(
    `${base}/conv/message`,
    { text: 'hi', conv_id: 'bad', ts },
    { origin: allowed, 'x-client-mac': macBadMsg }
  );
