...
test('per-tenant limiter returns 429 for excess requests within window', async () => {
  ...
  const env = {
    ...
    QUEUE_MAX: '0',
    TENANT_LIMIT: '2',
    TENANT_WINDOW_MS: '500',
    TENANT_INTERNAL_ERROR_ONCE: '0',
    LOG_JSON: '1',
  };
  ...
  const tenant429 =
    counters.find((c) => c.name === 'rate_limited_total' && c.labels?.reason === 'tenant')?.value ||
    0;
  const resp429 =
    counters.find((c) => c.name === 'responses_total' && c.labels?.status === '429')?.value || 0;
