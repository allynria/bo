...
test('Idempotency HMAC enforcement: missing/invalid MAC rejected; valid accepted and replays', async () => {
  ...
  // Missing MAC -> 401
  const miss = await postJsonWithHeaders(`${base}/conv/message`, body, {
  ...
  const bad = await postJsonWithHeaders(`${base}/conv/message`, body, {
    'Idempotency-Key': idemKey,
  ...
  const ok1 = await postJsonWithHeaders(`${base}/conv/message`, body, {
    'Idempotency-Key': idemKey,
    'Idempotency-MAC': expected,
  });
  assert.equal(ok1.status, 200);
  assert.ok(
    ok1.json && ok1.json.hash && ok1.json.bytes_b64,
    'first response includes determinism artifacts'
  );

  const ok2 = await postJsonWithHeaders(`${base}/conv/message`, body, {
    'Idempotency-Key': idemKey,
    'Idempotency-MAC': expected,
  });
