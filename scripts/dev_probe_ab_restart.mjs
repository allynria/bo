  ...
  return new Promise((resolve, reject) => {
    const tryOnce = () => {
      ...
      req.on('error', () => {
        ...
        else reject(new Error('service_up_error'));
      });
      req.end();
    };
    tryOnce();
...
async function main() {
  ...
  for (let i = 0; i < 300; i++) {
    ...
    if (r.status !== 200) {
      console.log(JSON.stringify({ ok: false, stage: 'first_boot_message', status: r.status }));
      process.exit(1);
    }
  ...
  for (let i = 0; i < convs.length; i++) {
    ...
    if (s.status !== 200) {
      console.log(JSON.stringify({ ok: false, stage: 'first_boot_stream', status: s.status }));
      process.exit(1);
    }
  }
  ...
  for (let i = 0; i < 300; i++) {
    ...
    const r = await postJson(`${base}/conv/message`, body);
    if (r.status !== 200) {
      mismatches.push({ cid, type: 'status', status: r.status });
      continue;
    }
    const v = String(r.json.variant_v || '');
  ...
  for (let i = 0; i < convs.length; i++) {
    ...
    const s = await fetchSSE(u);
    if (s.status !== 200) {
      streamMismatches.push({ cid, type: 'status', status: s.status });
      continue;
    }
  ...
  const summary = {
    ok: mismatches.length === 0 && streamMismatches.length === 0,
    mismatches,
    streamMismatches,
    sample: { message: mismatches[0] || null, stream: streamMismatches[0] || null },
  };
