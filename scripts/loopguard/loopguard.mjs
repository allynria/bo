...
function styleCycle(state, tokens) {
  const arr = String(tokens||'').split(',').map(s=>s.trim()).filter(Boolean);
...
export async function loopGuardDecide({ convId, candidate, prevBotReplies, cfg }) {
  if (!cfg.enabled) return { shouldReroll:false, reason:'disabled', sim:0, style:null, booster:null };
  ...
  if (sim >= cfg.simThresh) {
    ...
    return { shouldReroll:true, reason:'delta_sim', sim, style, booster: buildStyleBooster(style) };
  }
