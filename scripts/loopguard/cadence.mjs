...
export function getCadenceCfg(){
  return {
    ...
    styleTokens: String(process.env.CADENCE_STYLE_TOKENS || process.env.LOOP_GUARD_STYLE_TOKENS || 'descriptive,poetic,terse,inner-thought'),
...
export function pushTurn(convId, role='user', text='', cfg=getCadenceCfg()){
  ...
  const s = get(convId);
  const now = Date.now();
  if (role === 'user') s.lastUserTs = now; else if (role === 'bot') s.lastBotTs = now;
...
export function chooseStyleForNext(convId, cfg=getCadenceCfg()){
  ...
  const s = get(convId);
  // Cycle style tokens
  const tokens = String(cfg.styleTokens||'').split(',').map(t=>t.trim()).filter(Boolean);
