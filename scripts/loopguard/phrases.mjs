...
export function getPhraseCfg() {
  return {
    ...
    seeds: String(process.env.PHRASE_LIST || '')
      .split(';')
      .map(norm)
      .filter(Boolean),
  };
...
export function getPenalties(convId, cfg = getPhraseCfg()) {
  ...
  for (const [p, v] of m.entries()) {
    ...
    if (val >= cfg.maxScore) {
      penaltyHints.push(
        `(AVOID) Refrain from overused phrase "${p}". Use a fresh, specific alternative.`
      );
    }
  }
  return { penaltyHints };
