...
export async function runStyleHedgeStream({
  ...
  const ctxPrimary = initBotContext({
    ...serviceCtx,
    vars: { ...(serviceCtx.vars || {}), conv_id: convId },
  });
  const ctxBackup = initBotContext({
    ...serviceCtx,
    vars: { ...(serviceCtx.vars || {}), conv_id: convId, __style_hedge: chosen },
  });

  let winner = null;
  let primaryEnded = false,
    backupEnded = false;
  let cancelPrimary = null,
    cancelBackup = null;
    ...
    const onStart = (p) => {
      if (!startWritten) {
        ...
        write('start', {
          ...
          style: tag === 'backup' ? chosen : undefined,
        });
      }
    };
    ...
    return (payload) => {
      if (!firstDeltaResolved) {
        ...
        if (tag === 'backup') {
          ...
          try {
            serviceCtx?.io?.events?.emit?.('style.hedge.switch', { style: chosen });
          } catch {}
  ...
  const primaryPromise = (async () => {
    try {
      const final = await LLMService.stream(
        text,
        { model: engine, signal: acPrimary.signal },
        ctxPrimary
      );
      primaryEnded = true;
      return final;
  ...
  const backupPromise = (async () => {
    ...
    try {
      serviceCtx?.io?.events?.emit?.('style.hedge.start', {
        style: chosen,
        delay_ms: Number.isFinite(hedgeDelay) ? hedgeDelay : 0,
      });
    } catch {}
    try {
      const final = await LLMService.stream(
        `${booster}\n\n${text}`,
        { model: engine, signal: acBackup.signal },
        ctxBackup
      );
      backupEnded = true;
      return final;
  ...
  await Promise.race([primaryPromise, backupPromise]).catch(() => {
    /* errors handled by SSE error path upstream if needed */
  });
  await cancelLoser;
  // We don't need to await both streams; SSE end events will carry the final.
  return winner === 'backup'
    ? await backupPromise.catch(() => '')
    : await primaryPromise.catch(() => '');
}
