...
async function ensureBase() {
  await AsyncFS.mkdir(BASE, { recursive: true });
}
...
async function loadLog(convId) {
  ...
  try {
    return JSON.parse(await AsyncFS.readFile(f, 'utf8'));
  } catch {
    return { convId, turns: [], detections: [] };
  }
}
...
function extractAtoms(s) {
  ...
  const verbs = tokens.filter((w) =>
    /(walk|run|stand|open|close|die|kill|hug|kiss|jump|fall|talk|speak|resurrect|unlock|lock|enter|leave|push|pull)/.test(
      w
    )
  );
...
function detectTemporal(turns, userText) {
  ...
  const lastDeath = [...turns]
    .reverse()
    .find((t) => /you.*(die|died)|dead now|you are dead/.test(norm(t.text)));
...
function detectState(turns, userText) {
  ...
  const lastLock = [...turns]
    .reverse()
    .find((t) => /(door|gate|chest).*(locked|sealed)/.test(norm(t.text)));
  if (
    lastLock &&
    hasAny(userText, ['walk through', 'go through', 'open the door', 'swing the gate'])
  ) {
...
function detectEntity(turns, userText) {
  ...
  const lastHate = [...turns]
    .reverse()
    .find((t) => /(she|he|they).*(hate|despise)/.test(norm(t.text)));
  if (lastHate && hasAny(userText, ['she hugs me', 'she kisses me', 'she is affectionate'])) {
    return {
      type: 'entity',
      reason: 'affection after hatred without transition',
      evidence: lastHate.text,
    };
  }
  return null;
}
...
export async function detectContradictions(convId, userText) {
  ...
  const hits = [];
  const t = detectTemporal(recent, userText);
  if (t) hits.push(t);
  const s = detectState(recent, userText);
  if (s) hits.push(s);
  const e = detectEntity(recent, userText);
  if (e) hits.push(e);
...
export function buildContradictionLines(hits, style = 'inline', strict = false) {
  ...
  if (style === 'system') {
    return [
      ...
      ...hits
        .slice(0, 2)
        .map(
          (h) => `SYSTEM: Violation [${h.type}] — ${h.reason}. Consider addressing or refusing.`
        ),
    ];
  }
  ...
  const lines = hits
    .slice(0, 2)
    .map((h) => `(${h.type.toUpperCase()} inconsistency: ${h.reason}. Address it in-character.)`);
