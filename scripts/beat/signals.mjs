...
export function collectBeatSignals(convId = '', ctx = null, inputs = {}) {
  ...
  const tension =
    typeof ctxVars.tension === 'number'
      ? ctxVars.tension
      : typeof ctx?.memory?.tension === 'number'
        ? ctx.memory.tension
        : null;
  ...
  if (drag == null) {
    try {
      const d = computeDragScore({
        loopScore: num(loopScore, 0),
        deltaSim: num(deltaSim, 0),
        dwellMs: num(dwellMs, 0),
        tensionVar: num(tensionVar, 0),
      });
