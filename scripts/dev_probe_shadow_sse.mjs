...
function fetchSSE(url) {
  return new Promise((resolve) => {
    const u = new URL(url);
    const events = [];
    const req = http.request({ method: 'GET', hostname: u.hostname, port: u.port, path: u.pathname + (u.search || ''), headers: { Accept: 'text/event-stream' } }, (res) => {
      let buf = '';
      ...
      res.on('data', (d) => {
        ...
        for (const chunk of chunks) {
          ...
          let payload = null; try { payload = JSON.parse(dataStr); } catch {}
