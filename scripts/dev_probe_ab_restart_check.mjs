...
function postJson(url, body) {
  return new Promise((resolve) => {
    const u = new URL(url);
    const req = http.request({ method: 'POST', hostname: u.hostname, port: u.port, path: u.pathname + (u.search || ''), headers: { 'Content-Type': 'application/json' } }, (res) => {
      let buf = '';
    ...
    const req = http.request({ method: 'GET', hostname: u.hostname, port: u.port, path: u.pathname + (u.search || ''), headers }, (res) => {
      ...
      res.on('data', (d) => {
        ...
        for (const chunk of chunks) {
          ...
          if (evt === 'start') { try { startEvent = JSON.parse(dataStr); } catch {} }
          else if (evt === 'end') { try { endEvent = JSON.parse(dataStr); } catch {} }
        }
      });
...
async function main() {
  ...
  const r1 = await postJson(`${base}/conv/message`, { text: 'hello', conv_id: cid, turn: 0, engine: 'urga' });
  ...
  if (!(v1 === 'A' || v1 === 'B')) {
    ...
    console.error(JSON.stringify({ ok: false, where: 'first_message', status: r1.status, json: r1.json, logs: getLogs1() }));
    throw new Error(`invalid variant '${v1}'`);
  }

  const s1 = await fetchSSE(`${base}/conv/stream?conv_id=${encodeURIComponent(cid)}&turn=0&engine=urga&text=${encodeURIComponent('hello')}`);
  ...
  const s2 = await fetchSSE(`${base}/conv/stream?conv_id=${encodeURIComponent(cid)}&turn=1&text=${encodeURIComponent('after restart')}`);
  if (s2.status !== 200) {
    console.error(JSON.stringify({ ok: false, where: 'second_stream', status: s2.status, logs: getLogs2() }));
