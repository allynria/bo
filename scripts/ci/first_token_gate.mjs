      ...
      res.on('data', (d) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (line.startsWith('event: ')) {
            ...
            if (evt === 'delta' && !resolved) { resolved = true; resolve({ ok: true, status }); }
          }
        }
      });
...
async function main() {
  ...
  while (i < N) {
    ...
    for (let k = 0; k < concurrency && i < N; k++, i++) {
      ...
      batch.push(fetchSSEFirstDelta(u).then((res) => {
...
main().catch((e) => { console.error('first_token_gate_error', e && e.stack || e); process.exit(1); });
