...
function fetchSSEFirstDelta(url) {
  return new Promise((resolve, reject) => {
    const u = new URL(url);
    const req = http.get(
      ...
      (res) => {
        ...
        res.on('end', () => {
          if (!resolved) resolve({ ok: false, status });
        });
      }
    );
...
async function main() {
  ...
  const thresholdMs = Math.max(
    1,
    Number(process.env.FIRST_TOKEN_THRESHOLD_MS || ecfg.first_token_threshold_ms || 300)
  );
  const minPctUnder = Math.max(
    0,
    Math.min(
      100,
      Number(process.env.FIRST_TOKEN_MIN_PCT_UNDER || ecfg.first_token_min_pct_under || 90)
    )
  );
  ...
  const child = spawnService
    ? startService({
        ...
        PORT: String(port),
        QUEUE_MAX: '0',
        PREWARM_MODELS: String(process.env.PREWARM_MODELS || '') || undefined,
        LLM_TEST_STUBS: String(process.env.LLM_TEST_STUBS || '1'),
        URGA_PROVIDER: String(process.env.URGA_PROVIDER || 'stub-urga'),
        LLM_TURN_BUDGET: String(process.env.LLM_TURN_BUDGET || '5'),
        CONV_RATE_MAX: String(process.env.CONV_RATE_MAX || '1000'),
        CONV_RATE_WINDOW: String(process.env.CONV_RATE_WINDOW || '2000'),
        CONV_SOFT_MAX: String(process.env.CONV_SOFT_MAX || '5000'),
        CONV_SOFT_WINDOW_MS: String(process.env.CONV_SOFT_WINDOW_MS || '2000'),
        CONV_AUTH: convAuth,
        CORS_ALLOWLIST: '',
        METRICS_AUTH: '',
      })
  ...
  while (i < N) {
    ...
    for (let k = 0; k < concurrency && i < N; k++, i++) {
      const text = `gate-${i}-${Math.random().toString(36).slice(2, 8)}`;
      const u = `${base}/conv/stream?conv_id=fm-${i}&turn=0&engine=${encodeURIComponent(engine)}&text=${encodeURIComponent(text)}${auth ? `&auth=${encodeURIComponent(auth)}` : ''}`;
      batch.push(
        fetchSSEFirstDelta(u)
          .then((res) => {
            const st = Number(res?.status || 0);
            statusHist[st] = (statusHist[st] || 0) + 1;
            if (res?.ok) deltasSeen++;
            return res?.ok || false;
          })
          .catch(() => {
            statusHist[0] = (statusHist[0] || 0) + 1;
            return false;
          })
      );
    }
  ...
  if (m.status !== 200 || !m.json) {
    console.error('FIRST_TOKEN_GATE_FAIL: metrics unavailable', { status: m.status });
    try {
      child?.kill?.('SIGTERM');
    } catch {}
    process.exit(1);
  ...
  const ft = counters.filter((c) => c.name === 'first_token_ms_bucket');
  const total = ft.reduce((acc, c) => acc + Number(c.value || 0), 0);
  const under = ft
    .filter((c) => {
  ...
  const out = {
    gate: 'first_token_distribution',
    engine,
    threshold_ms: thresholdMs,
    min_pct_under: minPctUnder,
    total,
    under,
    pct_under: Math.round(pctUnder),
    deltas_seen: deltasSeen,
    requests: N,
    concurrency,
...
main().catch((e) => {
  console.error('first_token_gate_error', (e && e.stack) || e);
  process.exit(1);
});
