...
async function main() {
  ...
  const thresholdMs = Math.max(
    1,
    Number(process.env.STREAM_DURATION_THRESHOLD_MS || ecfg.stream_duration_threshold_ms || 5000)
  );
  const minPctUnder = Math.max(
    0,
    Math.min(
      100,
      Number(process.env.STREAM_DURATION_MIN_PCT_UNDER || ecfg.stream_duration_min_pct_under || 90)
    )
  );
  ...
  const out = {
    gate: 'stream_duration_distribution',
    engine,
...
main().catch((e) => {
  console.error('stream_duration_gate_error', (e && e.stack) || e);
  process.exit(1);
});
