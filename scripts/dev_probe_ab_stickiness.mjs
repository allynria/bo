  ...
  while ((Date.now() - start) < timeout) {
    try {
      ...
      const ok = await new Promise((resolve) => {
        const req = http.request({ method: 'GET', hostname: u.hostname, port: u.port, path: '/healthz' }, (res) => {
...
async function main() {
  ...
  const env = { PORT: String(port), QUEUE_MAX: '0', LLM_TURN_BUDGET: '5', LLM_TEST_STUBS: '1', URGA_PROVIDER: 'stub-urga', CONV_RATE_MAX: '1000', CONV_RATE_WINDOW: '2000' };
  ...
  for (let i = 0; i < N; i++) {
    ...
    tasks.push(postJson(`${base}/conv/message`, body).then((r) => {
      ...
      else if (body?.ctx?.vars?.engine && src !== 'ctx') badSource.push({ cid, src, expected: 'ctx' });
      else if (!body.engine && !body?.ctx?.vars?.engine && !(src === 'heuristic' || src === 'default')) badSource.push({ cid, src, expected: 'heuristic|default' });
    }));
  }
